...
const evaluate = (command: unknown) => evaluateCommandGate(decodeInput({
...
describe("shared command gate", () => {
  ...
  it("rejects unauthenticated commands before policy evaluation", () => {
    const input = decodeInput({
      envelope: {
        ...
        command: { _tag: "RequestAssetDownload", assetId: "asset-1", idempotencyKey: "intent-anonymous" },
      },
    ...
    if (decision._tag === "Rejected" && decision.failure._tag === "AuthenticationFailure") {
  ...
  it("allows owner preview without the control lease", () => {
    const input = decodeInput({
      envelope: {
        commandId: "command-2",
        command: { _tag: "PreviewRunMutation", runId: "run-1", expectedRunRevision: 12, proposedChange: { _tag: "AppendFutureSequence", sequence: m31Sequence } },
      },
      actor: ownerController,
      connected: true,
      snapshotVersion: 20,
    ...
    assert.equal(CommandGateDecision.$is("Accepted")(evaluateCommandGate(input)), true)
  })
  ...
  it("rejects an old controller command after takeover before domain action", () => {
    const input = decodeInput({
      envelope: {
        commandId: "command-3",
        command: { _tag: "SkipAcquireTarget", runId: "run-1", expectedRunRevision: 12, expectedLeaseRevision: 4, expectedAcquireRevision: 7, idempotencyKey: "intent-3" },
      },
    ...
    if (decision._tag === "Rejected") {
      ...
      if (decision.failure._tag === "AuthorizationFailure") assert.equal(decision.failure.reason, "ControlLeaseLost")
    }
  })
  ...
  it("rejects a phone mutation even for the owner", () => {
    const input = decodeInput({
      envelope: {
        commandId: "command-5",
        command: { _tag: "UndoProcessingStep", sessionId: "process-1", expectedProcessingRevision: 9, idempotencyKey: "intent-5" },
      },
    ...
    const decision = evaluateCommandGate(input)
    assert.equal(decision._tag, "Rejected")
    if (decision._tag === "Rejected" && decision.failure._tag === "AuthorizationFailure") {
  ...
  it("allows an authorized read-only viewer to request a download", () => {
    const input = decodeInput({
      envelope: {
        commandId: "command-6",
        command: { _tag: "RequestAssetDownload", assetId: "asset-1", idempotencyKey: "intent-6" },
      },
      actor: { ...ownerController, personId: "person-viewer", clientId: "client-phone", role: "viewer", capability: "readOnly" },
      connected: true,
      snapshotVersion: 20,
      currentRevisions,
      idempotency: { _tag: "Fresh" },
  ...
  it("does not let viewer membership perform owner Process work", () => {
    const input = decodeInput({
      envelope: {
        ...
        command: { _tag: "ResumeProcessingSession", sessionId: "process-1", expectedProcessingRevision: 9 },
      },
      actor: { ...ownerController, personId: "person-viewer", clientId: "client-viewer", role: "viewer" },
      connected: true,
      snapshotVersion: 20,
      currentRevisions,
      idempotency: { _tag: "Fresh" },
  ...
  it("returns a recorded result without repeating accepted work", () => {
    const input = decodeInput({
      envelope: {
        commandId: "command-7",
        command: { _tag: "ApplyRunMutation", runId: "run-1", expectedRunRevision: 11, expectedLeaseRevision: 3, previewId: "preview-1", idempotencyKey: "intent-7" },
      },
      actor: ownerController,
      connected: true,
      snapshotVersion: 20,
    ...
    assert.equal(CommandGateDecision.$is("ReplayRecorded")(evaluateCommandGate(input)), true)
  })
  ...
  it("returns an existing pending operation without accepting duplicate work", () => {
    const input = decodeInput({
      envelope: {
        ...
        command: { _tag: "ApplyProcessingPreview", sessionId: "process-1", expectedProcessingRevision: 8, previewId: "preview-1", idempotencyKey: "intent-pending" },
      },
      actor: ownerController,
      connected: true,
      snapshotVersion: 20,
    ...
    const decision = evaluateCommandGate(input)
    assert.deepEqual(decision, CommandGateDecision.ReplayPending({
  ...
  it("rejects idempotency-key reuse with different input", () => {
    const input = decodeInput({
      envelope: {
        commandId: "command-8",
        command: { _tag: "DiscardProcessingSession", sessionId: "process-1", expectedProcessingRevision: 9, confirmationId: "discard-1", idempotencyKey: "intent-used" },
      },
      actor: ownerController,
      connected: true,
      snapshotVersion: 20,
    ...
    const decision = evaluateCommandGate(input)
    assert.equal(decision._tag, "Rejected")
    if (decision._tag === "Rejected") assert.equal(decision.failure._tag, "IdempotencyConflict")
  })
})
