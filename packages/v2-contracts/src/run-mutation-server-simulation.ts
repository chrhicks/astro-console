...
export interface RunMutationProjection extends Schema.Schema.Type<
  typeof RunMutationProjection
> {}
...
export interface RunMutationResponse extends Schema.Schema.Type<
  typeof RunMutationResponse
> {}
...
>
type ApplyCommand =
  | typeof Command.cases.ApplyRunMutation.Type
  | typeof Command.cases.ApproveDisruptiveRunMutation.Type
...
export interface RunMutationServerSimulation {
  readonly preview: (
    ...
    typeof RunPreviewResponse.Type,
    Schema.SchemaError | RunCommandRejected | RunMutationRejected
  >
  readonly apply: (
    rawRequest: unknown,
    actor: MemberActor,
  ...
  readonly dispatchOutbox: <Error>(
    execute: (work: typeof RunWork.Type) => Effect.Effect<void, Error>,
...
export const makeRunMutationServerSimulation = Effect.fn('RunSimulation.make')(
  function* (initialState: RunMutationSimulationState, nowEpochMs: number) {
    ...
    const preview: RunMutationServerSimulation['preview'] = Effect.fn(
      'RunSimulation.preview',
    )(function* (rawRequest, actor) {
      const envelope =
        yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(
        Command.cases.PreviewRunMutation,
      )(envelope.command)
      ...
      return yield* simulation.transact((current) =>
        Effect.gen(function* () {
          const gate = gateCommand(
            ...
            envelope,
            actor,
            IdempotencyState.cases.Fresh.make({}),
          )
          ...
          const decision = previewRunMutation({
            ...
            forecast: {
              completionDeltaSeconds: 1_620,
              viabilityChanges: ['M31 loses 14 minutes of useful altitude'],
            },
          })
    ...
    const apply: RunMutationServerSimulation['apply'] = Effect.fn(
      'RunSimulation.apply',
    )(function* (rawRequest, actor) {
      const envelope =
        yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(
        Command.cases.ApplyRunMutation,
      )(envelope.command)
      return yield* simulation.transact((current) =>
        acceptMutationTransaction(
          current,
          envelope,
          command,
          actor,
    ...
    const approve: RunMutationServerSimulation['approve'] = Effect.fn(
      'RunSimulation.approve',
    )(function* (rawRequest, actor) {
      const envelope =
        yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(
        Command.cases.ApproveDisruptiveRunMutation,
      )(envelope.command)
      return yield* simulation.transact((current) =>
        acceptMutationTransaction(
          current,
    ...
    } satisfies RunMutationServerSimulation
  },
)
...
function commitPreviewDecision(
  ...
  AtomicCommit<RunMutationSimulationState, typeof RunPreviewResponse.Type>
> {
  return RunMutationPreviewDecision.$match(decision, {
    Ineligible: ({ consequences }) =>
      Effect.succeed({
        state: current,
        result: RunPreviewResponse.cases.Ineligible.make({
    ...
    Previewed: ({ preview }) => {
      ...
      return Effect.succeed({
        state: next,
        result: RunPreviewResponse.cases.Previewed.make({
          preview,
...
function acceptMutationTransaction(
  ...
  AtomicCommit<RunMutationSimulationState, RunMutationResponse>,
  RunCommandRejected | RunMutationRejected
> {
  ...
  const gate = gateCommand(
    ...
    envelope,
    actor,
    gateIdempotency(classification),
  )
  ...
  return CommandGateDecision.$match(gate, {
    ...
    Accepted: () => {
      const preview = current.previews.find(
        (candidate) => candidate.previewId === command.previewId,
      )
      ...
      const approvalId = Command.guards.ApproveDisruptiveRunMutation(command)
        ? command.approvalId
        : undefined
...
function commitMutation(
  ...
  RunMutationRejected
> {
  const decision = applyRunMutation(
    current.run,
    current.definition,
    preview,
    nowEpochMs,
...
function rejectMutation(
  current: RunMutationSimulationState,
  reason: RunMutationRejected['reason'],
) {
  return Effect.fail(
    new RunMutationRejected({
      reason,
...
function replayMutation(
  ...
  return Effect.succeed({
    state: current,
    result: RunMutationResponse.make({
      ...stored.response,
      replayed: true,
      projection: project(current),
    }),
  ...
  return CommandGateDecision.$match(gate, {
    ...
    ReplayPending: () =>
      Effect.fail(
        new RunMutationRejected({
          reason: 'PreviewUnavailable',
          currentRunRevision: RunRevision.make(0),
        }),
      ),
    ReplayRecorded: () =>
      Effect.fail(
...
function mutationInputHash(command: ApplyCommand) {
  return NormalizedInputHash.make(
    JSON.stringify([
      ...
      command.previewId,
      ...(Command.guards.ApproveDisruptiveRunMutation(command)
        ? [command.approvalId]
        : []),
    ]),
  )
