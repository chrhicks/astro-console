...
import {
  AcquireServerSimulation,
  AcquireWork,
  makeAcquireServerSimulation,
} from './acquire-server-simulation.js'
...
const exhaustOrdinarySolveBudget = Effect.fn('AcquireProof.exhaustBudget')(
  function* (server: AcquireServerSimulation) {
    yield* server.completeSolve(
      completion('solve-1', noSolution('diag-1'), 'solve-2'),
    )
    yield* server.completeSolve(
      completion('solve-2', noSolution('diag-2'), 'solve-3'),
    )
    yield* server.completeSolve(
      completion('solve-3', noSolution('diag-3'), 'unused'),
    )
...
describe('Acquire server proofs', () => {
  it('ACQ-01 and ACQ-05 preserve the exact correction through acknowledgement and fresh image verification', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        const server = yield* makeAcquireServerSimulation(
          makeState(pointingSession()),
        )
        ...
        assert.ok(
          move !== undefined && AcquireWork.guards.MovePointingCorrection(move),
        )
        if (
          move === undefined ||
          !AcquireWork.guards.MovePointingCorrection(move)
        )
          return
        ...
        const duplicate = yield* server
          .completeSolve(completion('solve-1', solved(120, -40)))
          .pipe(
            Effect.as('accepted' as const),
            Effect.catchTag('AcquireServer.WorkerResultRejected', () =>
        ...
        assert.equal(
          acknowledged.session.evidence.some(
        ...
        assert.equal(
          active.verificationOfCorrectionAttemptId,
          move.correctionAttemptId,
        )

        yield* server.completeSolve(
          completion(active.attemptId, solved(10, -5), 'unused', 2_000),
        )
        ...
        assert.ok(
          verification !== undefined &&
            AcquireEvidence.guards.SolveAttempt(verification),
        )
        if (
          verification !== undefined &&
          AcquireEvidence.guards.SolveAttempt(verification)
          ...
          assert.equal(
            verification.verificationOfCorrectionAttemptId,
            move.correctionAttemptId,
          )
          assert.equal(
            verification.sourceFrameAssetId,
            `frame-${active.attemptId}`,
          )
        }
        assert.equal(
          verified.outbox.some(AcquireWork.guards.ContinueToCapture),
          true,
        )
      }),
    )
  ...
  it('ACQ-02 records every failed frame, stops exactly at the ordinary bound, and never moves', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.equal(
          paused.session.evidence.filter(AcquireEvidence.guards.SolveAttempt)
            .length,
          3,
        )
        assert.deepEqual(paused.session.solveSeries[0]?.completedAttemptIds, [
          'solve-1',
          'solve-2',
          'solve-3',
        ])
        assert.equal(
          paused.outbox.filter(AcquireWork.guards.CaptureAndSolve).length,
          2,
        )
        assert.equal(
          paused.outbox.some(AcquireWork.guards.MovePointingCorrection),
          false,
        )
  ...
  it('ACQ-03 atomically opens one changed-parameter recovery series and replays a duplicate without new work', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.equal(
          state.session.solveSeries[1]?.parameters.exposureSeconds,
          15,
        )
        assert.equal(state.receipts.length, 1)
        assert.equal(state.results.length, 1)
        assert.equal(
          state.events.filter(({ kind }) => kind === 'RecoveryStarted').length,
          1,
        )
        assert.equal(
          state.outbox.filter(AcquireWork.guards.CaptureAndSolve).length,
          3,
        )
      }),
    )
  ...
  it('ACQ-03 serializes competing recovery choices and leaves the rejected command side-effect free', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const paused = yield* exhaustOrdinarySolveBudget(server)
        const submit = (suffix: string, exposureSeconds: number) =>
          server
            .submit(
              command(
                `command-${suffix}`,
            ...
            .pipe(
              Effect.as('accepted' as const),
              Effect.catchTag('AcquireServer.CommandRejected', () =>
                Effect.succeed('rejected' as const),
              ),
            )

        const outcomes = yield* Effect.all([submit('a', 15), submit('b', 20)], {
  ...
  it('ACQ-03 supports the explicit skip path only while paused with fallback work', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.ok(
          work !== undefined &&
            AcquireWork.guards.AdvanceAfterSkippedTarget(work),
        )
        if (
          work !== undefined &&
          AcquireWork.guards.AdvanceAfterSkippedTarget(work)
  ...
  it('ACQ-04 keeps proposal revision inert, binds approval to the current proposal, and queues only the revised vector', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.equal(
          revised.session.pendingCorrectionProposal?.basis,
          'operatorRevision',
        )

        const staleOutcome = yield* server
          .submit(
            command(
              'command-stale-approval',
  ...
  it('ACQ-06 and ACQ-07 store authoritative polar evidence and require explicit acceptance of the latest in-tolerance measurement', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        const server = yield* makeAcquireServerSimulation(
          makeState(polarSession()),
        )
        yield* server.submit(
          command('command-polar-1', 'CapturePolarAlignmentMeasurement', 0, {
        ...
        assert.ok(
          firstWork !== undefined &&
            AcquireWork.guards.CapturePolarMeasurement(firstWork),
        )
        if (
          firstWork === undefined ||
          !AcquireWork.guards.CapturePolarMeasurement(firstWork)
        ...
        assert.ok(
          secondWork !== undefined &&
            AcquireWork.guards.CapturePolarMeasurement(secondWork),
        )
        if (
          secondWork === undefined ||
          !AcquireWork.guards.CapturePolarMeasurement(secondWork)
        ...
        const stale = yield* server
          ...
          .pipe(
            Effect.as('accepted' as const),
            Effect.catchTag('AcquireServer.DecisionRejected', () =>
              Effect.succeed('rejected' as const),
            ),
          )
        ...
        assert.equal(
          state.outbox.some(AcquireWork.guards.ContinueToCapture),
          true,
        )
      }),
    )
      ...
      Effect.gen(function* () {
        ...
        const malformed = yield* server
          .submit(
            {
              commandId: 'bad',
              command: { _tag: 'CapturePolarAlignmentMeasurement' },
            },
            actor,
            1_000,
          )
        ...
        const stale = yield* server
          .submit(
            command('command-stale', 'CapturePolarAlignmentMeasurement', 99, {
