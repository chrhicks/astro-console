...
const target = (
  ...
  connection: ControlTarget['connection'] = 'current',
) => ControlTarget.make({ clientId, capability: 'controlCapable', connection })

const initialState = (
  holderClientId = owner.clientId,
): ControlSimulationState => ({
  ...
  targets: [
    target(owner.clientId),
    target(friendA.clientId),
    target(friendB.clientId),
  ],
  receipts: [],
  results: [],
  events: [],
...
const makeServer = (state = initialState(), nowEpochMs = 1_000) =>
  makeControlServerSimulation({
    initialState: state,
    nowEpochMs,
    requestTtlMs: 60_000,
...
describe('control authority server proofs', () => {
  it('atomically requests and grants control while duplicate delivery replays exactly once', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.deepEqual(
          committed.events.map((event) => event.event._tag),
  ...
  it('declines a current request durably without changing the ownership epoch', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        const server = yield* makeServer()
        const requested = yield* server.execute(
          requestCommand('command-request-decline', 'request-decline'),
          friendA,
        )
        const declined = yield* server.execute(
          declineCommand(
            'command-decline',
            requested.requestId ?? 'missing-request',
          ),
          owner,
        )
        ...
        assert.deepEqual(
          ...
          ['ControlRequested', 'ControlDeclined'],
        )
      }),
    )
  })
  ...
  it('serializes simultaneous requests without using or advancing LeaseRevision', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.equal(state.persistenceVersion, 2)
        assert.deepEqual(
          state.lease.requests
            .map((request) => request.requesterClientId)
            .sort(),
  ...
  it('keeps disconnect and reconnect transient, while expiry changes ownership durably', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const disconnected = yield* server.disconnectController(
          friendA.clientId,
          61_000,
        )
        ...
        assert.equal(
          afterExpiry.persistenceVersion,
          beforeExpiry.persistenceVersion + 1,
        )
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* server.execute(
          requestCommand('command-request-before-take', 'request-before-take'),
          friendB,
        )
        yield* server.disconnectController(friendA.clientId, 61_000)

        const taken = yield* server.execute(
          takeCommand('command-take', 4),
          owner,
        )
        ...
        const outcome = yield* server
          .execute(releaseCommand('command-delayed-release', 4), friendA)
          .pipe(
            Effect.as('released' as const),
            Effect.catchTag(
              'ControlServerSimulation.CommandRejected',
              ({ failure }) => {
                if (failure._tag !== 'AuthorizationFailure')
        ...
        const replayedTake = yield* server.execute(
          takeCommand('command-take', 4),
          owner,
        )
  ...
  it('leaves all authoritative surfaces unchanged for domain and authority rejection', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const alreadyController = yield* server
          .execute(
        ...
        const missingRequest = yield* server
          .execute(
            grantCommand(
              'command-missing-grant',
              'request-missing',
              friendA.clientId,
            ),
            owner,
        ...
        assert.equal(
          (yield* server.disconnectController(friendA.clientId, 61_000))._tag,
          'Unchanged',
        )
        assert.equal(
          (yield* server.reconnectController(owner.clientId))._tag,
          'Unchanged',
        )
        assert.equal((yield* server.expireGrace(61_000))._tag, 'Unchanged')
        assert.deepEqual(yield* server.readState(), before)
  ...
  it('rejects an expired request after reload without recording the attempted grant', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const outcome = yield* grantServer
          .execute(
            grantCommand(
              'command-grant-expired',
              requested.requestId ?? 'missing-request',
              friendA.clientId,
            ),
            owner,
