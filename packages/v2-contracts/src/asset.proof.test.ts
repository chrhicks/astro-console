...
import { makeAssetServerSimulation, makeProcessingOpenAuthority } from "./asset-server-simulation.js"
...
const processingState = (sessions: ReadonlyArray<ProcessingSession> = []): ProcessingSimulationState => ({
  sessions,
...
const makeProcessingServer = (state = processingState()) => makeProcessingServerSimulation({
  initialState: state,
...
describe("asset delivery server proof", () => {
  it("routes the same local original by trusted access path", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const lanServer = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const lan = yield* lanServer.requestDownload(request("download-lan", "download-lan"), viewer, "lan", 100)
      assert.equal(lan._tag, "StreamLocal")
      const lanState = yield* lanServer.readState()
      assert.equal(lanState.outbox.length, 0)

      const remoteServer = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const remote = yield* remoteServer.requestDownload(request("download-remote", "download-remote"), viewer, "remote", 100)
  ...
  it("commits staging identity, audit event, result, receipt, and outbox before upload", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const uploads = yield* Ref.make(0)

      yield* server.requestDownload(request("download-remote", "download-remote"), viewer, "remote", 100)
      ...
      yield* server.dispatchOutbox(() => Ref.update(uploads, (count) => count + 1))
  ...
  it("accepts only correlated verified publication evidence before granting remote delivery", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const preparing = yield* server.requestDownload(request("prepare-1", "prepare-key"), viewer, "remote", 100)
      assert.equal(preparing._tag, "Preparing")
      if (preparing._tag !== "Preparing") return
      const beforeBadEvidence = yield* server.readState()
      const mismatch = yield* server.completePublication({
        assetId: "raw-m27",
        ...
        Effect.catchTag("AssetServerSimulation.WorkerRejected", () => Effect.succeed("rejected" as const)),
      )
      ...
      const granted = yield* server.requestDownload(request("grant-after-stage", "grant-after-stage"), viewer, "remote", 200)
  ...
  it("replays a duplicate without staging or auditing twice", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const accepted = yield* server.requestDownload(request("download-1", "same-download"), owner, "remote", 100)
      const replayed = yield* server.requestDownload(request("download-2", "same-download"), owner, "remote", 100)
      const state = yield* server.readState()
  ...
  it("rejects idempotency reuse from another network path without changing state", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      yield* server.requestDownload(request("download-remote", "same-path-bound-key"), owner, "remote", 100)
      const before = yield* server.readState()
      const outcome = yield* server.requestDownload(request("download-lan", "same-path-bound-key"), owner, "lan", 100).pipe(
        Effect.as("accepted" as const),
        Effect.catchTag("AssetServerSimulation.CommandRejected", () => Effect.succeed("rejected" as const)),
      )
  ...
  it("rechecks current representation validity instead of replaying stale delivery authority", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const published = LibraryAsset.make({
        ...original,
        representations: [DeliveryRepresentation.cases.Published.make({
      ...
      const accepted = yield* server.requestDownload(request("grant-1", "grant-replay"), viewer, "remote", 100)
      assert.equal(accepted._tag, "PublishedRepresentationEligible")
      const before = yield* server.readState()

      const replay = yield* server.requestDownload(request("grant-2", "grant-replay"), viewer, "remote", 201).pipe(
        Effect.as("granted" as const),
        Effect.catchTag("AssetServerSimulation.DownloadRejected", () => Effect.succeed("expired" as const)),
      )
  ...
  it("atomically starts republication under the same stable asset identity", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const expired = LibraryAsset.make({
        ...original,
        representations: [DeliveryRepresentation.cases.Expired.make({
      ...
      const replay = yield* server.republish({ ...command, commandId: "republish-2" }, owner, 101)
  ...
  it("reuses equivalent staging across distinct requests and replaces an explicit expired representation", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const first = yield* server.requestDownload(request("stage-a", "stage-key-a"), viewer, "remote", 100)
      const second = yield* server.requestDownload(request("stage-b", "stage-key-b"), viewer, "remote", 100)
      ...
      assert.equal(replaced.assets[0]?.representations[0]?.representationId, "expired-stage")
  ...
  it("makes worker completion and failure idempotent while rejecting untrusted expiry claims", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const preparing = yield* server.requestDownload(request("stage-complete", "stage-complete"), viewer, "remote", 100)
      assert.equal(preparing._tag, "Preparing")
      if (preparing._tag !== "Preparing") return
      const staleExpiry = yield* server.completePublication({
        assetId: "raw-m27",
        operationId: preparing.operationId,
        ...
        Effect.as("accepted" as const),
        Effect.catchTag("AssetServerSimulation.WorkerRejected", ({ reason }) => Effect.succeed(reason)),
      )
      ...
      const failureServer = yield* makeAssetServerSimulation(initialState, "2026-07-23T02:00:00Z")
      const failing = yield* failureServer.requestDownload(request("stage-fail", "stage-fail"), viewer, "remote", 100)
      ...
      assert.equal(failedOnce.events.at(-1)?.event._tag, "AssetPublicationFailed")
    }))
  })
  ...
  it("durably expires and reuses republication before completing under the stable asset identity", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const expired = yield* server.expireRepresentation(original.assetId, RepresentationId.make("r2-final"), 101)
      ...
      assert.equal(expiredState.events.at(-1)?.event._tag, "AssetRepresentationExpired")

      const republish = (commandId: string, revision: number, key: string) => ({
        commandId,
        command: {
          _tag: "RepublishAssetRepresentation",
          assetId: "raw-m27",
      ...
      const first = yield* server.republish(republish("republish-a", 3, "republish-a"), owner, 101)
      const second = yield* server.republish(republish("republish-b", 4, "republish-b"), owner, 101)
      assert.equal(second.operationId, first.operationId)
      const preparing = yield* server.readState()
      assert.equal(preparing.outbox.length, 1)
      assert.equal(preparing.events.filter((event) => event.event._tag === "AssetRepublicationStarted").length, 1)
      yield* server.completePublication({
        assetId: "raw-m27",
  ...
  it("projects and compares exact Library lineage through an authorized query without mutation", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const preview = LibraryAsset.make({
        ...
        lineage: { ...final.lineage, processingOutputId: ProcessingOutputId.make("output-preview") },
      })
      const state = { ...initialState, assets: [final, preview] }
      const server = yield* makeAssetServerSimulation(state, "2026-07-23T02:00:00Z")
      const before = yield* server.readState()
      const snapshots = yield* server.librarySnapshot(viewer, 100, 50)
      const comparison = yield* server.compareAssets([final.assetId, preview.assetId], viewer)
      assert.equal(snapshots[0]?.processingSessionId, "process-m27")
      assert.equal(snapshots[0]?.processingOutputId, "output-final")
      assert.deepEqual(comparison.entries[0]?.operationIds, ["operation-stretch"])
      assert.equal(comparison.entries[1]?.processingOutputId, "output-preview")
      assert.deepEqual(yield* server.readState(), before)
    }))
  })
  ...
  it("opens stable Library identity through the authoritative Processing service for Start and Resume", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      assert.equal(processAfterStart.sessions[0]?.sources[0]?.assetId, original.assetId)

      const unfinished = ProcessingSession.make({
        ...(processAfterStart.sessions[0] ?? ProcessingSession.make({
      ...
      const resumeProcessing = yield* makeProcessingServer(processingState([unfinished]))
      ...
      const savedProcessing = yield* makeProcessingServer({ ...savedProcessingState, assets: [savedLinear] })
      ...
      const openedSaved = yield* savedServer.openInProcess({
        ...
        command: { _tag: "OpenAssetInProcess", assetId: savedLinear.assetId },
      }, owner)
      assert.equal(openedSaved._tag, "Started")
      assert.equal(openedSaved.phase, "develop")
      assert.equal((yield* savedProcessing.readState()).sessions[0]?.sources[0]?.assetId, savedLinear.assetId)
