...
const session = ProcessingSession.make({
  ...
  lifecycle: "active",
  phase: "develop",
  sources: [ProcessingSourceRef.make({
  ...
  history: [AppliedProcessingOperation.make({
...
describe("hardened Asset domain", () => {
  it("creates several durable assets and leaves Process as a working session", () => {
    ...
    if (decision._tag === "Saved") {
      ...
      assert.deepEqual(decision.assets[0]?.lineage.operationIds, [OperationId.make("stretch-m27")])
    }
  })
  ...
  it("fails the whole save if any permanent output is not ready", () => {
    ...
    assert.deepEqual(decision, { _tag: "Rejected", reason: "ArtifactBytesNotReady" })
  })
  ...
  it("rejects colliding stable asset identities and records only the operations that produced each output", () => {
    ...
    assert.deepEqual(saved.assets[1]?.lineage.operationIds, [first.operationId, second.operationId])

    assert.deepEqual(completeProcessingSave(twoStepSession, "m27-source", [
      {
    ...
    assert.deepEqual(completeProcessingSave(twoStepSession, "m27-source", [{
      ...
      permanentBytesReady: true,
    }], [AssetId.make("existing-asset")]), { _tag: "Rejected", reason: "AssetIdentityConflict" })
  })
  ...
  it("streams locally but stages a remote-only delivery without changing asset identity", () => {
    ...
    assert.deepEqual(local, { _tag: "StreamLocal", assetId: AssetId.make("raw-m27") })
  ...
  it("publishes only the correlated staging operation and honors expiry", () => {
    ...
    assert.equal(
      completeAssetPublication(started.asset, OperationId.make("superseded"), 500)._tag,
      "Rejected",
    )
    const completed = completeAssetPublication(started.asset, OperationId.make("stage-1"), 500)
  ...
  it("compares related saved assets without mutating them", () => {
    ...
    if (saved._tag === "Saved") {
      ...
      assert.equal(saved.assets.every((asset) => asset.revision === AssetRevision.make(0)), true)
    }
  })
  ...
  it("rejects duplicate representation identities and representation format drift", () => {
    ...
    assert.throws(() => LibraryAsset.make({
      ...original,
  ...
  it("opens only source roles that justify a Process phase", () => {
    ...
    assert.deepEqual(decideOpenAssetInProcess(derived), { _tag: "Rejected", reason: "SourceRoleUnsupported" })
  })
  ...
  it("expires and republishes delivery state without replacing canonical asset identity", () => {
    ...
    const expired = expireAssetRepresentation(published, RepresentationId.make("r2-final"), 101)
  ...
  it("derives honest delivery availability from representation evidence", () => {
    ...
    assert.equal(projectAssetSnapshot(republishing.asset, 100, 50).availability, "republishing")

    const published = completeAssetPublication(republishing.asset, OperationId.make("republish-final"), 200)
    assert.equal(published._tag, "Published")
    if (published._tag !== "Published") return
    assert.equal(projectAssetSnapshot(published.asset, 160, 50).availability, "expiring")
    assert.equal(projectAssetSnapshot(published.asset, 201, 50).availability, "expired")
  })
})
