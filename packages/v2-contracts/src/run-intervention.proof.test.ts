...
const initialState = () => ({
  ...
  run: ActiveRunState.make({
    ...
    futureSequenceIds: [],
    acceptedMutations: [],
    activeExposure: { startedAtEpochMs: 0, exposureSeconds: 180, provisionalEvidenceId: "frame-15" },
  }),
...
describe("run intervention server proofs", () => {
  it("pauses, resumes the prior phase, and stops through named durable intents", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeRunInterventionServerSimulation(initialState(), "2026-07-23T01:00:00Z")

      const paused = yield* server.pause(command("PauseRun", "pause-1", 12, "pause-key"), controller)
      ...
      const resumed = yield* server.resume(command("ResumeRun", "resume-1", 13, "resume-key"), controller)
      ...
      const stopped = yield* server.stop(command("StopRun", "stop-1", 14, "stop-key"), controller)
      ...
      const replayedStop = yield* server.stop(command("StopRun", "stop-retry", 14, "stop-key"), controller)
      ...
      assert.deepEqual(state.events.map((event) => event.event._tag), ["RunPaused", "RunResumed", "RunStopped"])
      assert.deepEqual(state.outbox.map((work) => work._tag), ["PauseRun", "ResumeRun", "StopRun"])
  ...
  it("rejects repeated terminal and stale interventions without changing any authoritative surface", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeRunInterventionServerSimulation(initialState(), "2026-07-23T01:00:00Z")
      yield* server.stop(command("StopRun", "stop-first", 12, "stop-first-key"), controller)
      const terminal = yield* server.readState()

      const repeated = yield* server.stop(command("StopRun", "stop-again", 13, "stop-again-key"), controller).pipe(
        Effect.as("stopped" as const),
        Effect.catchTag("RunSimulation.InterventionRejected", ({ reason }) => Effect.succeed(reason)),
      )
      assert.equal(repeated, "AlreadyTerminal")
      assert.deepEqual(yield* server.readState(), terminal)

      const stale = yield* server.pause(command("PauseRun", "pause-stale", 12, "pause-stale-key"), controller).pipe(
  ...
  it("serializes concurrent pause and stop so the losing revision creates no event, receipt, or work", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const outcomes = yield* Effect.all([
        server.pause(command("PauseRun", "pause-race", 12, "pause-race-key"), controller).pipe(
          Effect.as("accepted" as const),
          Effect.catchTag("RunSimulation.CommandRejected", () => Effect.succeed("rejected" as const)),
        ),
        server.stop(command("StopRun", "stop-race", 12, "stop-race-key"), controller).pipe(
          Effect.as("accepted" as const),
          Effect.catchTag("RunSimulation.CommandRejected", () => Effect.succeed("rejected" as const)),
        ),
