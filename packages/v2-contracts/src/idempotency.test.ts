...
describe('idempotency receipt classification', () => {
  it('classifies an unseen key as fresh', () => {
    assert.deepEqual(classifyIdempotency(request, undefined), {
      _tag: 'Fresh',
    })
  })
  ...
  it('rejects key reuse by another actor or with different normalized input', () => {
    ...
    assert.deepEqual(classifyIdempotency(otherActor, receipt), {
      _tag: 'Conflict',
    })
    assert.deepEqual(classifyIdempotency(otherInput, receipt), {
      _tag: 'Conflict',
    })
  })
})
