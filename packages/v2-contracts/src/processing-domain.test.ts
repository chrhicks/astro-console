...
const startedDevelop = () => {
  const decision = decideStartProcessingSession(ProcessingSessionId.make("process-1"), [source("linear-1", "linearMaster")])
...
const readyPreview = (session: ProcessingSession) => ProcessingPreviewSpec.make({
  previewId: PreviewId.make("preview-1"),
  clientPreviewSequence: 1,
  operation: "stretch",
  toolId: "siril",
  parameters: [{ key: "amount", value: { _tag: "NumberValue", value: 0.6 } }],
  input: currentProcessingImage(session) ?? assert.fail("missing current image"),
...
describe("hardened Processing domain", () => {
  it("starts raw sources in Build and emits only linear-master work", () => {
    const decision = decideStartProcessingSession(ProcessingSessionId.make("process-raw"), [
  ...
  it("starts one linear master directly in Develop without Build work", () => {
    const decision = decideStartProcessingSession(ProcessingSessionId.make("process-linear"), [source("linear-1", "linearMaster")])
    StartProcessingDecision.$match(decision, {
      Started: ({ session, work }) => {
  ...
  it("moves a raw session into Develop only when its linear master completes", () => {
    const started = decideStartProcessingSession(ProcessingSessionId.make("process-raw"), [source("raw-1", "original")])
  ...
  it("rejects mixed raw and linear-master source roles", () => {
    const decision = decideStartProcessingSession(ProcessingSessionId.make("process-mixed"), [
      source("raw-1", "original"),
  ...
  it("starts full-resolution Apply without promoting preview output into history", () => {
    const session = ProcessingSession.make({ ...startedDevelop(), preview: readyPreview(startedDevelop()) })
    const decision = startProcessingApply(session, AttemptId.make("attempt-1"), OperationId.make("operation-1"), PreviewId.make("preview-1"))
  ...
  it("adds history only after the correlated full-resolution attempt completes", () => {
    const session = ProcessingSession.make({ ...startedDevelop(), preview: readyPreview(startedDevelop()) })
    const started = startProcessingApply(session, AttemptId.make("attempt-1"), OperationId.make("operation-1"), PreviewId.make("preview-1"))
    ...
    if (completed._tag === "ApplyCompleted") {
      ...
      assert.equal(completed.session.history[0]?.output.outputId, ProcessingOutputId.make("full-output-1"))
    }
  })
  ...
  it("ignores a superseded preview completion and preserves applied history", () => {
    ...
    const second = queueProcessingPreview(first.session, {
      ...
      operation: "stretch",
      toolId: "siril",
      parameters: [{ key: "amount", value: { _tag: "NumberValue", value: 0.63 } }],
      baseHistoryPosition: 0,
    })
    ...
    assert.equal(second.session.preview?.previewId, PreviewId.make("preview-current"))
  ...
  it("rejects regressive preview sequences and duplicate worker completion", () => {
    ...
    const completed = completeProcessingPreview(first.session, PreviewId.make("preview-current"), ProcessingOutputId.make("preview-output"))
    ...
    assert.equal(ProcessingTransition.$is("Rejected")(
      completeProcessingPreview(completed.session, PreviewId.make("preview-current"), ProcessingOutputId.make("preview-output")),
    ), true)
  })
  ...
  it("queues only the failed stage from its protected checkpoint", () => {
    const session = ProcessingSession.make({
      ...
      failedAttempt: {
        ...
        toolId: "siril",
        parameters: [],
        input: ProcessingImageRef.cases.SourceAsset.make({ assetId: AssetId.make("linear-1"), checksum: "sha256:linear-1" }),
        baseHistoryPosition: 0,
  ...
  it("tombstones before cleanup while protecting source and saved assets", () => {
    const session = ProcessingSession.make({ ...startedDevelop(), savedAssetIds: [AssetId.make("saved-1")] })
    const decision = discardHardenedProcessingSession(session, "confirm-1", "confirm-1")
    assert.equal(decision._tag, "Discarded")
    if (decision._tag === "Discarded") {
      assert.equal(decision.session.lifecycle, "discarded")
      assert.equal(decision.session.sources[0]?.assetId, AssetId.make("linear-1"))
      assert.equal(decision.work._tag, "CleanupDiscardedSession")
      if (decision.work._tag === "CleanupDiscardedSession") {
        assert.deepEqual(decision.work.protectedAssetIds, [AssetId.make("saved-1")])
      }
    }
  })
  ...
  it("moves history only while no incompatible attempt is active", () => {
    ...
    if (started._tag === "ApplyStarted") {
      assert.equal(moveHardenedProcessingHistory(started.session, "undo")._tag, "Rejected")
    }
  })
  ...
  it("assistant correlation stays on Preview and never creates applied history", () => {
    ...
    if (decision._tag === "PreviewQueued") {
      assert.equal(decision.session.preview?.suggestionFindingId, FindingId.make("finding-1"))
