...
export const navigateWorkspace = (
  state: typeof ShellState.Type,
  workspace: typeof Workspace.Type,
): typeof ShellState.Type => ShellState.make({ ...state, workspace })

export const routeAttention = (
  state: typeof ShellState.Type,
  workspace: typeof Workspace.Type,
): typeof ShellState.Type =>
...
export const ClientConnection = Schema.TaggedUnion({
  ...
  Stale: {
    lastConfirmedAt: Schema.NonEmptyString,
    disconnectedAt: Schema.NonEmptyString,
  },
  Reconnecting: {
    lastConfirmedAt: Schema.NonEmptyString,
...
export interface ClientProjectionState extends Schema.Schema.Type<
  typeof ClientProjectionState
> {}
...
export const installAuthoritativeSnapshot = (
  ...
  ClientProjectionState.make({
    connection: ClientConnection.cases.Current.make({
...
export type IncrementalEventDecision = Data.TaggedEnum<{
  ...
  SnapshotRequired: {
    readonly state: ClientProjectionState
    readonly reason:
      'ConnectionNotCurrent' | 'EventCursorGap' | 'SnapshotVersionRegressed'
  }
}>

export const IncrementalEventDecision =
  Data.taggedEnum<IncrementalEventDecision>()
...
export const receiveIncrementalEvent = (
  ...
  return EventCursorDecision.$match(decision, {
    Apply: () =>
      event.snapshotVersion < state.snapshot.snapshotVersion
        ...
        : IncrementalEventDecision.Applied({
            state: applyIncrementalEvent(state, event),
          }),
...
export const projectSnapshotForClient = (
  ...
  AppSnapshot.make({
    ...canonical,
    membership,
    control: ControlSnapshot.make({
      ...canonical.control,
      actions: projectActions(canonical.control.actions, membership.capability),
    }),
    ...(canonical.plan === undefined
      ? {}
      : {
          plan: PlanSnapshot.make({
            ...canonical.plan,
      ...
      : {
          run: RunSnapshot.make({
            ...
            ...(canonical.run.acquire === undefined
              ...
              : {
                  acquire: {
                    ...canonical.run.acquire,
                    actions: projectActions(
                      canonical.run.acquire.actions,
                      membership.capability,
                    ),
                  },
                }),
...
export const decideClientCommand = (
  ...
  return allActions(state.snapshot).some(
    (availability) =>
      availability.action === action &&
      ActionAvailability.guards.Available(availability),
  )
...
function applyIncrementalEvent(
  ...
  const snapshot = IncrementalProjectionEvent.match(event, {
    ControlProjected: ({ control }) =>
      AppSnapshot.make({ ...state.snapshot, ...common, control }),
    PlanProjected: ({ plan }) =>
      plan === null
    ...
    HealthProjected: ({ health }) =>
      AppSnapshot.make({ ...state.snapshot, ...common, health }),
  })
  return ClientProjectionState.make({
    connection: ClientConnection.cases.Current.make({
      lastConfirmedAt: event.generatedAt,
    }),
    snapshot,
    changesWhileAway: [],
  })
...
function projectActions(
  ...
  return actions.map((availability) =>
    ActionAvailability.match(availability, {
      Available: ({ action }) =>
        commandPolicies[action].requiresDesktop
          ? ActionAvailability.cases.Unavailable.make({
              action,
