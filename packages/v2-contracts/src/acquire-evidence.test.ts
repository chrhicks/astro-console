...
describe('Acquire evidence model', () => {
  ...
  it('appends no-solution evidence and schedules a correlated bounded retry without movement', () => {
    ...
    const decision = recordSolveCompletion(
      pointingSession(),
      completion('solve-1', noSolution),
    )
    ...
    assert.equal(
      AcquireActiveWork.guards.SolveRequested(decision.session.activeWork),
      true,
    )
    assert.equal(
      AcquireActiveWork.guards.CorrectionRequested(decision.session.activeWork),
      false,
    )
    ...
    assert.ok(
      evidence !== undefined && AcquireEvidence.guards.SolveAttempt(evidence),
    )
    if (
      evidence !== undefined &&
  ...
  it('starts only one separately bounded recovery series with materially changed parameters', () => {
    ...
    const unchanged = openRecoverySeries(paused, {
      seriesId: decode(
        Schema.NonEmptyString.pipe(Schema.brand('RecoverySeriesId')),
        'recovery-1',
      ),
  ...
  it('carries an exact automatic correction through acknowledgement into image verification', () => {
    const correction = recordSolveCompletion(
      pointingSession(),
      completion('solve-1', solved(120, -40)),
    )
    assert.equal(
      SolveCompletionDecision.$is('AutomaticCorrectionStarted')(correction),
      true,
    )
    if (!SolveCompletionDecision.$is('AutomaticCorrectionStarted')(correction))
      return
    ...
    assert.equal(
      CorrectionAcknowledgementDecision.$is('VerificationScheduled')(
        acknowledged,
      ),
      true,
    )
    ...
    if (
      AcquireActiveWork.guards.SolveRequested(acknowledged.session.activeWork)
    ) {
      assert.equal(
        acknowledged.session.activeWork.verificationOfCorrectionAttemptId,
        'solve-1-correction',
      )
    } else {
  ...
  it('keeps an outside-automatic-bound proposal inert until current, unexpired approval', () => {
    const proposed = recordSolveCompletion(
      pointingSession(),
      completion('solve-1', solved(900, 100)),
    )
    assert.equal(
      SolveCompletionDecision.$is('CorrectionApprovalRequired')(proposed),
      true,
    )
    if (!SolveCompletionDecision.$is('CorrectionApprovalRequired')(proposed))
      return
    ...
    if (CorrectionCommandDecision.$is('Rejected')(expired))
      assert.equal(expired.reason, 'ProposalExpired')

    const revised = reviseCorrectionProposal(proposed.session, {
      currentProposalId: 'solve-1-proposal',
      nextProposalId: 'proposal-revised',
      correction: {
        rightAscensionArcsec: 700,
        declinationArcsec: 50,
        convention: 'mountRaDec',
      },
      nowEpochMs: 2_000,
    ...
    assert.equal(
      revised.session.pendingCorrectionProposal?.proposalId,
      'proposal-revised',
    )
  })

  it('stores the latest polar evidence, derives tolerance, and rejects superseded acceptance', () => {
    const firstRequest = requestPolarMeasurement(
      polarSession(),
    ...
    const first = recordPolarMeasurementEvidence(firstRequest.session, {
      ...
      sourceFrameAssetId: decode(
        Schema.NonEmptyString.pipe(Schema.brand('AssetId')),
        'polar-frame-1',
      ),
    ...
    const secondRequest = requestPolarMeasurement(
      first.session,
    ...
    const stale = acceptLatestPolarMeasurement(
      second.session,
      AttemptId.make('polar-1'),
    )
    assert.equal(PolarDecision.$is('Rejected')(stale), true)
    if (PolarDecision.$is('Rejected')(stale))
      assert.equal(stale.reason, 'MeasurementSuperseded')

    const accepted = acceptLatestPolarMeasurement(
      second.session,
      AttemptId.make('polar-2'),
    )
