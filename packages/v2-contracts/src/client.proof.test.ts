...
const snapshot = (
  ...
  decode(AppSnapshot, {
    ...
    control: {
      ...
      revision: 4,
      state: 'held',
      holderClientId: 'desktop-1',
      pendingRequestCount: 0,
      holderPersonId: 'owner-1',
    ...
    plan: {
      ...
      sequenceCount: 2,
      validation: 'ready',
      sequences: [{ sequenceId: 'm27', targetName: 'M27', state: 'active' }],
      limitations: [],
      startConditions: [],
...
const runProjected = (
  ...
  IncrementalProjectionEvent.cases.RunProjected.make({
    ...
    run: {
      ...
      revision: RunRevision.make(snapshotVersion),
      sourcePlanId: decode(
        Schema.NonEmptyString.pipe(Schema.brand('PlanId')),
        'plan-1',
      ),
      phase,
      completedSequenceCount: 0,
      acceptedMutations: [],
      warnings: [],
      lastConfirmedAt: `2026-07-22T20:00:${eventCursor}Z`,
      actions: [{ _tag: 'Available', action: 'PauseRun' }],
    },
  })
...
describe('client and phone server proofs', () => {
  it('CLIENT-01 preserves last-confirmed truth and preview age while refusing to send or buffer disconnected commands', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        assert.equal(
          ClientCommandDecision.$is('SendNow')(
            decideClientCommand(current, 'PauseRun'),
          ),
          true,
        )
        const disconnected = markClientDisconnected(
          current,
          '2026-07-22T20:00:05Z',
        )

        assert.equal(
          ClientConnection.guards.Stale(disconnected.connection),
          true,
        )
        if (ClientConnection.guards.Stale(disconnected.connection)) {
          assert.equal(
            disconnected.connection.lastConfirmedAt,
        ...
        const acceptedServerWork = {
          runId: 'run-1',
          state: 'running',
          completedFrames: 8,
        }
        assert.deepEqual(acceptedServerWork, {
          runId: 'run-1',
          state: 'running',
          completedFrames: 8,
        })
      ...
      health: [
        {
          subsystem: 'service',
          state: 'healthy',
          observedAt: '2026-07-22T20:05:00Z',
        },
      ],
    })
      ...
      health: [
        {
          subsystem: 'service',
          state: 'healthy',
          observedAt: '2026-07-22T20:05:01Z',
        },
        {
          subsystem: 'tunnel',
          state: 'unavailable',
          observedAt: '2026-07-22T20:05:01Z',
          reason: 'remote ingress unavailable',
        },
      ],
    })
  ...
  it('CLIENT-03 ignores duplicates, applies only the next event, and requires a snapshot on gaps or regression', () => {
    ...
    const duplicate = receiveIncrementalEvent(
      current,
      runProjected(40, 9, 'failed'),
    )
    assert.equal(IncrementalEventDecision.$is('Ignored')(duplicate), true)
    if (IncrementalEventDecision.$is('Ignored')(duplicate))
      assert.deepEqual(duplicate.state, current)

    const applied = receiveIncrementalEvent(
      current,
      runProjected(41, 11, 'verify'),
    )
    assert.equal(IncrementalEventDecision.$is('Applied')(applied), true)
    if (!IncrementalEventDecision.$is('Applied')(applied)) return
    assert.equal(applied.state.snapshot.run?.phase, 'verify')

    const gap = receiveIncrementalEvent(
      applied.state,
      runProjected(43, 12, 'failed'),
    )
    ...
    assert.equal(
      ClientConnection.guards.Reconnecting(gap.state.connection),
      true,
    )

    const whileReconnecting = receiveIncrementalEvent(
      gap.state,
      runProjected(42, 12, 'failed'),
    )
    assert.equal(
      IncrementalEventDecision.$is('SnapshotRequired')(whileReconnecting),
      true,
    )
    ...
    const regressed = receiveIncrementalEvent(
      current,
      runProjected(41, 9, 'failed'),
    )
    assert.equal(
      IncrementalEventDecision.$is('SnapshotRequired')(regressed),
      true,
    )
  ...
  it('PHONE-01 shares canonical truth while server projection and authority disable each desktop-only action', () => {
    ...
    const phone = projectSnapshotForClient(canonical, {
      ...
      clientId: decode(
        Schema.NonEmptyString.pipe(Schema.brand('ClientId')),
        'phone-1',
      ),
      capability: 'readOnly',
    })
    ...
    assert.equal(
      ClientCommandDecision.$is('DoNotSend')(
        decideClientCommand(phoneClient, 'PauseRun'),
      ),
      true,
    )
    assert.equal(
      ClientCommandDecision.$is('SendNow')(
        decideClientCommand(phoneClient, 'RequestAssetDownload'),
      ),
      true,
    )
    ...
    const gate = evaluateCommandGate({
      ...
      currentRevisions: {
        run: RunRevision.make(12),
        lease: LeaseRevision.make(4),
      },
    ...
    if (CommandGateDecision.$is('Rejected')(gate)) {
      ...
      if (gate.failure._tag === 'AuthorizationFailure')
        assert.equal(gate.failure.reason, 'ClientReadOnly')
    }
  })
})
