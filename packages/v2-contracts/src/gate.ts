...
export interface CommandGateInput extends Schema.Schema.Type<typeof CommandGateInput> {}
...
export const commandPolicies = {
  ...
  ApproveDisruptiveRunMutation: { authority: "controller", requiresDesktop: true },
  ...
  RetryPlateSolveWithParameters: { authority: "controller", requiresDesktop: true },
  ...
  CapturePolarAlignmentMeasurement: { authority: "controller", requiresDesktop: true },
  AcceptPolarAlignmentEvidence: { authority: "controller", requiresDesktop: true },
...
export const evaluateCommandGate = (input: CommandGateInput): CommandGateDecision =>
  ActorContext.match(input.actor, {
    Anonymous: () => reject(input, CommandFailure.cases.AuthenticationFailure.make({
      ...failureFields(input, "Authentication is required", false, false),
      reason: "Unauthenticated",
    })),
    AuthenticatedWithoutMembership: () => reject(input, CommandFailure.cases.AuthenticationFailure.make({
      ...failureFields(input, "Local observatory membership is required", false, false),
...
function evaluateMemberCommandGate(input: CommandGateInput, actor: MemberActor): CommandGateDecision {
  ...
  if (commandRequiresIdempotency(command)) {
    const replay = IdempotencyState.match(input.idempotency, {
      ...
      PendingMatch: ({ operationId }): CommandGateDecision => operationId === undefined
        ? CommandGateDecision.ReplayPending({})
        : CommandGateDecision.ReplayPending({ operationId }),
      RecordedMatch: (): CommandGateDecision => CommandGateDecision.ReplayRecorded(),
      Conflict: (): CommandGateDecision => reject(input, CommandFailure.cases.IdempotencyConflict.make({
        ...failureFields(input, "The idempotency key was already used for different input", false, false),
      })),
    })
  ...
  if (!input.connected) {
    return reject(input, CommandFailure.cases.FreshnessConflict.make({
      ...failureFields(input, "Reconnect and refresh current observatory state", true, true),
      reason: "ReconnectRequired",
    }))
  }

  if (policy.requiresDesktop && actor.capability !== "controlCapable") {
    return reject(input, CommandFailure.cases.AuthorizationFailure.make({
  ...
  if (policy.authority === "controller") {
    if (input.leaseHolderClientId === undefined) {
      return reject(input, CommandFailure.cases.AuthorizationFailure.make({
        ...failureFields(input, "The observing control lease is required", true, true),
        reason: "ControlLeaseRequired",
      }))
    }
    if (input.leaseHolderClientId !== actor.clientId) {
      return reject(input, CommandFailure.cases.AuthorizationFailure.make({
        ...failureFields(input, "Control moved to another client before this command arrived", false, true),
        reason: "ControlLeaseLost",
      }))
    }
  }

  if ("expectedPlanRevision" in command && command.expectedPlanRevision !== input.currentRevisions.plan) {
    return freshnessConflict(input, "PlanRevisionConflict", "The observing plan changed")
  }
  if ("expectedLeaseRevision" in command && command.expectedLeaseRevision !== input.currentRevisions.lease) {
    return reject(input, CommandFailure.cases.AuthorizationFailure.make({
      ...failureFields(input, "The control lease changed", true, true),
      reason: "ControlLeaseConflict",
    }))
  }
  if ("expectedRunRevision" in command && command.expectedRunRevision !== input.currentRevisions.run) {
    return freshnessConflict(input, "RunRevisionConflict", "The active run changed")
  }
  if ("expectedAcquireRevision" in command && command.expectedAcquireRevision !== input.currentRevisions.acquire) {
    return freshnessConflict(input, "AcquireRevisionConflict", "Acquire evidence or recovery state changed")
  }
  if ("expectedProcessingRevision" in command && command.expectedProcessingRevision !== input.currentRevisions.processing) {
    return freshnessConflict(input, "ProcessingSessionRevisionConflict", "The processing session changed")
  }
  if ("expectedAssetRevision" in command && command.expectedAssetRevision !== input.currentRevisions.asset) {
    return freshnessConflict(input, "AssetRevisionConflict", "The asset representation state changed")
  }
