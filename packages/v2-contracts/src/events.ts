...
export const DomainEvent = Schema.TaggedUnion({
  ...
  AcquireRecoveryStarted: {
    ...
    exposureSeconds: Schema.Finite,
  },
  AcquireTargetSkipped: { sequenceId: Schema.NonEmptyString },
  SolveAttemptRecorded: {
    attemptId: AttemptId,
    outcome: Schema.Literals(['solved', 'noSolution']),
    offsetArcsec: Schema.optionalKey(NonNegativeNumber),
  },
  ...
  AssetDownloadRequested: {
    assetId: AssetId,
    route: Schema.Literals(['lanStream', 'remoteGrantEligible', 'remoteStage']),
  },
  AssetPublicationStarted: {
    assetId: AssetId,
...
export const DomainEventEnvelope = Schema.Struct({
  ...
  Schema.makeFilter((envelope) => {
    ...
    if (envelope.aggregateKind !== expected.kind) {
      return {
        path: ['aggregateKind'],
        issue: `event belongs to ${expected.kind}`,
      }
    }
    if (expected.id !== undefined && envelope.aggregateId !== expected.id) {
      return {
        path: ['aggregateId'],
        issue: 'event identity must match its envelope aggregate',
      }
    }
  }),
)

export interface DomainEventEnvelope extends Schema.Schema.Type<
...
function aggregate(
  kind: typeof AggregateKind.Type,
  id?: string,
): ExpectedAggregate {
...
export interface ProjectionNoticeEnvelope extends Schema.Schema.Type<
  typeof ProjectionNoticeEnvelope
> {}
