...
const command = (
  ...
  acceptedPlanLimitationIds: ReadonlyArray<string> = [],
) => ({
  commandId,
  command: {
...
describe('run start server proof', () => {
  it('atomically commits the run, event, result, receipt, and outbox before adapter execution', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const response = yield* server.startRun(
          command('command-start-1', 'start-run-1'),
          actor,
        )
        ...
        assert.equal(
          committed.activeRun?.definition.sourcePlanRevision,
          PlanRevision.make(7),
        )
        assert.equal(
          committed.activeRun?.definition.executionContext.cameraDeviceId,
          'camera-sony',
        )
        assert.equal(
          committed.activeRun?.definition.executionContext.mountDeviceId,
          'mount-asi',
        )
        assert.equal(
          committed.activeRun?.definition.executionContext.unsafeBehavior,
          'pauseAndPark',
        )
        assert.deepEqual(
          committed.activeRun?.definition.sequences.map(
            (sequence) => sequence.sequenceId,
          ),
          ['m27-wide', 'm31-wide'],
        )
        assert.equal(
          committed.activeRun?.definition.sequences[0].exposureSeconds,
          180,
        )
        assert.equal(
          committed.activeRun?.definition.sequences[0].frameCount,
          24,
        )
        assert.equal(
          committed.activeRun?.definition.sequences[0].captureFailure,
          'retry',
        )
        ...
        assert.equal(
          response.projection.snapshotVersion,
          committed.snapshotVersion,
        )
        assert.equal(response.projection.eventCursor, committed.eventCursor)
        assert.equal(yield* Ref.get(adapterExecutions), 0)

        yield* server.dispatchOutbox((work) =>
          RunWork.match(work, {
            BeginRun: ({ runId }) =>
              Ref.update(adapterExecutions, (count) => {
                assert.equal(runId, response.runId)
                return count + 1
              }),
            RefreshFutureSchedule: () =>
              Effect.die(new Error('unexpected future schedule work')),
            StopActiveExposure: () =>
              Effect.die(new Error('unexpected exposure stop work')),
  ...
  it('replays the recorded result without creating another run or work item', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const accepted = yield* server.startRun(
          command('command-start-original', 'start-run-replay'),
          actor,
        )
        const replayed = yield* server.startRun(
          command('command-start-retry', 'start-run-replay'),
          actor,
        )
  ...
  it('serializes conflicting concurrent starts so exactly one commits', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        const server = yield* makeServer()
        const start = (commandId: string, idempotencyKey: string) =>
          server.startRun(command(commandId, idempotencyKey), actor).pipe(
            Effect.as('started' as const),
            Effect.catchTag('ServerSimulation.RunStartRejected', ({ reason }) =>
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const outcome = yield* server.startRun(staleLeaseRequest, actor).pipe(
          Effect.as('started' as const),
          Effect.catchTag('ServerSimulation.CommandRejected', () =>
  ...
  it('requires explicit acceptance of the exact validated plan limitations and freezes them into the run', async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const inventedAcceptance = yield* inventedAcceptanceServer
          .startRun(
            command('command-invented-limitation', 'invented-limitation', [
              'not-in-plan',
            ]),
            actor,
          )
        ...
        assert.deepEqual(
          yield* inventedAcceptanceServer.readState(),
          initialState,
        )
        ...
        const limitedState = {
          ...
          plan: ValidatedObservingPlan.make({
            ...
            limitations: [
              {
                limitationId: 'short-window',
                summary: 'Only 42 minutes remain above the local horizon',
              },
            ],
          }),
        }
        ...
        const missingAcceptance = yield* rejectingServer
          .startRun(
            command('command-limited-missing', 'limited-missing'),
            actor,
          )
          .pipe(
            Effect.as('started' as const),
        ...
        yield* acceptingServer.startRun(
          command('command-limited-accepted', 'limited-accepted', [
            'short-window',
          ]),
          actor,
        )
        ...
        assert.deepEqual(
          accepted.activeRun?.definition.acceptedLimitations,
          limitedState.plan.limitations,
        )
      }),
    )
  })
