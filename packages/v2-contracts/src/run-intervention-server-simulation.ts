...
import { RunCommandRejected, RunMutationProjection } from "./run-mutation-server-simulation.js"
...
export interface RunInterventionResponse extends Schema.Schema.Type<typeof RunInterventionResponse> {}
...
export class RunInterventionRejected extends Schema.TaggedErrorClass<RunInterventionRejected>()(
  ...
  {
    reason: Schema.Literals(["AlreadyPaused", "NotPaused", "AlreadyTerminal", "ResumePhaseUnavailable", "ResultUnavailable"]),
...
export interface RunInterventionServerSimulation {
  readonly pause: (rawRequest: unknown, actor: MemberActor) => Effect.Effect<RunInterventionResponse, Schema.SchemaError | RunCommandRejected | RunInterventionRejected>
  readonly resume: (rawRequest: unknown, actor: MemberActor) => Effect.Effect<RunInterventionResponse, Schema.SchemaError | RunCommandRejected | RunInterventionRejected>
  readonly stop: (rawRequest: unknown, actor: MemberActor) => Effect.Effect<RunInterventionResponse, Schema.SchemaError | RunCommandRejected | RunInterventionRejected>
  readonly readState: () => Effect.Effect<RunInterventionSimulationState>
}

export const makeRunInterventionServerSimulation = Effect.fn("RunInterventionSimulation.make")(
  function* (initialState: RunInterventionSimulationState, occurredAt: string) {
    ...
    const pause: RunInterventionServerSimulation["pause"] = Effect.fn("RunInterventionSimulation.pause")(function* (rawRequest, actor) {
      const envelope = yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(Command.cases.PauseRun)(envelope.command)
      return yield* simulation.transact((current) => acceptIntervention(current, envelope, command, actor, Intervention.Pause(), occurredAt))
    })
    const resume: RunInterventionServerSimulation["resume"] = Effect.fn("RunInterventionSimulation.resume")(function* (rawRequest, actor) {
      const envelope = yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(Command.cases.ResumeRun)(envelope.command)
      return yield* simulation.transact((current) => acceptIntervention(current, envelope, command, actor, Intervention.Resume(), occurredAt))
    })
    const stop: RunInterventionServerSimulation["stop"] = Effect.fn("RunInterventionSimulation.stop")(function* (rawRequest, actor) {
      const envelope = yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(Command.cases.StopRun)(envelope.command)
      return yield* simulation.transact((current) => acceptIntervention(current, envelope, command, actor, Intervention.Stop(), occurredAt))
    })

    return { pause, resume, stop, readState: simulation.readState } satisfies RunInterventionServerSimulation
  },
)
...
function acceptIntervention(
  ...
  occurredAt: string,
): Effect.Effect<AtomicCommit<RunInterventionSimulationState, RunInterventionResponse>, RunCommandRejected | RunInterventionRejected> {
  const request = IdempotencyRequest.make({
    ...
    normalizedInputHash: NormalizedInputHash.make(JSON.stringify([
      `${command._tag}.v1`, command.runId, command.expectedRunRevision, command.expectedLeaseRevision,
    ])),
  })
  ...
  const gate = evaluateCommandGate({
    ...
    currentRevisions: { run: current.run.revision, lease: current.leaseRevision },
  ...
  return CommandGateDecision.$match(gate, {
    ...
    Accepted: () => commitIntervention(current, envelope, request, intervention, occurredAt),
  })
}
...
function commitIntervention(
  ...
  intervention: Intervention,
  occurredAt: string,
): Effect.Effect<AtomicCommit<RunInterventionSimulationState, RunInterventionResponse>, RunInterventionRejected> {
  ...
  return RunInterventionDecision.$match(decision, {
    ...
    Applied: ({ state, work }) => {
      ...
      const event = interventionEvent(intervention, current.run.phase, state.phase, state.runId)
...
function interventionEvent(
  ...
  return Intervention.$match(intervention, {
    ...
    Resume: () => DomainEvent.cases.RunResumed.make({ runId, resumedPhase: nextPhase }),
    Stop: () => DomainEvent.cases.RunStopped.make({ runId, previousPhase }),
  })
}

function replay(current: RunInterventionSimulationState, receipt: typeof IdempotencyReceipt.Type | undefined) {
  if (receipt === undefined || !IdempotencyReceipt.guards.Recorded(receipt)) return reject(current, "ResultUnavailable")
  ...
  return Effect.succeed({
    state: current,
    result: RunInterventionResponse.make({ ...result.response, replayed: true, projection: project(current) }),
  })
}

function reject(current: RunInterventionSimulationState, reason: RunInterventionRejected["reason"]) {
  return Effect.fail(new RunInterventionRejected({ reason, currentRunRevision: current.run.revision }))
}
