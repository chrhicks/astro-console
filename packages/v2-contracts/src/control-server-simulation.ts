...
const ControlCommand = Schema.TaggedUnion({
  RequestControl: {
    expectedLeaseRevision:
      Command.cases.RequestControl.fields.expectedLeaseRevision,
    idempotencyKey: Command.cases.RequestControl.fields.idempotencyKey,
  },
  GrantControl: {
    expectedLeaseRevision:
      Command.cases.GrantControl.fields.expectedLeaseRevision,
...
export interface ControlProjection extends Schema.Schema.Type<
  typeof ControlProjection
> {}
...
export interface ControlResponse extends Schema.Schema.Type<
...
export const makeControlServerSimulation = Effect.fn(
  'ControlServerSimulation.make',
  ...
  const execute: ControlServerSimulation['execute'] = Effect.fn(
    'ControlServerSimulation.execute',
  )(function* (rawRequest: unknown, actor: MemberActor) {
    const envelope =
      yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
    const command = yield* Schema.decodeUnknownEffect(ControlCommand)(
      envelope.command,
    )
    ...
    return yield* simulation.transact((current) =>
      Effect.gen(function* () {
        ...
        const gate = evaluateCommandGate({
          ...
          ...(current.lease.holderClientId === undefined
            ? {}
            : { leaseHolderClientId: current.lease.holderClientId }),
  ...
  const transition = (
    ...
    durableEvent?: (
      previous: ControlLeaseState,
      next: ControlLeaseState,
          ...
          const next: ControlSimulationState = {
            ...
            events:
              event === undefined
                ...
                : [
                    ...current.events,
                    eventEnvelope(current, state, event, config.occurredAt),
                  ],
          }
  ...
  return {
    execute,
    disconnectController: (clientId, deadlineEpochMs) =>
      transition((state) =>
        markControllerDisconnected(state, clientId, deadlineEpochMs),
      ),
    reconnectController: (clientId) =>
      transition((state) => markControllerReconnected(state, clientId)),
    expireGrace: (nowEpochMs) =>
      transition(
        (state) => expireControlGrace(state, nowEpochMs),
        (previous) =>
          previous.holderClientId === undefined
            ? undefined
            : DomainEvent.cases.ControlLeaseExpired.make({
                previousHolderClientId: previous.holderClientId,
              }),
      ),
...
> {
  return CommandGateDecision.$match(gate, {
    Accepted: () =>
      acceptControlCommand(current, envelope, command, actor, request, config),
    ReplayPending: () =>
      Effect.fail(
        new ControlReplayInvariantViolation({
          message: 'control command is unexpectedly pending',
        }),
      ),
    ReplayRecorded: () =>
      replayControlCommand(current, receipt).pipe(
        Effect.map((result): ControlCommit => ({ state: current, result })),
      ),
    Rejected: ({ failure }) =>
      Effect.fail(new ControlCommandRejected({ failure })),
  })
}
...
function acceptControlCommand(
  ...
  const transition = decideControlCommand(
    current,
    command,
    actor,
    assignedRequestId,
  ...
  return ControlDecision.$match(transition, {
    Unchanged: ({ reason }) =>
      Effect.fail(new ControlTransitionRejected({ reason })),
    Updated: ({ state }) => {
      const effect = controlCommandEffect(command)
      const requestId = ControlCommand.guards.RequestControl(command)
        ? assignedRequestId
        : undefined
...
function controlCommandEffect(
  command: ControlCommand,
): ControlResponse['effect'] {
  return ControlCommand.match(command, {
...
function controlEvent(
  ...
  return ControlCommand.match(command, {
    RequestControl: (): typeof DomainEvent.Type =>
      DomainEvent.cases.ControlRequested.make({
        requestId: assignedRequestId,
        requesterClientId: actor.clientId,
      }),
    GrantControl: ({ requestId, targetClientId }): typeof DomainEvent.Type =>
      DomainEvent.cases.ControlGranted.make({
        requestId,
        holderClientId: targetClientId,
      }),
    DeclineControl: ({ requestId }): typeof DomainEvent.Type =>
      DomainEvent.cases.ControlDeclined.make({ requestId }),
    ReleaseControl: (): typeof DomainEvent.Type =>
      DomainEvent.cases.ControlReleased.make({
        previousHolderClientId: actor.clientId,
      }),
    TakeControl: (): typeof DomainEvent.Type =>
      DomainEvent.cases.OwnerTookControl.make({
...
function replayControlCommand(
  ...
  if (receipt === undefined) {
    return Effect.fail(
      new ControlReplayInvariantViolation({
        message: 'recorded control result has no receipt',
      }),
    )
  }
  return IdempotencyReceipt.match(receipt, {
    Pending: () =>
      Effect.fail(
        new ControlReplayInvariantViolation({
          message: 'recorded control result is still pending',
        }),
      ),
    Recorded: ({ resultRef }) => {
      ...
      return result === undefined
        ? Effect.fail(
            new ControlReplayInvariantViolation({
              message: 'recorded control result is unavailable',
            }),
          )
