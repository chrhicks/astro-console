...
const heldLease = () => decode(ControlLeaseState, {
  leaseId: "lease-1",
  revision: 4,
  state: "held",
...
const request = (state = heldLease(), requestId = "request-1", clientId = friendClientId) => {
  const decision = requestControl(state, requestId, clientId, 1_000, 61_000)
  assert.equal(decision._tag, "Updated")
  if (decision._tag !== "Updated") throw new Error("expected request to update control coordination")
...
describe("ControlLeaseState", () => {
  it("rejects impossible holder and reconnect combinations", () => {
    assert.throws(() => decode(ControlLeaseState, {
      leaseId: "lease-1",
      revision: 4,
      state: "available",
      holderClientId: ownerClientId,
...
describe("control request coordination", () => {
  it("creates and declines requests without changing the ownership epoch", () => {
    ...
    const declined = resolveControlRequest(requested, ControlRequestResolution.cases.Decline.make({
      requestId: "request-1",
      nowEpochMs: 2_000,
    }))
    assert.equal(declined._tag, "Updated")
    if (declined._tag !== "Updated") throw new Error("expected decline to update request coordination")
  ...
  it("rejects duplicate clients, conflicting request identities, and invalid expiry", () => {
    const requested = request()
    assert.equal(requestControl(requested, "request-2", friendClientId, 2_000, 62_000)._tag, "Unchanged")
    assert.equal(requestControl(requested, "request-1", ClientId.make("client-other"), 2_000, 62_000)._tag, "Unchanged")

    const invalidExpiry = requestControl(heldLease(), "request-2", friendClientId, 2_000, 2_000)
    assert.equal(invalidExpiry._tag, "Unchanged")
    if (invalidExpiry._tag === "Unchanged") assert.equal(invalidExpiry.reason, "RequestExpiryInvalid")
  })

  it("allows a new request after an earlier request expires", () => {
    const requested = request()
    const renewed = requestControl(requested, "request-2", friendClientId, 61_000, 121_000)
    assert.equal(renewed._tag, "Updated")
    if (renewed._tag !== "Updated") throw new Error("expected expired request replacement")
    assert.equal(renewed.state.revision, 4)
    assert.deepEqual(renewed.state.requests.map((candidate) => candidate.requestId), ["request-2"])
  })

  it("requires a current request for the exact available control-capable target", () => {
    const requested = request()

    const mismatch = resolveControlRequest(requested, grant("request-1", ClientId.make("client-other")))
    assert.equal(mismatch._tag, "Unchanged")
    if (mismatch._tag === "Unchanged") assert.equal(mismatch.reason, "ControlTargetMismatch")

    const readOnly = resolveControlRequest(requested, ControlRequestResolution.cases.Grant.make({
      requestId: "request-1",
      nowEpochMs: 2_000,
      target: { clientId: friendClientId, capability: "readOnly", connection: "current" },
    }))
    assert.equal(readOnly._tag, "Unchanged")
    if (readOnly._tag === "Unchanged") assert.equal(readOnly.reason, "ControlTargetUnavailable")

    const disconnected = resolveControlRequest(requested, ControlRequestResolution.cases.Grant.make({
      requestId: "request-1",
      nowEpochMs: 2_000,
      target: { clientId: friendClientId, capability: "controlCapable", connection: "disconnected" },
    }))
    assert.equal(disconnected._tag, "Unchanged")
    if (disconnected._tag === "Unchanged") assert.equal(disconnected.reason, "ControlTargetUnavailable")

    const expired = resolveControlRequest(requested, ControlRequestResolution.cases.Grant.make({
      ...grant(),
      nowEpochMs: 61_000,
    }))
    assert.equal(expired._tag, "Unchanged")
    if (expired._tag === "Unchanged") assert.equal(expired.reason, "RequestExpired")

    const superseded = resolveControlRequest(decode(ControlLeaseState, {
      ...requested,
      revision: 5,
    }), grant())
    assert.equal(superseded._tag, "Unchanged")
    if (superseded._tag === "Unchanged") assert.equal(superseded.reason, "RequestSuperseded")
  })

  it("grants exclusively, advances the ownership epoch once, and resolves incompatible requests", () => {
    const withTwoRequests = request(request(), "request-2", ClientId.make("client-other"))
...
describe("control ownership lifecycle", () => {
  it("starts and clears reconnect grace without changing ownership", () => {
    const disconnected = markControllerDisconnected(heldLease(), ownerClientId, 61_000)
    assert.equal(disconnected._tag, "Updated")
    if (disconnected._tag !== "Updated") throw new Error("expected reconnect grace")
    assert.equal(disconnected.state.revision, 4)
    assert.equal(disconnected.state.state, "reconnecting")

    const repeated = markControllerDisconnected(disconnected.state, ownerClientId, 121_000)
    assert.equal(repeated._tag, "Unchanged")
    if (repeated._tag === "Unchanged") assert.equal(repeated.reason, "AlreadyReconnecting")

    const reconnected = markControllerReconnected(disconnected.state, ownerClientId)
  ...
  it("expires grace to no controller and advances the ownership epoch once", () => {
    const disconnected = markControllerDisconnected(request(), ownerClientId, 61_000)
    assert.equal(disconnected._tag, "Updated")
    if (disconnected._tag !== "Updated") throw new Error("expected reconnect grace")

    assert.equal(expireControlGrace(disconnected.state, 60_999)._tag, "Unchanged")
  ...
  it("makes owner takeover idempotent and resolves incompatible requests", () => {
    ...
    if (alreadyOwner._tag === "Unchanged") assert.equal(alreadyOwner.reason, "AlreadyController")
    ...
    assert.equal(repeated._tag, "Unchanged")
    if (repeated._tag === "Unchanged") assert.equal(repeated.reason, "AlreadyController")
  })
