...
const sequence = (sequenceId: string, targetName: string, rightAscensionHours: number, declinationDegrees: number) =>
...
const previewCommand = (commandId: string, proposedChange: typeof Command.cases.PreviewRunMutation.Type["proposedChange"]) => ({
  commandId,
  command: {
...
const applyCommand = (commandId: string, previewId: string, idempotencyKey: string) => ({
  commandId,
  command: {
...
const approveCommand = (commandId: string, previewId: string, approvalId: string, idempotencyKey: string) => ({
  commandId,
  command: {
...
const requirePreview = (response: typeof RunPreviewResponse.Type): RunMutationPreview =>
  RunPreviewResponse.match(response, {
    Ineligible: ({ consequences }) => assert.fail(`unexpected ineligible preview: ${consequences.join("; ")}`),
...
describe("active-run mutation server proofs", () => {
  it("RUN-02 derives a future-only impact, allows owner preview, and requires the lease to apply once", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeRunMutationServerSimulation(initialState(), 112_000)
      const preview = requirePreview(yield* server.preview(previewCommand(
        "preview-append",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.AppendFutureSequence.make({ sequence: sh2101 }),
      ), owner))

      assert.equal(preview.impact, "nonDisruptive")
      assert.equal("impact" in previewCommand("inspect-command", preview.mutation).command, false)

      const beforeRejectedApply = yield* server.readState()
      const ownerOutcome = yield* server.apply(applyCommand("apply-owner", preview.previewId, "append-owner"), owner).pipe(
        Effect.as("applied" as const),
        Effect.catchTag("RunSimulation.CommandRejected", () => Effect.succeed("rejected" as const)),
      )
      assert.equal(ownerOutcome, "rejected")
      assert.deepEqual(yield* server.readState(), beforeRejectedApply)

      const accepted = yield* server.apply(applyCommand("apply-controller", preview.previewId, "append-controller"), controller)
      const replayed = yield* server.apply(applyCommand("apply-controller-retry", preview.previewId, "append-controller"), controller)
      ...
      assert.equal(state.run.activeExposure?.provisionalEvidenceId, "provisional-frame-15")
      assert.deepEqual(state.run.futureSequenceIds, ["m31", "ngc7000", "sh2-101"])
      assert.deepEqual(state.definition.sequences, initialState().definition.sequences)
      assert.equal(state.run.acceptedMutations.length, 1)
      assert.equal(RunMutation.guards.AppendFutureSequence(state.run.acceptedMutations[0]), true)
      assert.equal(state.events.length, 1)
      assert.equal(state.outbox.length, 1)
      assert.equal(state.receipts.length, 1)
      assert.deepEqual(accepted.projection, RunMutationProjection.make({
  ...
  it("RUN-03 derives notice from forecast consequences before reordering only future work", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const preview = requirePreview(yield* server.preview(previewCommand(
        "preview-reorder",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.ReorderFutureSequences.make({ sequenceIds: ["ngc7000", "m31"] }),
      ), owner))

      assert.equal(preview.impact, "notice")
      assert.equal(preview.consequences.some((consequence) => consequence.includes("1620 seconds")), true)
      assert.equal(preview.consequences.some((consequence) => consequence.includes("14 minutes")), true)

      yield* server.apply(applyCommand("apply-reorder", preview.previewId, "reorder-1"), controller)
      const state = yield* server.readState()
      assert.deepEqual(state.run.futureSequenceIds, ["ngc7000", "m31"])
      assert.equal(state.run.activeSequenceId, "m27")
      assert.equal(state.run.activeExposure?.provisionalEvidenceId, "provisional-frame-15")
      assert.equal(RunWork.guards.RefreshFutureSchedule(state.outbox[0]), true)
    }))
  })
  ...
  it("RUN-04 binds disruptive approval to exact loss and queues stop then slew and reacquire", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const preview = requirePreview(yield* server.preview(previewCommand(
        "preview-switch",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.SwitchTargetNow.make({ sequenceId: "m31" }),
      ), owner))

      assert.equal(preview.impact, "disruptive")
      assert.equal(preview.consequences.some((consequence) => consequence.includes("112 seconds")), true)
      assert.notEqual(preview.approvalId, undefined)
      const beforeApproval = yield* server.readState()

      const ordinaryApply = yield* server.apply(applyCommand("apply-switch", preview.previewId, "switch-ordinary"), controller).pipe(
        Effect.as("applied" as const),
        Effect.catchTag("RunSimulation.MutationRejected", ({ reason }) => Effect.succeed(reason)),
      )
      assert.equal(ordinaryApply, "RequiresApproval")
      assert.deepEqual(yield* server.readState(), beforeApproval)

      const wrongApproval = yield* server.approve(approveCommand("approve-wrong", preview.previewId, "wrong", "switch-wrong"), controller).pipe(
        Effect.as("applied" as const),
        Effect.catchTag("RunSimulation.MutationRejected", ({ reason }) => Effect.succeed(reason)),
      )
      ...
      yield* server.approve(approveCommand("approve-switch", preview.previewId, preview.approvalId ?? "missing", "switch-approved"), controller)
      const state = yield* server.readState()
      assert.equal(state.run.activeSequenceId, "m31")
      assert.equal(state.run.activeExposure, undefined)
      assert.equal(state.run.revision, RunRevision.make(13))
      assert.deepEqual(state.outbox.map((work) => work._tag), ["StopActiveExposure", "SlewAndAcquire"])
  ...
  it("RUN-05 rejects impossible and unauthorized proposals without partial state", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const impossible = yield* server.preview(previewCommand(
        "preview-impossible",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.SwitchTargetNow.make({ sequenceId: "not-queued" }),
      ), owner)
      RunPreviewResponse.match(impossible, {
        Previewed: () => assert.fail("impossible switch must not produce an eligible preview"),
        Ineligible: ({ consequences }) => assert.equal(consequences[0]?.includes("not queued"), true),
      })
      assert.deepEqual(yield* server.readState(), before)

      const unauthorized = yield* server.preview(previewCommand(
        "preview-viewer",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.AppendFutureSequence.make({ sequence: sh2101 }),
      ), viewer).pipe(
  ...
  it("RUN-06 serializes concurrent revision-12 applies and leaves no evidence or work for the loser", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      const server = yield* makeRunMutationServerSimulation(initialState(), 112_000)
      const appendPreview = requirePreview(yield* server.preview(previewCommand(
        "preview-concurrent-append",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.AppendFutureSequence.make({ sequence: sh2101 }),
      ), owner))
      const reorderPreview = requirePreview(yield* server.preview(previewCommand(
        "preview-concurrent-reorder",
        Command.cases.PreviewRunMutation.fields.proposedChange.cases.ReorderFutureSequences.make({ sequenceIds: ["ngc7000", "m31"] }),
      ), owner))
      const apply = (request: unknown) => server.apply(request, controller).pipe(
        Effect.as("applied" as const),
        Effect.catchTag("RunSimulation.CommandRejected", () => Effect.succeed("rejected" as const)),
      )

      const outcomes = yield* Effect.all([
        apply(applyCommand("apply-concurrent-append", appendPreview.previewId, "concurrent-append")),
        apply(applyCommand("apply-concurrent-reorder", reorderPreview.previewId, "concurrent-reorder")),
