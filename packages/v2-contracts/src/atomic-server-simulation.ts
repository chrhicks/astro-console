...
export interface AtomicServerSimulation<State, Work> {
  readonly transact: <Result, Error, Requirements = never>(
    decide: (state: State) => Effect.Effect<AtomicCommit<State, Result>, Error, Requirements>,
...
export const makeAtomicServerSimulation = <State, Work>(
...
): Effect.Effect<AtomicServerSimulation<State, Work>> => Effect.gen(function* () {
  const stateRef = yield* Ref.make(initialState)
  const transaction = yield* Semaphore.make(1)

  const transact: AtomicServerSimulation<State, Work>["transact"] = (decide) =>
    transaction.withPermit(Effect.gen(function* () {
  ...
  const readState = Effect.fn("AtomicServerSimulation.readState")(function* () {
    return yield* Ref.get(stateRef)
  })

  const dispatchOutbox: AtomicServerSimulation<State, Work>["dispatchOutbox"] = (execute) =>
    Ref.get(stateRef).pipe(
      Effect.flatMap((state) => Effect.forEach(outbox(state), execute, { discard: true })),
    )
