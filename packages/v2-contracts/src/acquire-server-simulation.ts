...
export interface AcquireRecordedEvent extends Schema.Schema.Type<
  typeof AcquireRecordedEvent
> {}
...
export interface AcquireProjection extends Schema.Schema.Type<
  typeof AcquireProjection
> {}
...
export interface AcquireResponse extends Schema.Schema.Type<
  typeof AcquireResponse
> {}
...
export interface AcquireServerSimulation {
  readonly submit: (
    ...
    AcquireResponse,
    | Schema.SchemaError
    | AcquireCommandRejected
    | AcquireDecisionRejected
    | AcquireReplayUnavailable
  >
  readonly completeSolve: (
    rawCompletion: unknown,
  ) => Effect.Effect<
    AcquireProjection,
    Schema.SchemaError | AcquireWorkerResultRejected
  >
  ...
  readonly dispatchOutbox: <E>(
    execute: (work: typeof AcquireWork.Type) => Effect.Effect<void, E>,
  ) => Effect.Effect<void, E>
}

type AcquireServiceError =
  AcquireCommandRejected | AcquireDecisionRejected | AcquireReplayUnavailable
type AcquireCommit = AtomicCommit<AcquireServerState, AcquireResponse>

export const makeAcquireServerSimulation = Effect.fn('AcquireServer.make')(
  function* (initialState: AcquireServerState) {
    const simulation = yield* makeAtomicServerSimulation(
      initialState,
      (state) => state.outbox,
    )

    const submit: AcquireServerSimulation['submit'] = Effect.fn(
      'AcquireServer.submit',
    )(function* (rawRequest, actor, nowEpochMs) {
      const envelope =
        yield* Schema.decodeUnknownEffect(CommandEnvelope)(rawRequest)
      const command = yield* Schema.decodeUnknownEffect(AcquireCommand)(
        envelope.command,
      )
        ...
        Effect.gen(function* () {
          const request = commandRequiresIdempotency(command)
            ? IdempotencyRequest.make({
                ...
                normalizedInputHash: versionedSemanticHash(
                  `${command._tag}.v1`,
                  command,
                ),
              })
    ...
    const completeSolve: AcquireServerSimulation['completeSolve'] = Effect.fn(
      'AcquireServer.completeSolve',
    )(function* (rawCompletion) {
      const completion =
        yield* Schema.decodeUnknownEffect(SolveCompletion)(rawCompletion)
      return yield* simulation.transact((current) =>
        Effect.gen(function* () {
          const decision = recordSolveCompletion(current.session, completion)
          return yield* SolveCompletionDecision.$match(decision, {
            Rejected: ({ reason }) =>
              Effect.fail(new AcquireWorkerResultRejected({ reason })),
            Centered: ({ session }) =>
              Effect.succeed(
                workerCommit(
                  current,
                  ...
                  [
                    AcquireWork.cases.ContinueToCapture.make({
                      operationId: OperationId.make(
                        `continue-after-${completion.attemptId}`,
                      ),
                    }),
                  ],
                ),
              ),
            RetryScheduled: ({ session }) =>
              Effect.succeed(
                workerCommit(
                  current,
                  session,
                  'SolveRetryScheduled',
                  [completion.attemptId],
                  workFromActive(session),
                ),
              ),
            AutomaticCorrectionStarted: ({ session }) =>
              Effect.succeed(
                workerCommit(
                  current,
                  session,
            ...
            CorrectionApprovalRequired: ({ session }) =>
              Effect.succeed(
                workerCommit(
                  current,
                  session,
    ...
    const acknowledgeCorrection: AcquireServerSimulation['acknowledgeCorrection'] =
      Effect.fn('AcquireServer.acknowledgeCorrection')(function* (input) {
        return yield* simulation.transact((current) =>
          Effect.gen(function* () {
    ...
    const completePolarMeasurement: AcquireServerSimulation['completePolarMeasurement'] =
      Effect.fn('AcquireServer.completePolar')(function* (input) {
        return yield* simulation.transact((current) =>
          Effect.gen(function* () {
            const decision = recordPolarMeasurementEvidence(current.session, {
              ...input,
              sourceFrameAssetId: Schema.NonEmptyString.pipe(
                Schema.brand('AssetId'),
              ).make(input.sourceFrameAssetId),
            })
            return yield* PolarDecision.$match(decision, {
              Rejected: ({ reason }) =>
                Effect.fail(new AcquireWorkerResultRejected({ reason })),
              GuidanceUpdated: ({ session }) =>
                Effect.succeed(
                  workerCommit(
                    current,
                    session,
              ...
              MeasurementScheduled: () =>
                Effect.fail(
                  new AcquireWorkerResultRejected({
                    reason: 'UnexpectedMeasurementScheduled',
                  }),
                ),
              Accepted: () =>
                Effect.fail(
                  new AcquireWorkerResultRejected({
                    reason: 'UnexpectedPolarAcceptance',
                  }),
                ),
            })
          }),
...
function acceptGate(
  ...
  return CommandGateDecision.$match(gate, {
    Rejected: ({ failure }) =>
      Effect.fail(new AcquireCommandRejected({ failure })),
    ReplayPending: () => Effect.fail(new AcquireReplayUnavailable()),
    ReplayRecorded: () => replay(current, receipt),
    Accepted: () =>
      decideCommand(current, command, commandId, request, nowEpochMs),
  })
}
...
function decideCommand(
  ...
  return AcquireCommand.match(command, {
    RetryPlateSolveWithParameters: ({ parameters }) =>
      RecoverySeriesDecision.$match(
    ...
    SkipAcquireTarget: () =>
      AcquireSkipDecision.$match(
        skipPausedAcquireTarget(current.session, current.nextSequenceId),
        {
          Rejected: ({ reason }) => rejectDecision(reason),
          Skipped: ({ session, nextSequenceId }) =>
            acceptCommand(
              current,
              session,
              commandId,
    ...
    ApprovePointingCorrection: ({ proposalId }) =>
      CorrectionCommandDecision.$match(
    ...
    RevisePointingCorrection: ({ proposalId, parameters }) => {
      const convention =
        current.session.pendingCorrectionProposal?.correction.convention ??
        'mountRaDec'
      return CorrectionCommandDecision.$match(
        reviseCorrectionProposal(current.session, {
    ...
    CapturePolarAlignmentMeasurement: () =>
      PolarDecision.$match(
        requestPolarMeasurement(
          current.session,
          AttemptId.make(`polar-attempt-${suffix}`),
        ),
        {
          Rejected: ({ reason }) => rejectDecision(reason),
          MeasurementScheduled: ({ session }) =>
            acceptCommand(
              current,
              session,
              commandId,
    ...
    AcceptPolarAlignmentEvidence: ({ attemptId }) =>
      PolarDecision.$match(
        acceptLatestPolarMeasurement(current.session, attemptId),
        {
          ...
          MeasurementScheduled: () =>
            rejectDecision('UnexpectedMeasurementScheduled'),
          GuidanceUpdated: () => rejectDecision('UnexpectedPolarGuidance'),
        },
      ),
  })
...
function acceptCommand(
  ...
  const resultRef =
    request === undefined
      ? undefined
      : CommandResultRef.make(`result-${commandId}`)
...
function commitState(
  ...
  return {
    ...
    events: [
      ...current.events,
      AcquireRecordedEvent.make({
        eventId:
          commandId === undefined
            ? `worker-event-${eventCursor}`
    ...
    receipts:
      request === undefined || resultRef === undefined
        ? current.receipts
        : [
            ...current.receipts,
...
function replay(
  ...
  return IdempotencyReceipt.match(receipt, {
    ...
    Recorded: ({ resultRef }) =>
      current.results.some((result) => result.resultRef === resultRef)
        ? Effect.succeed({
            state: current,
            result: AcquireResponse.make({
              replayed: true,
...
function rejectDecision(
  reason: string,
): Effect.Effect<never, AcquireDecisionRejected> {
  return Effect.fail(new AcquireDecisionRejected({ reason }))
}

function workFromActive(
  session: AcquireSession,
): ReadonlyArray<typeof AcquireWork.Type> {
  ...
  return AcquireActiveWork.match(session.activeWork, {
    SolveRequested: ({
      ...
      AcquireWork.cases.CaptureAndSolve.make({
    ...
    CorrectionRequested: ({
      ...
      AcquireWork.cases.MovePointingCorrection.make({
    ...
    PolarMeasurementRequested: ({
      ...
      AcquireWork.cases.CapturePolarMeasurement.make({
