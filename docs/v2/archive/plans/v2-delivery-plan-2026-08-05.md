...
Status: **V2.0 complete — Phases 0 through 6 and Production Convergence complete**
...
## Phase 6: Remote Viewing and Shared Control — Complete

Phase 6 was the final V2.0 phase. Its completed implementation record is in the
[Phase 6 archive](../archive/phase-6/implementation-plan-2026-08-05.md). It is split
into remote read-only viewing and shared control so public admission and normal
viewing are proven before any remote client can hold the existing exclusive
control lease.
...
1. **Remote read-only viewing:** publish the version-matched service through
   Cloudflare Tunnel and Access, verify a managed identity at the origin, map it
   to an owner or viewer membership, and expose the authoritative snapshot/SSE
   projection. Desktop viewers and all phone clients remain read-only.
2. **Shared control:** project durable client presence and the existing
   service-owned lease; add request, grant, decline, release, reconnect, and
   owner-take actions. Only an eligible non-phone client holding the current
   lease can submit an already-supported observing command.
3. **Remote evidence and closeout:** bound previews and explicit original
   downloads, run supervised public-path and outage drills, then close V2.0
   with the evidence and limits stated in the plan.

The existing Arch deployment already provides the Cloudflare account/zone,
Access policy, hostname, tunnel, and origin path. Phase 6 validates and
integrates that deployment; it does not create a second public path. It does
not authorize a provider write, a real rig command, or a physical capture
claim. Those require a separate approved boundary.
...
Phase 6 is complete with local, visual, deployment, and owner public-path
evidence. The detailed execution record is archived in
[Phase 6 archive](../archive/phase-6/implementation-plan-2026-08-05.md).
A successful public URL proves remote ingress and identity admission; it does
not prove hardware behavior.

## V2.0 Completion Rule

V2.0 completes when Phase 6 meets its exit criteria and the closeout records
the exact local, public-path, and visual proof boundaries. The completion
handoff must state that the service is still rig-local, Cloudflare is ingress
and identity admission rather than observatory authority, and no untested
provider, hardware, or physical-image behavior is claimed.

Do not turn completion into a prerequisite for V2.1, V2.5, or V3 ideas. New
versions may revisit intentionally deferred work after V2.0.
