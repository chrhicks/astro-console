# Phase 1 Host-Verification Record

Status: **historical evidence — superseded by the current handoff**

This preserves the completed July 29 host-verification chronology and detailed
test evidence. Use the [current handoff](../../current/handoff.md) for active
scope, current proof boundaries, and next work.
...
- Its SQLite/HTTP/SSE/worker/filesystem integration suite passes **58/58**
  tests. Those
...
- The deterministic M27 Plan→Observe fixture retains its accepted run, lease,
  SSE, reconnect, pause, resume, and stop projections without creating generic
  hardware work. The deployed rig worker claims only the separately named
  Solar intents; retired generic fixture rows are cancelled on migration.
...
- The disabled manifest processor can now first ingest a host-declared,
  ...
  group/run/solve lineage, checksum, receipt, event, and a recorded orphan on
  transaction failure. A selected output must reference that durable original and exactly
  match its lineage before it receives a `PublishAsset` record. Originals are
  never published by ingest alone. It is retained as a separate one-shot
  evidence utility with its own configuration example, never as origin
  configuration or an active workflow. The source-ingest tables are numbered
...
- The owner-only operations projection reports disk as `unknown`; no deployed
  storage measurement or capture-admission policy is configured. Process Save
  retains its bounded cleanup of recorded promoted-output orphans under the
  configured app-owned output root. This is local filesystem/SQLite proof; it
  does not configure external backup, R2, or a storage-health workflow.
...
2. [Phase 1 delivery plan](../../current/delivery-plan.md)
