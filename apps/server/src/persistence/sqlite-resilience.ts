...
export function assertSeparateFilesystems(
  sourcePath: string,
  backupDirectory: string,
  deviceId = (path: string) => statSync(path).dev,
) {
  if (deviceId(sourcePath) === deviceId(backupDirectory))
    throw new Error(
      'Backup destination must be on a different filesystem than the live database',
    )
}
...
export function restoreDrill(
  backupPath: string,
  drillPath: string,
): { readonly source: BackupEvidence; readonly restored: BackupEvidence } {
  if (resolve(backupPath) === resolve(drillPath))
    throw new Error('Restore drill target must be separate from the backup')
  if (existsSync(drillPath))
    throw new Error('Restore drill target already exists')
  const source = verifySqlite(backupPath)
  mkdirSync(dirname(drillPath), { recursive: true })
  const database = new DatabaseSync(backupPath)
  try {
    database.exec(`VACUUM INTO '${drillPath.replaceAll("'", "''")}'`)
  } finally {
    database.close()
  }
  try {
    const restored = verifySqlite(drillPath)
