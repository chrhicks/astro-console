...
const makeGraph = (
  ...
  Effect.gen(function* () {
    ...
    const publication = Context.get(publicationContext, ProjectionPublication)
...
test('sequential scope shutdown finalizes active SSE before closing the listener', async () => {
  const scope = Effect.runSync(Scope.make('sequential'))
  const graph = await Effect.runPromise(
    Scope.provide(makeGraph(webFixture()), scope),
  )
  ...
  assert.ok(graph.publicationEvents.includes('disconnect'))
