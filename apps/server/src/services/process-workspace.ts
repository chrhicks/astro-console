...
export function processSnapshot(
  ...
  return projectProcessingProjection(read(database), {
...
export function executeProcessCommand(
  ...
  try {
    ...
    const work = Effect.runSync(processor.readState()).outbox.slice(
      outboxLength,
    )[0]
    if (work?._tag === 'BuildLinearMaster')
      ...
      work?._tag === 'RunAppliedOperation' ||
      work?._tag === 'RetryProcessingStage'
    )
      Effect.runSync(
        work._tag === 'RunAppliedOperation' &&
          Effect.runSync(processor.readState()).sessions.find(
          ...
          : processor.completeApply(
              ...
              `sha256:output-${work.attemptId}`,
              CheckpointId.make(`checkpoint-${work.attemptId}`),
            ),
      )
    else if (work?._tag === 'MaterializeProcessingArtifacts') {
      const saved = Effect.runSync(
        processor.completeSave(
          work.operationId,
          work.artifacts.map((artifact) =>
            StagedArtifact.make({
              assetId: AssetId.make(`asset-process-${randomUUID()}`),
              outputId: artifact.outputId,
              role: artifact.role,
              format: artifact.format,
              checksum: `sha256:${artifact.outputId}`,
