...
function text(node,value){node.textContent=value}async function send(path,payload){if(!projection||projection.connection==='stale'||reconnecting)return;const v=await fetch(path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)}).then(r=>r.json());text(q('#notice'),v.message||'The service did not accept this action.');await loadProjection()}
...
async function previewMutation(mutation,label){const preview=await fetch('/api/commands/preview-run-mutation',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({_tag:'PreviewRunMutation',mutation,expectedLeaseRevision:projection.control.revision,expectedRunRevision:projection.run.revision,idempotencyKey:crypto.randomUUID()})}).then(response=>response.json());text(q('#notice'),preview.preview?.consequences||preview.message||'Preview unavailable.');if(!preview.preview)return;const approval=preview.preview.approvalRequired;q('#control-actions').append(action(approval?'Approve discard consequence':'Apply '+label,()=>send(approval?'/api/commands/approve-disruptive-run-mutation':'/api/commands/apply-run-mutation',{_tag:approval?'ApproveDisruptiveRunMutation':'ApplyRunMutation',previewId:preview.preview.previewId,approvalToken:preview.approvalToken,expectedLeaseRevision:projection.control.revision,expectedRunRevision:projection.run.revision,idempotencyKey:crypto.randomUUID()})))}
function action(label,fn){if(label==='Run plan')return document.createComment('Run plan deferred');if(label==='Stop fake run')label='Stop run';const b=document.createElement('button');text(b,label);b.onclick=fn;return b}
function phoneReadOnly(s){if(s.identity.capability==='readOnly')return true;return false}
function observeTitle(run){if(!run)return'No active run';if(run.phase==='preflight')return'Preflight · fake run is preparing';if(run.phase==='acquire')return'Acquire · fake run is centering';if(run.phase==='capture')return'Capture · fake run is continuing';if(run.phase==='verify')return'Verify · fake run is checking';if(run.phase==='completed')return'Completed · fake run finished';if(run.phase==='paused')return'Paused · fake run awaits Resume';if(run.phase==='stopped')return'Stopped · fake run will not resume';return'Park requested · no mount moved'}
function compactViewport(){if(innerWidth<=600)return true;return false}
const pauseBoundary='Pause preserves this deterministic managed run; it does not send hardware work.'
const resumeBoundary='Resume restores the preserved deterministic phase; it does not send hardware work.'
const stopBoundary='Stop is terminal: this managed run cannot be resumed. It does not send hardware work.'
...
let eventCursor=-1,snapshotVersion=-1,reconnecting=false;function validBootstrap(data){return data&&Number.isInteger(data.snapshotVersion)&&Number.isInteger(data.eventCursor)&&typeof data.generatedAt==='string'&&data.membership&&data.control&&data.activeRun&&data.health}function parseBootstrap(text){try{const data=JSON.parse(text);return validBootstrap(data)?data:null}catch{return null}}function cursorDecision(data,id){if(id!==data.eventCursor)return'reconnect';if(eventCursor<0)return'next';if(data.eventCursor<=eventCursor)return'duplicate';if(data.eventCursor!==eventCursor+1||data.snapshotVersion<snapshotVersion)return'reconnect';return'next'}function bootstrapProjection(data){const run=data.activeRun._tag==='Active'?data.activeRun.run:null;return{identity:data.membership,control:data.control,run,connection:'current',evidence:{frameId:'No current optical evidence in bootstrap projection.',capturedAt:data.generatedAt,quality:'warning',desired:'Not observed',solved:'Not observed',uncertaintyArcsec:0,correction:{state:'exhausted',evidence:'Optical evidence is not included in this bootstrap projection.',bound:'Use Observe evidence when it is installed.',protection:'No automatic correction or hardware command is implied.',action:'Review the current service projection.'}}}}function installBootstrap(data,id,reset=false){if(!validBootstrap(data)||(reset?false:cursorDecision(data,id)!=='next'))return false;eventCursor=data.eventCursor;snapshotVersion=data.snapshotVersion;render(bootstrapProjection(data));return true}function markStale(){if(!projection)return;projection={...projection,connection:'stale'};render(projection)}function reconnect(){markStale();void refreshProjection()}function eventProjection(event){const data=parseBootstrap(event.data),id=Number(event.lastEventId);if(data===null||!Number.isInteger(id)||cursorDecision(data,id)==='reconnect')return reconnect();if(cursorDecision(data,id)==='next')installBootstrap(data,id);return projection}async function refreshProjection(){if(reconnecting)return;reconnecting=true;markStale();try{const envelope=await fetch('/api/snapshot').then(r=>r.json()),data=envelope.ok===true&&validBootstrap(envelope.data)?envelope.data:null;if(data===null)return;installBootstrap(data,data.eventCursor,true);await loadPlan()}catch{}finally{reconnecting=false}}async function loadProjection(){await refreshProjection()}async function loadPlan(){plan=await fetch('/api/workspaces/plan').then(r=>r.json());renderPlan()}async function load(){await loadProjection();await loadPlan();select('Plan');const e=new EventSource('/api/events');e.addEventListener('ProjectionChanged',event=>{if(!reconnecting)eventProjection(event)});e.onerror=()=>reconnect()}addEventListener('resize',()=>{if(projection){render(projection);renderPlanActions()}});addEventListener('orientationchange',()=>{if(projection){render(projection);renderPlanActions()}});load()/* detail.availability==='availableLocally'; no action will be replayed; temporarily unavailable and cannot open in Process */`
