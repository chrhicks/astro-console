...
export type DownloadGrantIssuer = {
...
export function createR2DownloadGrantIssuer(config: {
  ...
  return {
    issue: async ({ objectKey, expiresAt }) => {
      if (
        !/^published\/[A-Za-z0-9._/-]+$/.test(objectKey) ||
        objectKey.includes('//') ||
        objectKey.includes('..')
      )
        throw new Error('R2 object key is outside the publisher prefix')
      const expiresSeconds = Math.floor(
        (Date.parse(expiresAt) - now().valueOf()) / 1_000,
      )
      if (
        ...
        throw new Error(
          'R2 download grant expiry must be between one second and five minutes',
        )
      const url = new URL(
      ...
      url.search = serializeQuery(query)
      return url.toString()
    },
  }
}
