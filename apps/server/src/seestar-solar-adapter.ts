  ...
  return {
    startSolarTestObservation: async (work) => {
      try {
        ...
        if (
          preflight.mountClosed ||
          (preflight.viewMode && preflight.viewMode !== 'none')
        )
          return 'uncertain'
        unsubscribe?.()
        unsubscribe = device.rawClient.subscribeToPushEvents((event) => {
          if (event.Event === 'Stack')
            options.onStack(work.intentId, event, new Date().toISOString())
        })
        const startedView = await device.startView('sun', undefined, {
          waitForCompletion: true,
          timeoutMs: 30_000,
          pollIntervalMs: 500,
        })
        if (!startedView) return 'uncertain'
        const startedStack = await device.startStack(true, {
    ...
    stopSolarTestObservation: async () => {
      try {
        const stoppedStack = await device.stopStack({
          waitForCompletion: true,
          timeoutMs: 30_000,
          pollIntervalMs: 500,
        })
        const stoppedView = await device.stopView(undefined, {
          waitForCompletion: true,
          timeoutMs: 30_000,
          pollIntervalMs: 500,
        })
