...
const DurableOriginal = Schema.Struct({
  asset_id: Schema.String,
  role: Schema.Literal('original'),
  comparison_group_id: Schema.String,
  detail: Schema.String,
})
const DurableOriginalDetail = Schema.Struct({
  lineage: Schema.Struct({
    runId: Schema.String,
    solveAttemptId: Schema.String,
  }),
})
const OrphanPath = Schema.Struct({ path: Schema.String })
export type ProcessSaveStorage = {
  readonly sourcesRoot: string
  readonly outputsRoot: string
  readonly sources: Readonly<Record<string, string>>
}
export type ProcessSaveResult =
  | { readonly outcome: 'accepted'; readonly assetIds: ReadonlyArray<string> }
  | {
      ...
      readonly reason:
        ...
        | 'OrphanRecordingFailed'
    }

export function saveProcessOutputs(
  database: DatabaseSync,
  storage: ProcessSaveStorage,
  raw: unknown,
  ...
  try {
    input = Schema.decodeUnknownSync(SaveInput)(raw)
  } catch {
  ...
  if (
    ...
    new Set(input.outputs.map((output) => output.representation)).size !==
      input.outputs.length
  )
    return { outcome: 'rejected', reason: 'InvalidInput' }
  if (
    input.metadata !== undefined &&
    !matchesDurableSourceMetadata(database, input.metadata)
  )
    return { outcome: 'rejected', reason: 'InvalidInput' }
  ...
  const existingRaw: unknown = database
    .prepare(
      'SELECT semantic_key,response FROM process_save_receipts WHERE idempotency_key=? AND owner_person_id=?',
    )
  ...
  try {
    ...
    for (const output of input.outputs) {
      ...
      const finalPath = appPath(
        storage.outputsRoot,
        `${assetId}.${output.representation === 'final' ? 'tiff' : 'fits'}`,
      )
    ...
    if (!recordOrphans(database, staged))
      return { outcome: 'rejected', reason: 'OrphanRecordingFailed' }
    return { outcome: 'rejected', reason: 'MaterializationFailed' }
  }
  try {
    database.exec('BEGIN IMMEDIATE')
    const result: ProcessSaveResult = {
      outcome: 'accepted',
      assetIds: staged.map((output) => output.assetId),
    }
    for (const output of staged) {
      ...
      database
        ...
        .run(output.assetId, 'ProcessSaved', output.checksum)
      database
    ...
    database
      .prepare('INSERT INTO process_save_receipts VALUES (?,?,?,?)')
      .run(
...
export function cleanupProcessOrphans(
  database: DatabaseSync,
  storage: ProcessSaveStorage,
) {
  const rowsRaw: unknown = database
    .prepare('SELECT path FROM process_save_orphans ORDER BY rowid LIMIT 100')
    .all()
  const rows = Schema.decodeUnknownSync(Schema.Array(OrphanPath))(rowsRaw)
  return rows.reduce((removed, row) => {
    try {
      const path = appPath(storage.outputsRoot, basename(row.path))
      if (existsSync(path)) rmSync(path)
      database
        .prepare('DELETE FROM process_save_orphans WHERE path=?')
        .run(row.path)
      return removed + 1
    } catch {
      return removed
...
function selectedSource(storage: ProcessSaveStorage, sourceId: string) {
...
function matchesDurableSourceMetadata(
  database: DatabaseSync,
  metadata: {
    readonly comparisonGroupId: string
    readonly sourceAssetIds: ReadonlyArray<string>
    readonly runId: string
    readonly solveAttemptId: string
  },
) {
  if (
    metadata.sourceAssetIds.length === 0 ||
    metadata.sourceAssetIds.length > 16 ||
    new Set(metadata.sourceAssetIds).size !== metadata.sourceAssetIds.length
  )
    return false
  return metadata.sourceAssetIds.every((assetId) => {
    try {
      const original = Schema.decodeUnknownSync(
        Schema.optional(DurableOriginal),
      )(
        database
          .prepare(
            'SELECT asset_id,role,comparison_group_id,detail FROM library_assets WHERE asset_id=?',
          )
          .get(assetId),
      )
      if (
        original === undefined ||
        original.comparison_group_id !== metadata.comparisonGroupId
      )
        return false
      const detail = Schema.decodeUnknownSync(DurableOriginalDetail)(
        JSON.parse(original.detail),
      )
      return (
        detail.lineage.runId === metadata.runId &&
        detail.lineage.solveAttemptId === metadata.solveAttemptId
      )
    } catch {
      return false
    }
...
function appPath(root: string, child: string) {
  ...
  if (
    relative(resolvedRoot, candidate).startsWith('..') ||
...
function checksumFile(path: string) {
  return createHash('sha256').update(requireBytes(path)).digest('hex')
}
function requireBytes(path: string) {
  return readFileSync(path)
}
function recordOrphans(
  database: DatabaseSync,
  ...
  try {
    staged.forEach((output) =>
      database
        .prepare('INSERT OR IGNORE INTO process_save_orphans VALUES (?,?,?)')
        .run(output.finalPath, output.checksum, new Date().toISOString()),
    )
    return true
  } catch {
    return false
