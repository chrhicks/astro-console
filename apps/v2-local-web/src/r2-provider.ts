...
export function createR2Provider(config: Pick<R2PublisherConfig, "accountId" | "bucket" | "endpoint" | "credentialsPath">, fetcher: Fetcher = fetch): PublisherProvider {
  ...
  const request = async (method: "PUT" | "HEAD", key: string, file?: PublisherFile, metadata?: { readonly assetId: string; readonly checksum: string }) => {
  ...
  return { put: async (key, file, metadata) => { await request("PUT", key, file, metadata) }, head: async (key) => { const response = await request("HEAD", key); const checksum = response.headers.get("x-amz-meta-checksum"); const bytes = response.headers.get("content-length"); if (checksum === null || bytes === null || !/^\d+$/.test(bytes)) return undefined; return { checksum, bytes: Number(bytes) } } }
}
