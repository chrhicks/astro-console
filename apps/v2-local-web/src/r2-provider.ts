...
import type { PublisherProvider } from "./publisher-worker.ts"
...
export function createR2Provider(config: Pick<R2PublisherConfig, "accountId" | "bucket" | "endpoint" | "credentialsPath">, fetcher: Fetcher = fetch): PublisherProvider {
  ...
  const request = async (method: "PUT" | "HEAD", key: string, bytes?: Uint8Array, metadata?: { readonly assetId: string; readonly checksum: string }) => {
    if (!/^published\/[A-Za-z0-9._/-]+$/.test(key) || key.includes("//") || key.includes("..")) throw new Error("R2 object key is outside the publisher prefix")
    const url = new URL(`/${config.bucket}/${key.split("/").map(encodeURIComponent).join("/")}`, config.endpoint)
    const payloadHash = createHash("sha256").update(bytes ?? new Uint8Array()).digest("hex"); const amzDate = new Date().toISOString().replace(/[:-]|\.\d{3}/g, ""); const date = amzDate.slice(0, 8)
    const headers: Record<string, string> = { host: url.host, "x-amz-content-sha256": payloadHash, "x-amz-date": amzDate, ...(metadata === undefined ? {} : { "x-amz-meta-asset-id": metadata.assetId, "x-amz-meta-checksum": metadata.checksum }) }
    ...
    const response = await fetcher(url, { method, headers: { ...headers, authorization: `AWS4-HMAC-SHA256 Credential=${credentials.accessKeyId}/${scope}, SignedHeaders=${signedHeaders.join(";")}, Signature=${signature}` }, ...(bytes === undefined ? {} : { body: bytes }) })
    if (!response.ok) throw new Error(`R2 ${method} failed with ${response.status}`)
    return response
  }
  return { put: async (key, bytes, metadata) => { await request("PUT", key, bytes, metadata) }, head: async (key) => { const response = await request("HEAD", key); const checksum = response.headers.get("x-amz-meta-checksum"); const bytes = response.headers.get("content-length"); if (checksum === null || bytes === null || !/^\d+$/.test(bytes)) return undefined; return { checksum, bytes: Number(bytes) } } }
}
