...
import { configuredAdmission, configuredListenHost, configuredListenPort, configuredRuntime, createJwksKeyResolver, createLocalWebService as createRuntimeService, createMembershipBootstrapResolver, createProductionAccessAdmission, openMigrationDatabase, openProcessorDatabase, openPublisherDatabase } from "./server.ts"
...
function assertNoM27Fixture(database: DatabaseSync) {
  assert.equal((database.prepare("SELECT count(*) AS count FROM state WHERE value LIKE '%m27%'").get() as { count: number }).count, 0)
  assert.equal((database.prepare("SELECT count(*) AS count FROM library_assets WHERE asset_id LIKE '%m27%'").get() as { count: number }).count, 0)
  assert.equal((database.prepare("SELECT count(*) AS count FROM workspace_projections WHERE value LIKE '%m27%'").get() as { count: number }).count, 0)
}
...
test("authorized Library downloads issue an Asset-ID grant and redirect without projecting bearer data", async (t) => {
  ...
  const first = await request(); assert.equal(first.status, 303); assert.equal(first.headers.get("cache-control"), "private, no-store"); assert.equal(first.headers.get("referrer-policy"), "no-referrer"); assert.equal(first.headers.get("location"), "https://r2.example/published/run-m27-001/finals/asset-m27-001-checksum.fits?X-Amz-Signature=bearer-1"); assert.equal(await first.text(), ""); assert.deepEqual(grants[0], { objectKey: "published/run-m27-001/finals/asset-m27-001-checksum.fits", expiresAt: "2026-07-28T12:05:00.000Z" })
  const next = await request(); assert.equal(next.status, 303); assert.equal(grants.length, 2); assert.equal(grants[1]?.expiresAt, "2026-07-28T12:05:00.000Z"); assert.equal(JSON.stringify(await fetch(`${base}/api/snapshot`, { headers: { authorization: "Bearer viewer" } }).then((response) => response.json())).includes("published/run"), false); assert.equal(JSON.stringify(await fetch(`${base}/api/library/assets/${assetId}`, { headers: { authorization: "Bearer viewer" } }).then((response) => response.json())).includes("published/run"), false)
  now = new Date("2026-07-28T12:05:01.000Z"); assert.equal((await request()).status, 303); assert.equal((await request("Bearer owner")).status, 303); service.database.prepare("UPDATE library_assets SET availability='published' WHERE asset_id='asset-m27-002'").run(); service.database.prepare("INSERT INTO asset_publications (asset_id,checksum,state,updated_at,object_key) VALUES (?,?,?,?,?)").run("asset-m27-002", "checksum", "published", now.toISOString(), "published/run-m27-001/finals/asset-m27-002-checksum.fits"); assert.equal((await fetch(`${base}/api/library/assets/asset-m27-002/download?requestId=ignored`, { headers: { authorization: "Bearer viewer" }, redirect: "manual" })).status, 303)
  service.database.prepare("UPDATE asset_publications SET state='temporarilyUnavailable' WHERE asset_id=?").run(assetId); assert.equal((await request()).status, 409); assert.equal((await fetch(`${base}/api/library/assets/asset-m27-999/download`, { headers: { authorization: "Bearer viewer" }, redirect: "manual" })).status, 404); assert.equal((await fetch(`${base}/api/library/assets/not-an-asset/download`, { headers: { authorization: "Bearer viewer" }, redirect: "manual" })).status, 400)
  service.database.prepare("UPDATE asset_publications SET state='published' WHERE asset_id=?").run(assetId); issuerUnavailable = true; assert.equal((await request()).status, 503)
  const credentialFree = createLocalWebService(); const credentialFreeListener = await credentialFree.listen(); assert.equal((await fetch(`http://127.0.0.1:${credentialFreeListener.port}/api/library/assets/asset-m27-001/download`, { redirect: "manual" })).status, 503); await credentialFreeListener.close(); credentialFree.close()
})
...
test("numbered SQLite migrations upgrade a legacy database and reject a newer schema", () => {
  ...
  seeded.database.prepare("DELETE FROM schema_migrations WHERE version>13").run(); seeded.database.exec("DROP TABLE observing_plans; DROP TABLE observing_plan_receipts")
  seeded.database.prepare("UPDATE workspace_projections SET value=? WHERE name='plan'").run(JSON.stringify({ planId: "plan-m27", revision: 3, target: "M27 · Dumbbell Nebula", readiness: "ready", readinessSummary: "Legacy fixture projection.", observingWindow: { startsAt: "2026-07-25T03:18:00.000Z", endsAt: "2026-07-25T05:02:00.000Z", usableMinutes: 104, peakAltitudeDeg: 62, horizonClearanceDeg: 28 }, sequences: [{ sequenceId: "sequence-m27-luminance", target: "M27 · Dumbbell Nebula", capture: "24 × 180s · L", acquisition: "Solve, center, focus, then start capture.", stopCondition: "Stop at 24 verified frames or 01:02 local.", viability: "viable" }] }))
  seeded.close()
  const migratedFixture = createLocalWebService(fixturePath)
  const migratedPlan = JSON.parse((migratedFixture.database.prepare("SELECT value FROM workspace_projections WHERE name='plan'").get() as { value: string }).value)
  assert.equal(migratedPlan.sequences.length, 2); assert.deepEqual(migratedPlan.limitations, []); assert.equal(migratedPlan.sequences[0].window.horizonClearanceDeg, 28)
  migratedFixture.close()
  const recorded15Path = join(mkdtempSync(join(tmpdir(), "astro-recorded-15-plan-")), "state.sqlite")
  const recorded15 = createLocalWebService(recorded15Path)
  recorded15.database.prepare("DELETE FROM schema_migrations WHERE version>=16").run(); recorded15.database.prepare("UPDATE workspace_projections SET value=? WHERE name='plan'").run(JSON.stringify({ planId: "plan-m27", revision: 3, target: "M27", readiness: "ready", readinessSummary: "Recorded schema-15 fixture projection.", observingWindow: { startsAt: "2026-07-25T03:18:00.000Z", endsAt: "2026-07-25T05:02:00.000Z", usableMinutes: 104, peakAltitudeDeg: 62, horizonClearanceDeg: 28 }, sequences: [{ sequenceId: "legacy-l", order: 1, target: "M27", capture: "24 × 180s · L", acquisition: "Solve and center.", stopCondition: "Window end.", viability: "viable" }] }))
  recorded15.close()
...
test("non-fixture origin, workers, and service databases migrate without M27 Plan, Library, or Process seeds", async (t) => {
  const root = mkdtempSync(join(tmpdir(), "astro-no-fixture-")); const allowedRoot = `${root}/`
  const origin = createRuntimeService(join(root, "origin.sqlite")); assertNoM27Fixture(origin.database); const listener = await origin.listen(); t.after(async () => { await listener.close(); origin.close() }); const base = `http://127.0.0.1:${listener.port}`
  const snapshot = await fetch(`${base}/api/snapshot`).then((response) => response.json()); const ready = await fetch(`${base}/api/health/ready`).then((response) => response.json())
  assert.equal(snapshot.plan.readiness, "unavailable"); assert.equal(snapshot.plan.id, "uninitialized"); assert.equal(ready.status, "unavailable"); const shell = await fetch(`${base}/`).then((response) => response.text()); assert.doesNotMatch(shell, /M27|Run plan/); assert.match(shell, /unavailable|No observation plan is installed/); const start = await fetch(`${base}/api/commands/start-run`, { method: "POST", body: JSON.stringify({ _tag: "StartRunFromPlan", planId: "uninitialized", expectedPlanRevision: 0, expectedLeaseRevision: 0, idempotencyKey: "uninitialized-start" }) }); assert.equal(start.status, 409); assert.equal((await start.json()).reason, "PlanUnavailable"); assert.equal((origin.database.prepare("SELECT count(*) AS count FROM events").get() as { count: number }).count, 0); await new Promise<void>((resolve) => setTimeout(resolve, 300)); assert.equal((await fetch(`${base}/api/snapshot`)).status, 200)
  const publisher = openPublisherDatabase(join(root, "publisher.sqlite"), allowedRoot); assertNoM27Fixture(publisher); publisher.close()
  const processor = openProcessorDatabase(join(root, "processor.sqlite"), allowedRoot); assertNoM27Fixture(processor); processor.close()

  const workerPath = join(root, "worker.sqlite"); const worker = createRigWorkerService({ mode: "seestar", databasePath: workerPath, rigId: "seestar-s30", host: "192.168.4.63", pemPath: "/run/secrets/seestar.pem" }, undefined); worker.close(); const workerDatabase = openMigrationDatabase(workerPath, allowedRoot); assertNoM27Fixture(workerDatabase); workerDatabase.close()

  const solarPath = join(root, "solar.sqlite"); const solar = createRuntimeService(solarPath); solar.database.prepare("INSERT INTO memberships VALUES (?,?,?)").run("solar-owner", "owner", "owner"); assertNoM27Fixture(solar.database); solar.close()
  const solarResult = runSolarTestIntentFromEnvironment({ ASTRO_LOCAL_WEB_DB: solarPath, ASTRO_SOLAR_TEST_CONFIRM: "submit-solar-test", ASTRO_SOLAR_TEST_SUBJECT: "solar-owner", ASTRO_SOLAR_TEST_NAME: "Solar fixture boundary", ASTRO_SOLAR_TEST_IDEMPOTENCY_KEY: "solar-no-fixture" })
  assert.equal(solarResult.outcome, "accepted"); const solarDatabase = openMigrationDatabase(solarPath, allowedRoot); assertNoM27Fixture(solarDatabase); solarDatabase.close()
})
...
test("current controller resumes only the paused revision and replays idempotently", async (t) => {
  ...
  const paused = await fetch(`${base}/api/commands/pause-run`, { method: "POST", body: JSON.stringify({ _tag: "PauseRun", expectedLeaseRevision: 1, expectedRunRevision: 1, idempotencyKey: "resume-pause" }) }).then((response) => response.json())
  assert.equal(paused.snapshot.run.phase, "paused")
  const command = { _tag: "ResumeRun", expectedLeaseRevision: 1, expectedRunRevision: 2, idempotencyKey: "resume-once" }
...
test("resume preserves accepted fixture capture without hardware dispatch", async (t) => {
  ...
  const resume = async (key: string) => {
    ...
    await fetch(`${base}/api/commands/pause-run`, { method: "POST", body: JSON.stringify({ _tag: "PauseRun", expectedLeaseRevision: 1, expectedRunRevision: 1, idempotencyKey: `${key}-pause` }) })
    return fetch(`${base}/api/commands/resume-run`, { method: "POST", body: JSON.stringify({ _tag: "ResumeRun", expectedLeaseRevision: 1, expectedRunRevision: 2, idempotencyKey: `${key}-resume` }) })
  }
...
test("a non-RunDefinition active run cannot use the bounded pause path", async (t) => {
  const service = createLocalWebService(); const listener = await service.listen(); const base = `http://127.0.0.1:${listener.port}`
  t.after(async () => { await listener.close(); service.close() })
  service.database.prepare("UPDATE state SET value=? WHERE key='run'").run(JSON.stringify({ id: "legacy-run", revision: 4, phase: "capture", target: "Legacy capture", progress: 50 }))
  const paused = await fetch(`${base}/api/commands/pause-run`, { method: "POST", body: JSON.stringify({ _tag: "PauseRun", expectedLeaseRevision: 1, expectedRunRevision: 4, idempotencyKey: "source-less-pause" }) })
  assert.equal(paused.status, 409); assert.equal((await paused.json()).reason, "RunRevisionConflict")
  assert.equal((await fetch(`${base}/api/snapshot`).then((response) => response.json())).run.phase, "capture")
  assert.equal((service.database.prepare("SELECT count(*) AS count FROM run_intervention_receipts").get() as { count: number }).count, 0)
})
...
test("Library detail uses stable identities and snapshot delivery remains catalog-bounded", async (t) => {
  ...
  assert.deepEqual(await malformedPath.json(), { outcome: "rejected", reason: "InvalidInput", message: "The service could not read that action." })
  ...
  assert.match(html, /Pause preserves this deterministic managed run; it does not send hardware work./)
  assert.match(html, /Resume restores the preserved deterministic phase; it does not send hardware work./)
...
test("authenticated workspace projections preserve future intent, bounded Library evidence, and a stable Process handoff", async (t) => {
  ...
  assert.equal(plan.planId, "plan-m27"); assert.equal(plan.readiness, "ready"); assert.equal(plan.sequences.length, 2); assert.equal(plan.sequences[0].capture, "24 × 180s · L"); assert.equal(plan.sequences[0].window.horizonClearanceDeg, 28); assert.equal(new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(new Date(plan.sequences[0].window.startsAt)), "23:18"); assert.equal(new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(new Date(plan.sequences[0].window.endsAt)), "01:02")
...
test("SQLite-backed plan drafts persist deterministic verdicts, revision guards, idempotency, and SSE projection", async (t) => {
  const databasePath = join(mkdtempSync(join(tmpdir(), "astro-plan-draft-")), "state.sqlite")
  const service = createLocalWebService(databasePath); const listener = await service.listen(); const base = `http://127.0.0.1:${listener.port}`
  t.after(async () => { await listener.close(); service.close() })
  const stream = await fetch(`${base}/api/events`); const reader = stream.body?.getReader(); await reader?.read()
  const initial = await fetch(`${base}/api/workspaces/plan`).then((response) => response.json())
  const sequences = initial.sequences.map(({ viability, ...sequence }: { readonly viability: string }) => sequence)
  const command = { planId: initial.planId, expectedPlanRevision: initial.revision, idempotencyKey: "plan-ready-001", sequences }
  const duplicate = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ ...command, idempotencyKey: "plan-duplicate-001", sequences: [sequences[0], sequences[0]] }) })
  assert.equal(duplicate.status, 400)
  const ready = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify(command) })
  assert.equal(ready.status, 202); assert.equal((await ready.json()).plan.readiness, "ready")
  const savedSnapshot = await fetch(`${base}/api/snapshot`).then((response) => response.json())
  assert.equal(savedSnapshot.plan.runEligible, false)
  const startSavedDraft = await fetch(`${base}/api/commands/start-run`, { method: "POST", body: JSON.stringify({ _tag: "StartRunFromPlan", planId: initial.planId, expectedPlanRevision: initial.revision + 1, expectedLeaseRevision: savedSnapshot.control.revision, idempotencyKey: "saved-draft-start" }) })
  assert.equal(startSavedDraft.status, 409); assert.equal((await startSavedDraft.json()).reason, "PlanUnavailable")
  const event = new TextDecoder().decode((await reader?.read()).value); assert.match(event, /event: PlanDraftSaved/); assert.match(event, /data: \{"snapshotVersion"/)
  const replay = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify(command) })
  assert.equal(replay.status, 200); assert.equal((service.database.prepare("SELECT count(*) AS count FROM events WHERE type='PlanDraftSaved'").get() as { count: number }).count, 1)
  const limited = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ ...command, expectedPlanRevision: initial.revision + 1, idempotencyKey: "plan-limited-001", sequences: sequences.map((sequence: { readonly horizon: string }, index: number) => index === 0 ? { ...sequence, horizon: "limited" } : sequence) }) })
  assert.equal(limited.status, 202); assert.equal((await limited.json()).plan.readiness, "readyWithLimitations")
  const blocked = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ ...command, expectedPlanRevision: initial.revision + 2, idempotencyKey: "plan-blocked-001", sequences: sequences.map((sequence: { readonly storage: string }, index: number) => index === 1 ? { ...sequence, storage: "missing" } : sequence) }) })
  assert.equal(blocked.status, 202); assert.equal((await blocked.json()).plan.readiness, "blocked")
  const stale = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ ...command, expectedPlanRevision: initial.revision + 2, idempotencyKey: "plan-stale-001" }) })
  assert.equal(stale.status, 409)
  await reader?.cancel(); await listener.close(); service.close()
  const recovered = createLocalWebService(databasePath); const recoveredListener = await recovered.listen(); const recoveredPlan = await fetch(`http://127.0.0.1:${recoveredListener.port}/api/workspaces/plan`).then((response) => response.json())
  assert.equal(recoveredPlan.readiness, "blocked"); assert.equal(recoveredPlan.revision, initial.revision + 3)
  const phone = createLocalWebService(databasePath, () => ({ personId: "owner-chicks", clientId: "phone-monitor", capability: "readOnly", role: "owner" })); const phoneListener = await phone.listen()
  assert.equal((await fetch(`http://127.0.0.1:${phoneListener.port}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ ...command, expectedPlanRevision: recoveredPlan.revision, idempotencyKey: "plan-phone-001" }) })).status, 403)
  await recoveredListener.close(); recovered.close(); await phoneListener.close(); phone.close()
})

test("SQLite accepts one immutable RunDefinition from a ready persisted plan and emits no device work", async (t) => {
  const databasePath = join(mkdtempSync(join(tmpdir(), "astro-run-definition-")), "state.sqlite")
  const service = createLocalWebService(databasePath); const listener = await service.listen(); const base = `http://127.0.0.1:${listener.port}`
  t.after(async () => { await listener.close(); service.close() })
  const initial = await fetch(`${base}/api/workspaces/plan`).then((response) => response.json())
  const sequences = initial.sequences.map(({ viability, ...sequence }: { readonly viability: string }) => sequence)
  const saved = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: initial.planId, expectedPlanRevision: initial.revision, idempotencyKey: "run-definition-draft-001", sequences }) })
  assert.equal(saved.status, 202)
  const draft = await saved.json()
  const stream = await fetch(`${base}/api/events`); const reader = stream.body?.getReader(); await reader?.read()
  const command = { _tag: "AcceptRunDefinition", planId: draft.plan.planId, expectedPlanRevision: draft.plan.revision, expectedLeaseRevision: draft.snapshot.control.revision, idempotencyKey: "run-definition-001" }
  const malformed = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ _tag: "AcceptRunDefinition" }) })
  assert.equal(malformed.status, 400); assert.equal((await malformed.json()).reason, "InvalidInput")
  const staleLease = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ ...command, expectedLeaseRevision: command.expectedLeaseRevision + 1, idempotencyKey: "run-definition-stale-lease" }) })
  assert.equal(staleLease.status, 409); assert.equal((await staleLease.json()).reason, "FreshnessConflict")
  const phone = createLocalWebService(databasePath, () => ({ personId: "owner-chicks", clientId: "phone-monitor", capability: "readOnly", role: "owner" })); const phoneListener = await phone.listen(); const phoneResult = await fetch(`http://127.0.0.1:${phoneListener.port}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify(command) })
  assert.equal(phoneResult.status, 403); assert.equal((await phoneResult.json()).reason, "ClientReadOnly"); await phoneListener.close(); phone.close()
  const unavailable = createRuntimeService(":memory:"); const unavailableListener = await unavailable.listen(); const unavailableResult = await fetch(`http://127.0.0.1:${unavailableListener.port}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ _tag: "AcceptRunDefinition", planId: "uninitialized", expectedPlanRevision: 0, expectedLeaseRevision: 0, idempotencyKey: "run-definition-unavailable" }) })
  assert.equal(unavailableResult.status, 409); assert.equal((await unavailableResult.json()).reason, "PlanUnavailable"); await unavailableListener.close(); unavailable.close()
  const limitedPath = join(mkdtempSync(join(tmpdir(), "astro-run-definition-limited-")), "state.sqlite"); const limitedService = createLocalWebService(limitedPath); const limitedListener = await limitedService.listen(); const limitedBase = `http://127.0.0.1:${limitedListener.port}`; const limitedInitial = await fetch(`${limitedBase}/api/workspaces/plan`).then((response) => response.json()); const limitedSequences = limitedInitial.sequences.map(({ viability, ...sequence }: { readonly viability: string }, index: number) => index === 0 ? { ...sequence, horizon: "limited" } : sequence)
  const limitedDraft = await fetch(`${limitedBase}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: limitedInitial.planId, expectedPlanRevision: limitedInitial.revision, idempotencyKey: "run-definition-limited-draft", sequences: limitedSequences }) }).then((response) => response.json())
  const limitedResult = await fetch(`${limitedBase}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ _tag: "AcceptRunDefinition", planId: limitedDraft.plan.planId, expectedPlanRevision: limitedDraft.plan.revision, expectedLeaseRevision: limitedDraft.snapshot.control.revision, idempotencyKey: "run-definition-limited" }) })
  assert.equal(limitedResult.status, 409); assert.equal((await limitedResult.json()).reason, "PlanNotReady"); await limitedListener.close(); limitedService.close()
  const activePath = join(mkdtempSync(join(tmpdir(), "astro-run-definition-active-")), "state.sqlite"); const activeService = createLocalWebService(activePath); const activeListener = await activeService.listen(); const activeBase = `http://127.0.0.1:${activeListener.port}`; const activeInitial = await fetch(`${activeBase}/api/workspaces/plan`).then((response) => response.json()); const activeSequences = activeInitial.sequences.map(({ viability, ...sequence }: { readonly viability: string }) => sequence)
  const activeDraft = await fetch(`${activeBase}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: activeInitial.planId, expectedPlanRevision: activeInitial.revision, idempotencyKey: "run-definition-active-draft", sequences: activeSequences }) }).then((response) => response.json())
  const activeCommand = { _tag: "AcceptRunDefinition", planId: activeDraft.plan.planId, expectedPlanRevision: activeDraft.plan.revision, expectedLeaseRevision: activeDraft.snapshot.control.revision, idempotencyKey: "run-definition-active-accept" }
  assert.equal((await fetch(`${activeBase}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify(activeCommand) })).status, 202)
  assert.equal((await fetch(`${activeBase}/api/commands/start-run`, { method: "POST", body: JSON.stringify({ _tag: "StartRunFromPlan", planId: activeCommand.planId, expectedPlanRevision: activeCommand.expectedPlanRevision, expectedLeaseRevision: activeCommand.expectedLeaseRevision, idempotencyKey: "run-definition-active-run" }) })).status, 202)
  const activeResult = await fetch(`${activeBase}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ ...activeCommand, idempotencyKey: "run-definition-active" }) })
  assert.equal(activeResult.status, 409); assert.equal((await activeResult.json()).reason, "ActiveRunConflict"); await activeListener.close(); activeService.close()
  const accepted = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify(command) })
  assert.equal(accepted.status, 202)
  const result = await accepted.json()
  assert.equal(result.runDefinition.executor, "fake"); assert.equal(result.runDefinition.sourcePlanRevision, draft.plan.revision); assert.equal(result.snapshot.plan.runEligible, true)
  const event = new TextDecoder().decode((await reader?.read()).value); assert.match(event, /event: RunDefinitionAccepted/)
  const replay = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify(command) })
  assert.equal(replay.status, 200); assert.equal((await replay.json()).runDefinition.id, result.runDefinition.id)
  const changedReplay = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ ...command, expectedLeaseRevision: command.expectedLeaseRevision + 1 }) })
  assert.equal(changedReplay.status, 400); assert.equal((await changedReplay.json()).reason, "InvalidInput")
  const second = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ ...command, idempotencyKey: "run-definition-002" }) })
  assert.equal(second.status, 409); assert.equal((await second.json()).reason, "RunDefinitionAlreadyAccepted")
  const stale = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ ...command, expectedPlanRevision: draft.plan.revision - 1, idempotencyKey: "run-definition-stale" }) })
  assert.equal(stale.status, 409); assert.equal((await stale.json()).reason, "FreshnessConflict")
  const next = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: draft.plan.planId, expectedPlanRevision: draft.plan.revision, idempotencyKey: "run-definition-next-draft", sequences: sequences.map((sequence: { readonly capture: string }, index: number) => index === 0 ? { ...sequence, capture: "48 × 180s · L" } : sequence) }) })
  assert.equal(next.status, 202)
  const nextPlan = (await next.json()).plan
  const blocked = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: nextPlan.planId, expectedPlanRevision: nextPlan.revision, idempotencyKey: "run-definition-blocked-draft", sequences: sequences.map((sequence: { readonly storage: string }, index: number) => index === 0 ? { ...sequence, storage: "missing" } : sequence) }) })
  const blockedPlan = (await blocked.json()).plan
  const notReady = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ _tag: "AcceptRunDefinition", planId: blockedPlan.planId, expectedPlanRevision: blockedPlan.revision, expectedLeaseRevision: draft.snapshot.control.revision, idempotencyKey: "run-definition-blocked" }) })
  assert.equal(notReady.status, 409); assert.equal((await notReady.json()).reason, "PlanNotReady")
  const definition = JSON.parse((service.database.prepare("SELECT definition FROM run_definitions WHERE run_definition_id=?").get(result.runDefinition.id) as { definition: string }).definition)
  assert.equal(definition.plan.sequences[0].capture, sequences[0].capture)
  assert.equal((service.database.prepare("SELECT count(*) AS count FROM events WHERE type='RunDefinitionAccepted'").get() as { count: number }).count, 1)
  assert.equal((service.database.prepare("SELECT count(*) AS count FROM outbox").get() as { count: number }).count, 0)
  assert.equal((await fetch(`${base}/api/snapshot`).then((response) => response.json())).run, null)
  await reader?.cancel(); await listener.close(); service.close()
  const recovered = createLocalWebService(databasePath); const recoveredDefinition = recovered.database.prepare("SELECT source_plan_revision,definition FROM run_definitions WHERE run_definition_id=?").get(result.runDefinition.id) as { source_plan_revision: number; definition: string }
  assert.equal(recoveredDefinition.source_plan_revision, draft.plan.revision); assert.equal(JSON.parse(recoveredDefinition.definition).id, result.runDefinition.id)
  recovered.close()
})

test("an accepted fake RunDefinition advances two immutable sequences through durable service-owned phases", async (t) => {
  const databasePath = join(mkdtempSync(join(tmpdir(), "astro-fake-run-")), "state.sqlite")
  const service = createLocalWebService(databasePath); const listener = await service.listen(); const base = `http://127.0.0.1:${listener.port}`
  const initial = await fetch(`${base}/api/workspaces/plan`).then((response) => response.json()); const sequences = initial.sequences.map(({ viability, ...sequence }: { readonly viability: string }) => sequence)
  const draft = await fetch(`${base}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: initial.planId, expectedPlanRevision: initial.revision, idempotencyKey: "fake-run-draft", sequences }) }).then((response) => response.json())
  const accepted = await fetch(`${base}/api/commands/accept-run-definition`, { method: "POST", body: JSON.stringify({ _tag: "AcceptRunDefinition", planId: draft.plan.planId, expectedPlanRevision: draft.plan.revision, expectedLeaseRevision: draft.snapshot.control.revision, idempotencyKey: "fake-run-definition" }) }).then((response) => response.json())
  const stream = await fetch(`${base}/api/events`); const reader = stream.body?.getReader(); await reader?.read()
  const start = { _tag: "StartRunFromPlan", planId: draft.plan.planId, expectedPlanRevision: draft.plan.revision, expectedLeaseRevision: draft.snapshot.control.revision, idempotencyKey: "fake-run-start" }
  const nonHolder = createLocalWebService(databasePath, () => ({ personId: "friend-ada", clientId: "desktop-ada", role: "viewer", capability: "controlCapable" })); const nonHolderListener = await nonHolder.listen(); const nonHolderStart = await fetch(`http://127.0.0.1:${nonHolderListener.port}/api/commands/start-run`, { method: "POST", body: JSON.stringify({ ...start, idempotencyKey: "fake-run-non-holder" }) })
  assert.equal(nonHolderStart.status, 403); assert.equal((await nonHolderStart.json()).reason, "ControlLeaseLost"); assert.equal((service.database.prepare("SELECT count(*) AS count FROM events WHERE type='RunStarted'").get() as { count: number }).count, 0); await nonHolderListener.close(); nonHolder.close()
  const started = await fetch(`${base}/api/commands/start-run`, { method: "POST", body: JSON.stringify(start) })
  assert.equal(started.status, 202); const startedRun = (await started.json()).run; assert.equal(startedRun.phase, "preflight"); assert.equal(startedRun.sourceDefinitionId, accepted.runDefinition.id); assert.equal(startedRun.target, accepted.runDefinition.plan.sequences[0].target)
  assert.match(new TextDecoder().decode((await reader?.read()).value), /event: RunStarted/)
  assert.equal((await fetch(`${base}/api/commands/start-run`, { method: "POST", body: JSON.stringify(start) })).status, 200)
  assert.equal((await fetch(`${base}/api/commands/start-run`, { method: "POST", body: JSON.stringify({ ...start, idempotencyKey: "fake-run-conflict" }) })).status, 409)
  assert.equal(service.advanceFakeRun()?.run.phase, "acquire"); assert.match(new TextDecoder().decode((await reader?.read()).value), /event: RunPreflightCompleted/)
  assert.equal(service.advanceFakeRun()?.run.phase, "capture"); assert.match(new TextDecoder().decode((await reader?.read()).value), /event: RunAcquireCompleted/)
  const beforePause = await fetch(`${base}/api/snapshot`).then((response) => response.json())
  const pause = { _tag: "PauseRun", expectedLeaseRevision: beforePause.control.revision, expectedRunRevision: beforePause.run.revision, idempotencyKey: "fake-run-pause" }
  const paused = await fetch(`${base}/api/commands/pause-run`, { method: "POST", body: JSON.stringify(pause) })
  assert.equal(paused.status, 202); const pausedBody = await paused.json(); assert.equal(pausedBody.snapshot.run.phase, "paused"); assert.equal(pausedBody.snapshot.run.resumablePhase, "capture")
  assert.match(new TextDecoder().decode((await reader?.read()).value), /event: RunPaused/)
  assert.equal((await fetch(`${base}/api/commands/pause-run`, { method: "POST", body: JSON.stringify(pause) })).status, 200)
  const idempotencyConflict = await fetch(`${base}/api/commands/resume-run`, { method: "POST", body: JSON.stringify({ ...pause, _tag: "ResumeRun" }) })
  assert.equal(idempotencyConflict.status, 409); assert.equal((await idempotencyConflict.json()).reason, "IdempotencyConflict")
  assert.equal(service.advanceFakeRun(), undefined)
  assert.equal((service.database.prepare("SELECT count(*) AS count FROM events WHERE type='RunPaused'").get() as { count: number }).count, 1)
  await reader?.cancel(); await listener.close(); service.close()
  const recovered = createLocalWebService(databasePath); const recoveredListener = await recovered.listen(); const recoveredBase = `http://127.0.0.1:${recoveredListener.port}`; const recoveredStream = await fetch(`${recoveredBase}/api/events`); const recoveredReader = recoveredStream.body?.getReader(); await recoveredReader?.read()
  const afterRestart = await fetch(`${recoveredBase}/api/snapshot`).then((response) => response.json()); assert.equal(afterRestart.run.phase, "paused"); assert.equal(afterRestart.run.resumablePhase, "capture"); assert.equal(afterRestart.run.activeSequenceIndex, 0); assert.equal(afterRestart.run.completedSequenceCount, 0); assert.equal(afterRestart.run.sourceDefinitionId, accepted.runDefinition.id)
  assert.equal(recovered.advanceFakeRun(), undefined)
  const notPaused = await fetch(`${recoveredBase}/api/commands/resume-run`, { method: "POST", body: JSON.stringify({ _tag: "ResumeRun", expectedLeaseRevision: afterRestart.control.revision, expectedRunRevision: afterRestart.run.revision - 1, idempotencyKey: "fake-run-stale-resume" }) })
  assert.equal(notPaused.status, 409); assert.equal((await notPaused.json()).reason, "RunRevisionConflict")
  const resumed = await fetch(`${recoveredBase}/api/commands/resume-run`, { method: "POST", body: JSON.stringify({ _tag: "ResumeRun", expectedLeaseRevision: afterRestart.control.revision, expectedRunRevision: afterRestart.run.revision, idempotencyKey: "fake-run-resume" }) })
  assert.equal(resumed.status, 202); assert.equal((await resumed.json()).snapshot.run.phase, "capture")
  assert.equal(recovered.advanceFakeRun()?.run.phase, "verify"); assert.equal(recovered.advanceFakeRun()?.run.phase, "preflight")
  const secondSequence = await fetch(`${recoveredBase}/api/snapshot`).then((response) => response.json()); assert.equal(secondSequence.run.activeSequenceIndex, 1); assert.equal(secondSequence.run.completedSequenceCount, 1)
  assert.equal(recovered.advanceFakeRun()?.run.phase, "acquire"); assert.equal(recovered.advanceFakeRun()?.run.phase, "capture"); assert.equal(recovered.advanceFakeRun()?.run.phase, "verify"); assert.equal(recovered.advanceFakeRun()?.run.phase, "completed")
  assert.match(new TextDecoder().decode((await recoveredReader?.read()).value), /event: RunResumed|event: RunCaptureCompleted/)
  const completed = await fetch(`${recoveredBase}/api/snapshot`).then((response) => response.json()); assert.equal(completed.run.phase, "completed"); assert.equal(completed.run.completedSequenceCount, 2); assert.equal(completed.run.revision, 11)
  assert.equal(recovered.advanceFakeRun(), undefined); assert.equal((recovered.database.prepare("SELECT count(*) AS count FROM outbox").get() as { count: number }).count, 0); assert.equal((recovered.database.prepare("SELECT count(*) AS count FROM events WHERE type='RunCompleted'").get() as { count: number }).count, 1)
  await recoveredReader?.cancel(); await recoveredListener.close(); recovered.close()
})

test("fixture installation restores only a missing M27 plan record and keeps a saved draft visible after restart", async (t) => {
  const missingPath = join(mkdtempSync(join(tmpdir(), "astro-missing-fixture-plan-")), "state.sqlite")
  const fixture = createLocalWebService(missingPath)
  fixture.database.prepare("DELETE FROM observing_plans WHERE plan_id='plan-m27'").run(); fixture.close()
  const restored = createLocalWebService(missingPath); const restoredListener = await restored.listen(); const restoredBase = `http://127.0.0.1:${restoredListener.port}`
  assert.equal((await fetch(`${restoredBase}/api/snapshot`).then((response) => response.json())).plan.id, "plan-m27")
  const restoredShell = await fetch(`${restoredBase}/`).then((response) => response.text()); assert.match(restoredShell, /data-room="Plan"/); assert.match(restoredShell, /Save deterministic two-sequence draft/)
  await restoredListener.close(); restored.close()

  const savedPath = join(mkdtempSync(join(tmpdir(), "astro-saved-draft-restart-")), "state.sqlite")
  const saved = createLocalWebService(savedPath); const savedListener = await saved.listen(); const savedBase = `http://127.0.0.1:${savedListener.port}`
  const initial = await fetch(`${savedBase}/api/workspaces/plan`).then((response) => response.json()); const sequences = initial.sequences.map(({ viability, ...sequence }: { readonly viability: string }) => sequence)
  assert.equal((await fetch(`${savedBase}/api/commands/save-plan-draft`, { method: "POST", body: JSON.stringify({ planId: initial.planId, expectedPlanRevision: initial.revision, idempotencyKey: "saved-restart-001", sequences }) })).status, 202)
  await savedListener.close(); saved.close()
  const recovered = createLocalWebService(savedPath); const recoveredListener = await recovered.listen(); const recoveredBase = `http://127.0.0.1:${recoveredListener.port}`
  t.after(async () => { await recoveredListener.close(); recovered.close() })
  const snapshot = await fetch(`${recoveredBase}/api/snapshot`).then((response) => response.json()); assert.equal(snapshot.plan.readiness, "ready"); assert.equal(snapshot.plan.runEligible, false)
  const shell = await fetch(`${recoveredBase}/`).then((response) => response.text()); assert.match(shell, /data-room="Plan"/); assert.doesNotMatch(shell, /No observation plan is installed/)
})
...
test("a request query cannot select phone or controller capability", async (t) => {
  ...
  assert.match(html, /if\(label==='Run plan'\)return document\.createComment\('Run plan deferred'\)/)
  assert.match(html, /function eventProjection\(event\)\{const payload=JSON\.parse\(event\.data\);return payload\.snapshot\|\|payload\}/)
  assert.match(html, /if\(innerWidth<=600\)return/)
  assert.match(html, /addEventListener\('resize',\(\)=>\{if\(projection\)\{render\(projection\);renderPlanActions\(\)\}\}\)/)
  assert.match(html, /addEventListener\('orientationchange',\(\)=>\{if\(projection\)\{render\(projection\);renderPlanActions\(\)\}\}\)/)
...
test("two server-configured desktops transfer control without stopping the accepted run", async (t) => {
  ...
  const oldController = await fetch(`${ownerBase}/api/commands/pause-run`, { method: "POST", body: JSON.stringify({ _tag: "PauseRun", expectedLeaseRevision: 2, expectedRunRevision: 1, idempotencyKey: "old-pause" }) })
