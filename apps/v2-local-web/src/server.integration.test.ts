...
test("R2 publisher configuration and signed fake transport fail closed without network", async () => {
  ...
  const downloadIssuer = createR2DownloadGrantIssuer({ bucket: env.R2_BUCKET, endpoint: env.R2_ENDPOINT, credentialsPath, now: () => new Date("2026-07-28T12:00:00.000Z") }); const signed = new URL(await downloadIssuer.issue({ objectKey: "published/run/finals/asset.tiff", expiresAt: "2026-07-28T12:05:00.000Z" })); assert.equal(signed.pathname, "/astro-console-artifacts/published/run/finals/asset.tiff"); assert.equal(signed.searchParams.get("X-Amz-Algorithm"), "AWS4-HMAC-SHA256"); assert.equal(signed.searchParams.get("X-Amz-Expires"), "300"); assert.equal(signed.searchParams.get("response-content-disposition"), "attachment; filename=\"asset.tiff\""); assert.match(signed.search, /response-content-disposition=attachment%3B%20filename%3D%22asset\.tiff%22/); assert.equal(signed.search.includes("+"), false); assert.match(signed.searchParams.get("X-Amz-Signature") ?? "", /^[0-9a-f]{64}$/); assert.equal(signed.toString().includes("secret"), false)
