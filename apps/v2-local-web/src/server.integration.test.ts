...
test("numbered SQLite migrations upgrade a legacy database and reject a newer schema", () => {
  ...
  assert.equal((fresh.database.prepare("SELECT max(version) AS version FROM schema_migrations").get() as { version: number }).version, 18); assert.equal((fresh.database.prepare("SELECT count(*) AS count FROM pragma_table_info('observing_plans') WHERE name='run_eligible'").get() as { count: number }).count, 1)
  fresh.close()
  ...
  assert.equal(repairedPlan.sequences.length, 2); assert.deepEqual(repairedPlan.limitations, []); assert.equal((repaired.database.prepare("SELECT max(version) AS version FROM schema_migrations").get() as { version: number }).version, 18)
  repaired.close()
...
test("operational endpoints expose bounded admitted health without internal detail", async (t) => {
  ...
  assert.equal(operations.release, "local-web-fixture"); assert.equal(operations.schemaVersion, 18); assert.equal(operations.sqlite.journalMode, "wal"); assert.equal(operations.disk, "unknown"); assert.equal(operations.rig, "unknown"); assert.equal(JSON.stringify(operations).includes("/"), false)
...
test("an accepted fake RunDefinition advances two immutable sequences through durable service-owned phases", async (t) => {
  ...
  assert.equal(service.advanceFakeRun()?.run.phase, "verify"); assert.match(new TextDecoder().decode((await reader?.read()).value), /event: RunCaptureCompleted/)
  assert.equal(service.advanceFakeRun()?.run.phase, "preflight")
  await reader?.cancel(); await listener.close(); service.close()
  ...
  const afterRestart = await fetch(`${recoveredBase}/api/snapshot`).then((response) => response.json()); assert.equal(afterRestart.run.phase, "preflight"); assert.equal(afterRestart.run.activeSequenceIndex, 1); assert.equal(afterRestart.run.completedSequenceCount, 1); assert.equal(afterRestart.run.sourceDefinitionId, accepted.runDefinition.id)
  ...
  assert.match(new TextDecoder().decode((await recoveredReader?.read()).value), /event: RunAcquireCompleted|event: RunPreflightCompleted/)
  const completed = await fetch(`${recoveredBase}/api/snapshot`).then((response) => response.json()); assert.equal(completed.run.phase, "completed"); assert.equal(completed.run.completedSequenceCount, 2); assert.equal(completed.run.revision, 9)
