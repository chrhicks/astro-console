...
test("manifest processor is disabled by default and only saves bounded configured source outputs", () => {
  ...
  assert.throws(() => processorConfig({ ASTRO_PROCESSOR_MODE: "manifest", ASTRO_LOCAL_WEB_DB: "/tmp/state.sqlite", ASTRO_PROCESSOR_SOURCES_ROOT: "/var/lib/astro-console/sources", ASTRO_PROCESSOR_OUTPUTS_ROOT: "/var/lib/astro-console/outputs", ASTRO_PROCESSOR_MANIFEST_PATH: "/run/config/manifest.json", ASTRO_PROCESSOR_OWNER_PERSON_ID: "owner" }), /app-owned/); assert.throws(() => processorConfig({ ASTRO_PROCESSOR_MODE: "manifest", ASTRO_LOCAL_WEB_DB: "/var/lib/astro-console/state.sqlite", ASTRO_PROCESSOR_SOURCES_ROOT: "/var/lib/astro-console/sources", ASTRO_PROCESSOR_OUTPUTS_ROOT: "/var/lib/astro-console/outputs", ASTRO_PROCESSOR_MANIFEST_PATH: "/run/config/manifest.json" }), /owner person ID/); assert.throws(() => processorConfig({ ASTRO_PROCESSOR_MODE: "wrong" }), /disabled or manifest/)
  const root = mkdtempSync(join(tmpdir(), "astro-processor-")); const sources = join(root, "sources"); const outputs = join(root, "outputs"); const databasePath = join(root, "state.sqlite"); const manifestPath = join(root, "manifest.json"); mkdirSync(sources); writeFileSync(join(sources, "final.tiff"), "processor-bytes")
  const config = { mode: "manifest" as const, databasePath, sourcesRoot: sources, outputsRoot: outputs, manifestPath, ownerPersonId: "owner" }; const manifest = { sessionId: "process-m27-001", expectedRevision: 4, idempotencyKey: "processor-save", outputs: [{ sourceId: "final", representation: "final" }], sources: { final: "final.tiff" } }; writeFileSync(manifestPath, JSON.stringify(manifest))
