...
test("numbered SQLite migrations upgrade a legacy database and reject a newer schema", () => {
  ...
  assert.equal((fresh.database.prepare("SELECT max(version) AS version FROM schema_migrations").get() as { version: number }).version, 16); assert.equal((fresh.database.prepare("SELECT count(*) AS count FROM pragma_table_info('observing_plans') WHERE name='run_eligible'").get() as { count: number }).count, 1)
  fresh.close()
  ...
  assert.equal(repairedPlan.sequences.length, 2); assert.deepEqual(repairedPlan.limitations, []); assert.equal((repaired.database.prepare("SELECT max(version) AS version FROM schema_migrations").get() as { version: number }).version, 16)
  repaired.close()
...
test("operational endpoints expose bounded admitted health without internal detail", async (t) => {
  ...
  assert.equal(operations.release, "local-web-fixture"); assert.equal(operations.schemaVersion, 16); assert.equal(operations.sqlite.journalMode, "wal"); assert.equal(operations.disk, "unknown"); assert.equal(operations.rig, "unknown"); assert.equal(JSON.stringify(operations).includes("/"), false)
