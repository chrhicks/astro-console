...
test("operational endpoints expose bounded admitted health without internal detail", async (t) => {
  ...
  assert.equal(operations.release, "local-web-fixture"); assert.equal(operations.schemaVersion, 11); assert.equal(operations.sqlite.journalMode, "wal"); assert.equal(operations.rig, "unknown"); assert.equal(JSON.stringify(operations).includes("/"), false)
