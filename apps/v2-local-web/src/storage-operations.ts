...
const CleanupRow = Schema.Struct({ path: Schema.String, kind: Schema.Literals(["orphan", "scratch"]) })
...
export type StorageOperationsConfig = { readonly scratchRoot: string; readonly outputsRoot: string; readonly thresholds: StorageThresholds; readonly probe?: StorageProbe; readonly cleanupBatchSize?: number }
...
function removeRecord(database: DatabaseSync, row: typeof CleanupRow.Type) { database.prepare(row.kind === "orphan" ? "DELETE FROM process_save_orphans WHERE path=?" : "DELETE FROM storage_scratch_entries WHERE path=?").run(row.path) }
