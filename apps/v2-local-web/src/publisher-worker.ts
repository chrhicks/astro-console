...
const Asset = Schema.Struct({ asset_id: Schema.String, role: Schema.Literals(["linearMaster", "final"]), format: Schema.Literals(["fits", "tiff"]), detail: Schema.String })
...
export function createPublisherWorker(database: DatabaseSync, storage: Pick<ProcessSaveStorage, "outputsRoot">, provider: PublisherProvider) {
  const pass = async (workerId = "publisher-worker") => {
    ...
    try {
      database.prepare("UPDATE outbox SET state='failed',claim_token=NULL,claimed_by=NULL,claim_until=NULL,last_error='publisher lease expired',retry_after=? WHERE kind='PublishAsset' AND state='claimed' AND claim_until<=?").run(now, now)
      const raw: unknown = database.prepare("SELECT id,payload FROM outbox WHERE kind='PublishAsset' AND state IN ('pending','failed') AND (retry_after IS NULL OR retry_after<=?) ORDER BY rowid LIMIT 1").get(now)
      const work = Schema.decodeUnknownSync(Schema.optional(ClaimedWork))(raw)
      if (work === undefined) { database.exec("COMMIT"); return "none" as const }
      const claimed = database.prepare("UPDATE outbox SET state='claimed',claim_token=?,claimed_by=?,claim_until=?,attempts=attempts+1 WHERE id=? AND state IN ('pending','failed')").run(token, workerId, new Date(Date.now() + 30_000).toISOString(), work.id)
  ...
  const publish = async (claimed: typeof ClaimedWork.Type, token: string) => {
    ...
    try {
      ...
      const raw: unknown = database.prepare("SELECT asset_id,role,format,detail FROM library_assets WHERE asset_id=?").get(work.assetId)
      ...
      const key = publication?.object_key ? publication.object_key : publicationKey(asset, detail.lineage.runId, work.checksum)
      database.exec("BEGIN IMMEDIATE")
  ...
  const settlePublished = (outboxId: string, token: string, asset: typeof Asset.Type, checksum: string) => {
    database.exec("BEGIN IMMEDIATE")
    try {
      const acknowledged = database.prepare("UPDATE outbox SET state='dispatched',ack_at=?,claim_token=NULL,claimed_by=NULL,claim_until=NULL,last_error=NULL WHERE id=? AND state='claimed' AND claim_token=?").run(new Date().toISOString(), outboxId, token)
      if (acknowledged.changes !== 1) { database.exec("COMMIT"); return "superseded" as const }
      database.prepare("UPDATE asset_publications SET state='published',updated_at=? WHERE asset_id=? AND checksum=?").run(new Date().toISOString(), asset.asset_id, checksum)
      projectAvailability(asset.asset_id, "published")
      database.exec("COMMIT"); return "published" as const
    } catch (error) { database.exec("ROLLBACK"); throw error }
  }
  const settleFailure = (outboxId: string, token: string, assetId: string | undefined, state: typeof Publication.Type["state"], message: string) => {
    database.exec("BEGIN IMMEDIATE")
    try {
      const failed = database.prepare("UPDATE outbox SET state='failed',claim_token=NULL,claimed_by=NULL,claim_until=NULL,last_error=?,retry_after=? WHERE id=? AND state='claimed' AND claim_token=?").run(message, new Date().toISOString(), outboxId, token)
      if (failed.changes !== 1) { database.exec("COMMIT"); return "superseded" as const }
      if (assetId !== undefined) { const checksumRaw: unknown = database.prepare("SELECT checksum,object_key,state FROM asset_publications WHERE asset_id=?").get(assetId); const publication = Schema.decodeUnknownSync(Schema.optional(Publication))(checksumRaw); if (publication !== undefined) database.prepare("UPDATE asset_publications SET state=?,updated_at=? WHERE asset_id=?").run(state, new Date().toISOString(), assetId); projectAvailability(assetId, state) }
      database.exec("COMMIT"); return "failed" as const
    } catch (error) { database.exec("ROLLBACK"); throw error }
  }
  const projectAvailability = (assetId: string, availability: "published" | "temporarilyUnavailable" | "failedPublication") => {
    const raw: unknown = database.prepare("SELECT detail FROM library_assets WHERE asset_id=?").get(assetId)
    const row = Schema.decodeUnknownSync(Schema.Struct({ detail: Schema.String }))(raw)
    const detail = Schema.decodeUnknownSync(Schema.Struct({ assetId: Schema.String, revision: Schema.Int, role: Schema.String, format: Schema.String, availability: Schema.String, capturedAt: Schema.String, comparisonGroupId: Schema.String, lineage: Schema.Unknown, representations: Schema.Array(Schema.Struct({ label: Schema.String, state: Schema.String })) }))(JSON.parse(row.detail))
    database.prepare("UPDATE library_assets SET availability=?,detail=?,updated_at=? WHERE asset_id=?").run(availability, JSON.stringify({ ...detail, availability, representations: [{ label: availability === "published" ? "Published representation verified" : "Published representation temporarily unavailable", state: availability }] }), new Date().toISOString(), assetId)
  }
  return { pass }
}
