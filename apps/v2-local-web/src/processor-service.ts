...
const Manifest = Schema.Struct({ sessionId: Schema.NonEmptyString, expectedRevision: Schema.Int.check(Schema.isGreaterThanOrEqualTo(0)), idempotencyKey: Schema.NonEmptyString.check(Schema.isMaxLength(128)), outputs: Schema.Array(Schema.Struct({ sourceId: Schema.NonEmptyString, representation: Schema.Literals(["linearMaster", "final"]) })), sources: Schema.Unknown, metadata: Schema.Struct({ comparisonGroupId: Schema.NonEmptyString, sourceAssetIds: Schema.Array(Schema.String), runId: Schema.NonEmptyString, solveAttemptId: Schema.NonEmptyString }), sourceIngest: Schema.optionalKey(Schema.Unknown) })
const Membership = Schema.Struct({ person_id: Schema.String, role: Schema.Literal("owner") })
type ProcessorResult = ProcessSaveResult | SourceIngestResult | { readonly outcome: "disabled" } | { readonly outcome: "rejected"; readonly reason: "ManifestUnavailable" | "InvalidManifest" | "OwnerUnavailable" }
...
export function createProcessorService(config: ProcessorConfig, options: { readonly databaseOpener?: (path: string) => ReturnType<typeof openProcessorDatabase> } = {}) {
  ...
  const runOnce = (): ProcessorResult => {
    ...
    if (raw.sourceIngest !== undefined) { const ingested = ingestSourceAsset(database, { sourcesRoot: config.sourcesRoot, originalsRoot: config.originalsRoot, sources }, raw.sourceIngest, { personId: config.ownerPersonId, clientId: "processor-manifest", role: "owner", capability: "controlCapable" }); if (ingested.outcome !== "accepted") return ingested }
