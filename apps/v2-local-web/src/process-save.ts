...
const SaveInput = Schema.Struct({ sessionId: Schema.Literal("process-m27-001"), expectedRevision: Schema.Literal(4), idempotencyKey: Schema.NonEmptyString.check(Schema.isMaxLength(128)), outputs: Schema.Array(Schema.Struct({ sourceId: Schema.NonEmptyString, representation: Schema.Literals(["linearMaster", "final"]) })) })
...
export function saveProcessOutputs(database: DatabaseSync, storage: ProcessSaveStorage, raw: unknown, identity: LocalIdentity): ProcessSaveResult {
  ...
  const semanticKey = createHash("sha256").update(JSON.stringify({ version: 1, sessionId: input.sessionId, expectedRevision: input.expectedRevision, outputs: input.outputs, personId: identity.personId })).digest("hex")
  ...
  try {
    ...
    for (const output of staged) {
      const detail = { assetId: output.assetId, revision: 1, role: output.representation, format: output.representation === "final" ? "tiff" : "fits", availability: "availableLocally", capturedAt: new Date().toISOString(), comparisonGroupId: "m27-stack-1", lineage: { sourceAssetIds: ["asset-m27-001"], runId: "run-m27-001", solveAttemptId: "solve-m27-001" }, representations: [{ label: "Permanent local output retained", state: "available" }] }
