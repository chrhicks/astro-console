...
- Define a `Schema.Struct` for the expected shape and decode with
  `Schema.decodeUnknownResult` (or `Schema.decodeUnknownEffect` when decoding
  belongs in a generator).
- Handle the failure branch explicitly. Do not let parse errors escape as
  unchecked runtime values, and do not cast the input to a typed shape without
  decoding it first.
