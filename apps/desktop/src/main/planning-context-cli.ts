...
async function main(): Promise<void> {
  ...
  const windowStepMinutes = parseNumberOption("--window-step-minutes", args.windowStepMinutes);
  const siteLat = parseNumberOption("--site-lat", args.siteLat);
  const siteLon = parseNumberOption("--site-lon", args.siteLon);
  const siteMinAltitudeDeg = parseNumberOption("--site-min-altitude-deg", args.siteMinAltitudeDeg);
  const blockedAzimuthRanges = parseBlockedAzimuthRanges(args.siteBlockedRange ?? []);
    ...
    siteOverride:
      ...
      blockedAzimuthRanges.length > 0
        ? {
            ...
            blockedAzimuthRanges: blockedAzimuthRanges.length > 0 ? blockedAzimuthRanges : undefined,
          }
        : undefined,
  };
      ...
      JSON.stringify(
        {
          ...
          selected: selection.selected
            ...
            : undefined,
          candidates: selection.candidates.map((candidate) => formatCandidate(candidate)),
        },
        null,
        2
      )
...
function resolveStateFile(rawStateFile: string | undefined): string {
  ...
  const stateFile = resolve(fromArg || fromEnv || "planning/planning-state.json");
  if (!existsSync(stateFile)) {
    throw new Error(`Planning state file not found at ${stateFile}. Use --state-file <path>.`);
  }
  return stateFile;
}
...
function parseRecommendation(
  ...
  if (value === "any" || value === "good_now" || value === "later_tonight" || value === "not_tonight") {
    return value;
  }
  throw new Error(`Invalid --recommendation ${value}. Use good_now, later_tonight, not_tonight, or any.`);
}

function parseNumberOption(label: string, value: string | undefined): number | undefined {
...
function parseBlockedAzimuthRanges(values: string[]): SiteAzimuthRange[] {
  return values.map((value, index) => {
    ...
    if (!match) {
      throw new Error(`Invalid --site-blocked-range at position ${index + 1}: ${value}`);
    }
...
function printTextOutput(input: {
  ...
  recommendation: string;
  activeSite: { name: string; id: string; timezone: string; minAltitudeDeg: number };
