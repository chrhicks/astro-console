...
export class PlanningStore {
  ...
  async createSiteProfile(input: {
    ...
    return this.updateState((state) => {
      ...
      return {
        ...
        activeSiteId:
          input.makeActive || !hasSelectableSite(state)
            ? nextSite.id
            : state.activeSiteId,
      }
    })
  ...
  async updateSiteProfile(input: {
    ...
    return this.updateState((state) => {
      ...
      if (currentSite.archivedAt) {
        throw new Error(
          'Archived sites cannot be edited until they are restored',
        )
      }
      ...
      return {
        ...
        sites: state.sites.map((site) =>
          site.id === input.siteId ? nextSite : site,
        ),
      }
    })
  }

  async duplicateSiteProfile(input: {
    siteId: string
    makeActive?: boolean
  }): Promise<PlanningSnapshot> {
    return this.updateState((state) => {
  ...
  async archiveSiteProfile(input: {
    siteId: string
  }): Promise<PlanningSnapshot> {
    return this.updateState((state) => {
      const currentSite = state.sites.find((site) => site.id === input.siteId)
      ...
      return {
        ...
        sites,
        activeSiteId:
          state.activeSiteId === input.siteId
            ? pickFallbackActiveSiteId(sites, input.siteId)
            : state.activeSiteId,
      }
    })
  }
  ...
  async setActiveSite(input: { siteId: string }): Promise<PlanningSnapshot> {
    return this.updateState((state) => {
      const site = state.sites.find(
        (candidate) => candidate.id === input.siteId && !candidate.archivedAt,
      )
      if (!site) {
  ...
  async createQueueFromDrafts(input: {
  ...
  private resolvePlanningRootDir(): string {
    ...
    throw new Error(
      'PlanningStore requires getPlanningRootDir or getUserDataDir',
    )
  }
  ...
  private async loadState(filePath: string): Promise<PersistedPlanningState> {
    try {
      ...
      if (isMissingFileError(error)) {
        const emptyState = await this.ensureBundledCatalog(
          filePath,
          createEmptyPlanningState(),
        )
  ...
  private parseState(filePath: string, input: unknown): PersistedPlanningState {
    ...
    if (errors.length > 0) {
      throw new Error(
        `Invalid planning state in ${filePath}: ${errors.join('; ')}`,
      )
    }
  ...
  private async ensureBundledCatalog(
    filePath: string,
    state: PersistedPlanningState,
  ): Promise<PersistedPlanningState> {
    const starterErrors = validateStarterCatalogTargets()
    if (starterErrors.length > 0) {
      throw new Error(
        `Invalid bundled starter catalog: ${starterErrors.join('; ')}`,
      )
    }
...
function pickFallbackActiveSiteId(
  sites: SiteProfile[],
  archivedSiteId: string,
): string | undefined {
  return sites.find((site) => site.id !== archivedSiteId && !site.archivedAt)
    ?.id
}
