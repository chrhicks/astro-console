...
} from '../../shared/legacy/visibility-engine'
import {
  type CatalogTarget,
...
import {
  ...
  type SiteDiagnostic,
} from '../../shared/legacy/site-diagnostics'
import {
...
export class PlanningContextService {
  ...
  async resolveContext(
    ...
    if (!resolved) {
      throw new Error(
        createMissingActiveSiteMessage(snapshot, input.activeSiteId),
      )
    }
  ...
  async selectTarget(
    input: PlanningTargetSelectionInput = {},
  ): Promise<PlanningTargetSelection> {
    const context = await this.resolveContext(input)
    const recommendation = input.recommendation ?? 'good_now'
    const candidatesByScore = toRankedCandidates(
      context.snapshot.state.catalog,
      context.rankedTargets,
    )
    let candidates =
      recommendation === 'any'
        ? candidatesByScore
        : candidatesByScore.filter(
            (candidate) => candidate.ranking.recommendation === recommendation,
          )

    if (input.targetId) {
      candidates = candidates.filter(
        (candidate) => candidate.target.id === input.targetId,
      )
    }

    const query = input.query?.trim()
    if (query) {
      const searchResults = searchCatalogTargets(
        context.snapshot.state.catalog,
        query,
        context.snapshot.state.catalog.length,
      )
      const byTargetId = new Map(
        candidates.map((candidate) => [candidate.target.id, candidate]),
      )
...
export function createPlanningContextServiceFromStorage(
  input: PlanningContextStorageOptions,
...
export function createPlanningContextServiceFromStateFile(
  filePath: string,
): PlanningContextService {
  const store = new PlanningStore({
...
export function resolvePlanningActiveSite(
  ...
  if (requestedSiteId) {
    const requestedSite = availableSites.find(
      (site) => site.id === requestedSiteId,
    )
    return requestedSite
      ? { site: requestedSite, source: 'requested' }
      : undefined
  }

  const stateActiveSiteId = snapshot.state.activeSiteId
  if (stateActiveSiteId) {
    const stateSite = availableSites.find(
      (site) => site.id === stateActiveSiteId,
    )
    if (stateSite) {
...
function createMissingActiveSiteMessage(
  snapshot: PlanningSnapshot,
  ...
  const siteNames = availableSites
    .slice(0, 5)
    .map((site) => site.name)
    .join(', ')
...
function applySiteOverride(
  site: SiteProfile,
  override: PlanningSiteOverride | undefined,
): SiteProfile {
  if (!override) {
  ...
  if (override.blockedAzimuthRanges !== undefined) {
    nextSite.blockedAzimuthRanges = override.blockedAzimuthRanges.map(
      (range) => ({ ...range }),
    )
  }
...
function cloneSiteProfile(site: SiteProfile): SiteProfile {
  return {
    ...site,
    blockedAzimuthRanges: site.blockedAzimuthRanges.map((range) => ({
      ...range,
    })),
  }
}
