...
export class QueueRunner {
  ...
  async requestStop(): Promise<DesktopQueueRunnerState> {
    ...
    this.log(
      'info',
      'queue.run.stop_requested',
      'Operator requested queue stop',
      {
  ...
  private async run(runId: string, dryRun: boolean): Promise<void> {
    try {
      ...
      const activeSite = resolvePlanningActiveSite(snapshot, {
      ...
      await this.updateState(
        {
          ...
          completedCount: 0,
          summary: dryRun
            ? 'Queue validated for dry run'
            : 'Queue validated for execution',
        },
        'runner.validated',
      )
      this.log(
        'info',
        'queue.run.started',
        dryRun ? 'Started queue dry run' : 'Started queue run',
        {
          runId,
          dryRun,
      ...
      for (let index = 0; index < queue.length; index += 1) {
        ...
        if (dryRun) {
          ...
          const completed = await this.runLiveItem(
            runId,
            item,
            activeSite,
            index,
      ...
      await this.finishRun(
        'completed',
        dryRun ? 'Queue dry run completed' : 'Queue run completed',
        'queue.run.completed',
      )
    } catch (error) {
  ...
  private async runDryItem(
    ...
    if (
      item.notBeforeLocal &&
      isBeforeLocalTime(new Date(), site.timezone, item.notBeforeLocal)
    ) {
      await this.transition(
        'waiting',
        `Would wait until ${item.notBeforeLocal} local for ${item.targetName}`,
        {
    ...
    if (item.autofocusBeforeStart) {
      await this.transition(
        'focusing',
        `Would autofocus before starting ${item.targetName}`,
        {
          currentItemId: item.id,
          currentTargetName: item.targetName,
          currentIndex: index,
    ...
    this.log(
      ...
      `Completed dry-run item ${item.targetName}`,
      {
        runId,
        queueItemId: item.id,
        targetId: item.targetId,
  ...
  private async runLiveItem(
    ...
    if (mountClosed) {
      this.log(
        'info',
        'queue.preflight.arm.opening',
        `Opening arm before ${item.targetName}`,
        {
          runId,
          queueItemId: item.id,
        },
      ...
      await this.expectAccepted(
        ...
        'Device rejected arm-open request',
      )
    }
    ...
    if (item.autofocusBeforeStart) {
      await this.transition(
        'focusing',
        `Running autofocus on ${item.targetName}`,
        {
          currentItemId: item.id,
          currentTargetName: item.targetName,
          currentIndex: index,
    ...
    await this.transition(
      'stopping',
      `Stopping ${item.targetName}: ${stopCondition.summary}`,
      {
        currentItemId: item.id,
        currentTargetName: item.targetName,
        currentIndex: index,
    ...
    this.log(
      'info',
      'queue.item.completed',
      `Completed queue item ${item.targetName}`,
      {
        runId,
        queueItemId: item.id,
        targetId: item.targetId,
  ...
  private async waitUntilNotBefore(
    ...
    if (isBeforeLocalTime(new Date(), site.timezone, item.notBeforeLocal)) {
      this.log(
        'info',
        'queue.item.waiting',
        `Waiting until ${item.notBeforeLocal} local for ${item.targetName}`,
        {
          runId,
          queueItemId: item.id,
          targetId: item.targetId,
  ...
  private async waitForStopCondition(
    ...
    while (true) {
      ...
      if (!refreshed.planner.ready) {
        throw new Error(
          `Planner health degraded during run: ${refreshed.planner.issues.join('; ') || 'unknown issue'}`,
        )
      }
  ...
  private async stopActiveObservation(device: SeestarDevice): Promise<void> {
    ...
    if (isStackActive(viewState)) {
      await this.expectAccepted(
        ...
        'Device rejected stack stop while stopping queue item',
      )
    }
    ...
    if (isViewActive(nextViewState)) {
      await this.expectAccepted(
        ...
        'Device rejected view stop while stopping queue item',
      )
    }
  }
  ...
  private async startItem(
    ...
    this.log(
      'info',
      'queue.item.started',
      `${dryRun ? 'Dry run' : 'Executing'} queue item ${item.targetName}`,
      {
        runId,
        dryRun,
        queueItemId: item.id,
  ...
  private async updateState(
    nextState: DesktopQueueRunnerState,
    reason: string,
  ): Promise<void> {
...
function validateQueueRun(
  ...
  if (
    activeSite &&
    snapshot.state.queue.some((item) => item.siteId !== activeSite.id)
  ) {
    issues.push(
      'Queue contains items for a different site than the active session site',
    )
  }
  if (!dryRun) {
    ...
    if (!status.planner.ready) {
      issues.push(
        `Planner is not ready: ${status.planner.issues.join('; ') || 'unknown issue'}`,
      )
    }
...
function toFilterPosition(
  filter: QueueItem['requestedFilter'],
): number | undefined {
  switch (filter) {
    case 'clear':
      return 0
