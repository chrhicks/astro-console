...
export function validateStarterCatalogTargets(
  targets: CatalogTarget[] = STARTER_CATALOG_TARGETS,
): string[] {
  return validateCatalogTargets(targets)
}

export function mergeStarterCatalogTargets(
  existingTargets: CatalogTarget[],
): CatalogTarget[] {
...
export function searchCatalogTargets(
  ...
  if (!trimmed) {
    return catalog
      .slice(0, limit)
      .map((target) => ({ target, matchKind: 'text' }))
  }
  ...
  return catalog
    ...
    .sort(
      (left, right) =>
        ...
        left.target.primaryName.localeCompare(right.target.primaryName),
    )
    .slice(0, limit)
    .map(({ target, matchKind }) => ({ target, matchKind }))
}

export function createManualCatalogTarget(
  input: ManualCatalogTargetInput,
): CatalogTarget {
  const primaryName = input.primaryName.trim()
  const normalizedName = normalizeLookupKey(primaryName || 'target')
  const idSuffix =
    normalizedName || `${input.raHours.toFixed(4)}_${input.decDeg.toFixed(4)}`

  return {
    id: `manual:${idSuffix}`,
    primaryName:
      primaryName ||
      `RA ${input.raHours.toFixed(4)} Dec ${input.decDeg.toFixed(4)}`,
...
const STARTER_CATALOG_IDS = new Set(
  STARTER_CATALOG_TARGETS.map((target) => target.id),
)
...
function scoreCatalogMatch(
  ...
  if (
    target.aliases.some(
      (alias) => normalizeLookupKey(alias) === normalizedQuery,
    )
  ) {
