...
export function evaluateBackyardMask(
  site: SiteProfile,
  samples: TargetPathSample[],
): BackyardMaskResult {
  const skyVisibleSamples = samples.filter(
    (sample) => sample.altitudeDeg >= site.minAltitudeDeg,
  )
  const usableSamples = skyVisibleSamples.filter(
    (sample) => !isAzimuthBlocked(site.blockedAzimuthRanges, sample.azimuthDeg),
  )
  const blockedSamples = skyVisibleSamples.filter((sample) =>
    isAzimuthBlocked(site.blockedAzimuthRanges, sample.azimuthDeg),
  )
...
export function isAzimuthBlocked(
  ranges: SiteAzimuthRange[],
  azimuthDeg: number,
): boolean {
  return ranges.some((range) => matchesAzimuthRange(azimuthDeg, range))
}

export function matchesAzimuthRange(
  azimuthDeg: number,
