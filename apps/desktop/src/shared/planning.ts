...
export function validatePersistedPlanningState(
  state: PersistedPlanningState,
): string[] {
  const errors: string[] = []
  ...
  if (
    ...
    !isNonEmptyString(state.activeSiteId)
  ) {
  ...
  if (
    state.activeSiteId &&
    !state.sites.some(
      (site) => site.id === state.activeSiteId && !site.archivedAt,
    )
  ) {
    errors.push(
      `activeSiteId ${state.activeSiteId} does not match a non-archived site`,
    )
  }
...
export function validateSiteProfile(site: SiteProfile): string[] {
  ...
  if (site.archivedAt !== undefined && !isNonEmptyString(site.archivedAt)) {
    errors.push(
      `site ${String(site.id)} archivedAt must be non-empty when provided`,
    )
  }
...
export function validateSiteProfileDraft(
  ...
  if (
    !isFiniteNumber(site.minAltitudeDeg) ||
    site.minAltitudeDeg < 0 ||
  ...
  for (const range of site.blockedAzimuthRanges) {
    ...
    if (range.label !== undefined && !isNonEmptyString(range.label)) {
      errors.push(
        `${label} blocked azimuth label must be non-empty when provided`,
      )
    }
  }
...
export function validateCatalogTarget(target: CatalogTarget): string[] {
  ...
  if (
    ...
    target.aliases.some((alias) => !isNonEmptyString(alias))
  ) {
    errors.push(
      `catalog target ${String(target.id)} aliases must be an array of non-empty strings`,
    )
  }
  ...
  if (
    target.constellation !== undefined &&
    !isNonEmptyString(target.constellation)
  ) {
    errors.push(
      `catalog target ${String(target.id)} constellation must be non-empty when provided`,
    )
  }
  if (
    !isFiniteNumber(target.raHours) ||
    target.raHours < 0 ||
    target.raHours >= 24
  ) {
    errors.push(
      `catalog target ${String(target.id)} raHours must be between 0 and 24`,
    )
  }
  if (
    !isFiniteNumber(target.decDeg) ||
    target.decDeg < -90 ||
    target.decDeg > 90
  ) {
    errors.push(
      `catalog target ${String(target.id)} decDeg must be between -90 and 90`,
    )
  }
  if (target.magnitude !== undefined && !isFiniteNumber(target.magnitude)) {
    errors.push(
      `catalog target ${String(target.id)} magnitude must be finite when provided`,
    )
  }
  if (
    ...
    errors.push(
      `catalog target ${String(target.id)} recommendedFilter must be clear, ir, or lp when provided`,
    )
  }
  if (
    target.tags !== undefined &&
    (!Array.isArray(target.tags) ||
      target.tags.some((tag) => !isNonEmptyString(tag)))
  ) {
    errors.push(
      `catalog target ${String(target.id)} tags must be an array of non-empty strings when provided`,
    )
  }
  if (target.source !== 'starter' && target.source !== 'manual') {
    errors.push(
      `catalog target ${String(target.id)} source must be starter or manual`,
    )
  }
  ...
  for (const target of targets) {
    ...
    for (const key of createCatalogLookupKeys(target)) {
      ...
      if (currentOwner && currentOwner !== target.id) {
        errors.push(
          `catalog lookup key ${key} is shared by ${currentOwner} and ${target.id}`,
        )
        continue
      }
...
export function validateRankedTarget(target: RankedTarget): string[] {
  ...
  if (
    !isFiniteNumber(target.skyVisibleMinutes) ||
    target.skyVisibleMinutes < 0
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} skyVisibleMinutes must be 0 or greater`,
    )
  }
  if (!isNonEmptyString(target.evaluatedAt)) {
    errors.push(
      `ranked target ${String(target.targetId)} evaluatedAt is required`,
    )
  }
  if (!isFiniteNumber(target.visibleMinutes) || target.visibleMinutes < 0) {
    errors.push(
      `ranked target ${String(target.targetId)} visibleMinutes must be 0 or greater`,
    )
  }
  if (
    target.altitudeNowDeg !== undefined &&
    (!isFiniteNumber(target.altitudeNowDeg) ||
      target.altitudeNowDeg < -90 ||
      target.altitudeNowDeg > 90)
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} altitudeNowDeg must be between -90 and 90 when provided`,
    )
  }
  if (
    target.bestAltitudeDeg !== undefined &&
    (!isFiniteNumber(target.bestAltitudeDeg) ||
      target.bestAltitudeDeg < 0 ||
      target.bestAltitudeDeg > 90)
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} bestAltitudeDeg must be between 0 and 90 when provided`,
    )
  }
  if (
    target.windowStartAt !== undefined &&
    !isNonEmptyString(target.windowStartAt)
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} windowStartAt must be non-empty when provided`,
    )
  }
  if (
    target.windowEndAt !== undefined &&
    !isNonEmptyString(target.windowEndAt)
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} windowEndAt must be non-empty when provided`,
    )
  }
  if (
    target.moonSeparationDeg !== undefined &&
    (!isFiniteNumber(target.moonSeparationDeg) ||
      target.moonSeparationDeg < 0 ||
      target.moonSeparationDeg > 180)
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} moonSeparationDeg must be between 0 and 180 when provided`,
    )
  }
  if (
    ...
    errors.push(
      `ranked target ${String(target.targetId)} recommendation must be good_now, later_tonight, or not_tonight`,
    )
  }
  if (
    !Array.isArray(target.positiveReasons) ||
    target.positiveReasons.some((reason) => !isNonEmptyString(reason))
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} positiveReasons must be an array of non-empty strings`,
    )
  }
  if (
    !Array.isArray(target.rejectionReasons) ||
    target.rejectionReasons.some((reason) => !isNonEmptyString(reason))
  ) {
    errors.push(
      `ranked target ${String(target.targetId)} rejectionReasons must be an array of non-empty strings`,
    )
  }
...
export function validateQueueItem(item: QueueItem): string[] {
  ...
  if (
    !isFiniteNumber(item.targetRaHours) ||
    item.targetRaHours < 0 ||
    item.targetRaHours >= 24
  ) {
    errors.push(
      `queue item ${String(item.id)} targetRaHours must be between 0 and 24`,
    )
  }
  if (
    !isFiniteNumber(item.targetDecDeg) ||
    item.targetDecDeg < -90 ||
    item.targetDecDeg > 90
  ) {
    errors.push(
      `queue item ${String(item.id)} targetDecDeg must be between -90 and 90`,
    )
  }
  if (
    ...
    errors.push(
      `queue item ${String(item.id)} requestedFilter must be clear, ir, or lp when provided`,
    )
  }
  if (
    !isFiniteNumber(item.desiredDurationMin) ||
    item.desiredDurationMin <= 0
  ) {
    errors.push(
      `queue item ${String(item.id)} desiredDurationMin must be greater than 0`,
    )
  }
  if (
    item.notBeforeLocal !== undefined &&
    !isNonEmptyString(item.notBeforeLocal)
  ) {
    errors.push(
      `queue item ${String(item.id)} notBeforeLocal must be non-empty when provided`,
    )
  } else if (
    item.notBeforeLocal !== undefined &&
    !isValidLocalClock(item.notBeforeLocal)
  ) {
    errors.push(
      `queue item ${String(item.id)} notBeforeLocal must use HH:MM local time`,
    )
  }
  if (
    item.stopWhenBelowAltitudeDeg !== undefined &&
    (!isFiniteNumber(item.stopWhenBelowAltitudeDeg) ||
      item.stopWhenBelowAltitudeDeg < 0 ||
      item.stopWhenBelowAltitudeDeg > 90)
  ) {
    errors.push(
      `queue item ${String(item.id)} stopWhenBelowAltitudeDeg must be between 0 and 90 when provided`,
    )
  }
  if (
    item.status !== 'pending' &&
    ...
    errors.push(
      `queue item ${String(item.id)} status must be pending, running, completed, failed, cancelled, or skipped`,
    )
  }
...
export function validateQueueItems(
  ...
  const validSiteIds = new Set(
    sites.filter((site) => !site.archivedAt).map((site) => site.id),
  )
  ...
  for (const item of items) {
    ...
    if (!validSiteIds.has(item.siteId)) {
      errors.push(
        `queue item ${String(item.id)} siteId ${item.siteId} does not match an active site`,
      )
    }
    ...
    if (!catalogTarget) {
      errors.push(
        `queue item ${String(item.id)} targetId ${item.targetId} does not match the catalog`,
      )
      continue
    }
    ...
    if (catalogTarget.primaryName !== item.targetName) {
      errors.push(
        `queue item ${String(item.id)} targetName must match catalog target ${item.targetId}`,
      )
    }
    if (catalogTarget.raHours !== item.targetRaHours) {
      errors.push(
        `queue item ${String(item.id)} targetRaHours must match catalog target ${item.targetId}`,
      )
    }
    if (catalogTarget.decDeg !== item.targetDecDeg) {
      errors.push(
        `queue item ${String(item.id)} targetDecDeg must match catalog target ${item.targetId}`,
      )
    }
  }
