...
import { evaluateBackyardMask, isAzimuthBlocked, type TargetPathSample } from "./backyard-mask";
...
export function rankTargetsForWindow(input: {
  ...
  return input.targets
    .map((target) => rankTarget(input.site, target, observer, evaluationTime, sampleTimes))
...
export function resolveTonightWindow(
  ...
  const dusk = SearchAltitude(Body.Sun, observer, -1, currentTime, 2, ASTRONOMICAL_TWILIGHT_ALTITUDE_DEG);
  const dawn = SearchAltitude(Body.Sun, observer, +1, currentTime, 2, ASTRONOMICAL_TWILIGHT_ALTITUDE_DEG);

  const startAt = window?.startAt
    ? new Date(window.startAt)
    : sunAltitudeNow > ASTRONOMICAL_TWILIGHT_ALTITUDE_DEG && dusk && dawn && dusk.date > currentTime
      ? dusk.date
      : currentTime;

  const dawnAfterStart = SearchAltitude(Body.Sun, observer, +1, startAt, 2, ASTRONOMICAL_TWILIGHT_ALTITUDE_DEG);
  const endAt = window?.endAt
    ? new Date(window.endAt)
    : dawnAfterStart?.date ?? dawn?.date ?? new Date(startAt.getTime() + 12 * 60 * 60 * 1000);
...
function rankTarget(
  ...
  const currentPosition = readHorizontalCoordinates(target, observer, evaluationTime);
  ...
  const bestAltitudeDeg = samples.reduce((best, sample) => Math.max(best, sample.altitudeDeg), -90);
  const skyVisibleMinutes = computeVisibleMinutes(maskResult.skyVisibleSampleCount, sampleTimes);
  const visibleMinutes = computeVisibleMinutes(maskResult.usableSampleCount, sampleTimes);
  ...
  const recommendation = decideRecommendation(
    altitudeNowDeg >= site.minAltitudeDeg && !isAzimuthBlocked(site.blockedAzimuthRanges, currentPosition.azimuthDeg),
    visibleMinutes
  );
  ...
  if (recommendation === "good_now") {
    positiveReasons.push(`Above the ${site.minAltitudeDeg} deg altitude floor now`);
  } else if (recommendation === "later_tonight") {
    positiveReasons.push(`Becomes visible later tonight around ${formatLocalTime(windowStartAt)}`);
  } else {
    rejectionReasons.push(`No usable window clears the ${site.minAltitudeDeg} deg altitude floor tonight`);
  }

  if (skyVisibleMinutes >= MIN_USEFUL_VISIBLE_MINUTES) {
    positiveReasons.push(`Sky-visible for about ${skyVisibleMinutes} minutes tonight`);
  } else if (skyVisibleMinutes > 0) {
    rejectionReasons.push(`Only sky-visible for about ${skyVisibleMinutes} minutes tonight`);
  }

  if (visibleMinutes >= MIN_USEFUL_VISIBLE_MINUTES) {
    positiveReasons.push(`Usable from this site for about ${visibleMinutes} minutes tonight`);
    ...
    rejectionReasons.push(`Usable from this site for only about ${visibleMinutes} minutes tonight`);
  }

  if (skyVisibleMinutes > visibleMinutes && visibleMinutes > 0) {
    rejectionReasons.push(`Backyard mask removes about ${skyVisibleMinutes - visibleMinutes} minutes of otherwise visible time`);
  }

  if (bestAltitudeDeg >= site.minAltitudeDeg) {
    positiveReasons.push(`Peaks near ${bestAltitudeDeg.toFixed(1)} deg altitude`);
  }

  if (moonSeparationDeg >= LOW_MOON_SEPARATION_DEG) {
    positiveReasons.push(`Moon separation is ${moonSeparationDeg.toFixed(1)} deg`);
  } else {
    rejectionReasons.push(`Moon is close at ${moonSeparationDeg.toFixed(1)} deg`);
  }

  if (typeof target.magnitude === "number" && target.magnitude <= 6.5) {
    positiveReasons.push(`Catalog brightness is favorable at magnitude ${target.magnitude}`);
  }

  return {
    targetId: target.id,
    siteId: site.id,
    score: scoreTarget({ recommendation, altitudeNowDeg, bestAltitudeDeg, visibleMinutes, moonSeparationDeg, magnitude: target.magnitude }),
    evaluatedAt: evaluationTime.toISOString(),
    skyVisibleMinutes,
    visibleMinutes,
    altitudeNowDeg: roundToOne(altitudeNowDeg),
    bestAltitudeDeg: bestAltitudeDeg > -90 ? roundToOne(bestAltitudeDeg) : undefined,
    windowStartAt,
    windowEndAt,
...
function createSampleTimes(window: VisibilityWindow): Date[] {
  ...
  for (let current = start.getTime(); current <= end.getTime(); current += stepMs) {
...
function readAltitude(target: CatalogTarget, observer: Observer, time: Date): number {
  return readHorizontalCoordinates(target, observer, time).altitudeDeg;
}

function readMoonSeparation(target: CatalogTarget, observer: Observer, time: Date): number {
  const moonEquator = Equator(Body.Moon, time, observer, true, true);
  return sphericalSeparationDeg(target.raHours, target.decDeg, moonEquator.ra, moonEquator.dec);
}

function computeVisibleMinutes(visibleSampleCount: number, allSamples: Date[]): number {
  if (visibleSampleCount === 0) return 0;
  if (visibleSampleCount === 1) return 0;
  const averageStepMinutes =
    allSamples.length > 1 ? Math.round((allSamples[1].getTime() - allSamples[0].getTime()) / 60000) || DEFAULT_STEP_MINUTES : DEFAULT_STEP_MINUTES;
...
function scoreTarget(input: {
  ...
  const recommendationWeight =
    input.recommendation === "good_now" ? 45 : input.recommendation === "later_tonight" ? 25 : 0;
  ...
  const brightnessWeight = typeof input.magnitude === "number" ? Math.max(0, 10 - input.magnitude) : 0;
  const nowBonus = input.altitudeNowDeg > 0 ? Math.min(input.altitudeNowDeg, 70) * 0.35 : 0;

  return roundToOne(recommendationWeight + altitudeWeight + visibilityWeight + moonWeight + brightnessWeight + nowBonus);
}

function sphericalSeparationDeg(ra1Hours: number, dec1Deg: number, ra2Hours: number, dec2Deg: number): number {
...
function readHorizontalCoordinates(target: CatalogTarget, observer: Observer, time: Date): {
  azimuthDeg: number;
  altitudeDeg: number;
} {
  const horizontal = Horizon(time, observer, target.raHours, target.decDeg, "normal");
  return {
