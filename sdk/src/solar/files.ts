...
export function createSolarOutputLayout(
...
export async function ensureSolarLayout(
...
export async function stageSourceAsset(
  inputPath: string,
  layout: SolarOutputLayout,
): Promise<{ sourceVideoPath: string; sourceCompanionPaths: string[] }> {
...
export async function writeSolarMetadata(
  metadataPath: string,
  payload: unknown,
): Promise<void> {
  await writeFile(
...
export async function createOrReplaceSymlink(
  targetPath: string,
  linkPath: string,
): Promise<void> {
  try {
