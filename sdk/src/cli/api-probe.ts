...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const probe of DEFAULT_METHODS) {
      try {
        const response = await client.sendSync(probe.method, probe.params, timeoutMs);
        results.push({
          method: probe.method,
          params: probe.params,
          ok: response.code === 0,
    ...
    if (args.json) {
      console.log(JSON.stringify({ host, discoveredDevices, authenticated, results }, null, 2));
      return;
    }

    console.log(`API probe completed for ${host}.`);
    if (discoveredDevices.length > 0) {
      console.log(`- discovered: ${discoveredDevices.map((device) => device.host).join(", ")}`);
    }
    console.log(`- authenticated: ${authenticated ? "yes" : "no"}`);
    for (const result of results) {
      const status = result.supported ? (result.ok ? "ok" : "error") : "unsupported";
      console.log(`- ${result.method}: ${status}${result.code !== undefined ? ` (code ${result.code})` : ""}`);
      if (result.error) {
...
function sliceObject(record: Record<string, unknown>, limit: number): Record<string, unknown> {
