...
function parseArgs(argv: string[]): ExtractArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (key === 'subframes')
      out.subframesMode = asSubframesMode(value, out.subframesMode)
    if (key === 'sample-count')
      out.sampleCount = asPositiveInt(value, out.sampleCount)
    if (key === 'frame-cadence-sec')
      out.frameCadenceSec = asPositiveInt(value, out.frameCadenceSec)
    if (key === 'search-window-sec')
      out.searchWindowSec = asNonNegativeInt(value, out.searchWindowSec)
  }
...
function asSubframesMode(
  value: string | undefined,
  fallback: ExtractArgs['subframesMode'],
): ExtractArgs['subframesMode'] {
  return value === 'all' || value === 'sample' ? value : fallback
}

function flattenAlbumEntries(
  entries: Array<{
    groupName?: string
    files: Array<{ name: string; thn: string; count?: number; type?: number }>
  }>,
): AlbumAssetEntry[] {
...
function pickAlbumEntry(
  ...
  const normalizedMatches = entries.filter(
    (entry) => normalizeAlbumName(entry.name) === normalizedName,
  )
...
function pickLatestEntry(entries: AlbumAssetEntry[]): AlbumAssetEntry {
  return [...entries].sort((left, right) =>
    extractTimestampToken(right.thn).localeCompare(
      extractTimestampToken(left.thn),
    ),
  )[0]!
}
...
async function downloadStillEntry(
  ...
  try {
    fit = await downloadAsset(
      device.resolveImageUrl(entry.thn, false, '.fit'),
      outDir,
    )
    console.log(`Downloaded ${entry.name} FIT: ${fit.fileName}`)
  } catch (error) {
    console.warn(
      `Could not download FIT for ${entry.name}: ${toErrorMessage(error)}`,
    )
  }
...
async function downloadSampledSubs(
  ...
  const totalCount = Math.max(
    subEntry.count ?? options.sampleCount,
    options.sampleCount,
  )
  ...
  for (let index = 0; index < options.sampleCount; index += 1) {
    const sampleFraction =
      options.sampleCount === 1 ? 0 : index / (options.sampleCount - 1)
    const frameOffset = Math.round((totalCount - 1) * sampleFraction)
    const estimate = new Date(
      latestDate.getTime() - frameOffset * options.frameCadenceSec * 1000,
    )
...
async function downloadAllSubs(
  ...
  const latest = await downloadKnownSubframe(
    device,
    latestThumbPath,
    latestDate,
    outDir,
  ...
  if (downloads.length < expectedCount) {
    console.warn(
      `Expected about ${expectedCount} subframes but downloaded ${downloads.length}`,
    )
  }
...
async function findNearbySubframe(
  ...
  for (let delta = 0; delta <= searchWindowSec; delta += 1) {
    ...
    for (const offsetSeconds of secondOffsets) {
      ...
      try {
        return await downloadKnownSubframe(
          device,
          candidateThumbPath,
          candidateDate,
          outDir,
...
async function downloadKnownSubframe(
  ...
  const jpg = await downloadAsset(
    device.resolveImageUrl(candidateThumbPath, false, '.jpg'),
    outDir,
  )
  ...
  try {
    fit = await downloadAsset(
      device.resolveImageUrl(candidateThumbPath, false, '.fit'),
      outDir,
    )
    console.log(`Downloaded ${modeLabel} subframe FIT: ${fit.fileName}`)
  } catch (error) {
    console.warn(
      `Could not download FIT for ${jpg.fileName}: ${toErrorMessage(error)}`,
    )
  }
