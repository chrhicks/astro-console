  ...
  try {
    while (!shutdown.signal.aborted) {
      ...
      try {
        if (!connected) {
          ...
          if (reconnectAttempt > 0) {
            writeNotice(
              args,
              'info',
              `Reconnected after ${reconnectAttempt} failed poll cycles`,
            )
            reconnectAttempt = 0
          } else {
            writeNotice(
              args,
              'info',
              'Connected and authenticated for read-only stack watch',
            )
          }
        }
        ...
        if (args.json) {
          ...
          if (payload.warnings.length > 0) {
            writeNotice(
              args,
              'warn',
              `Partial poll warnings: ${payload.warnings.join(' | ')}`,
            )
          }
        }
        ...
        if (
          args.once ||
          (args.maxSamples !== undefined && sampleCount >= args.maxSamples)
        ) {
          break
        }
...
async function buildSnapshot(
  ...
  const equRead = await safeRead('scope_get_equ_coord', () =>
    device.getEquCoord(),
  )
  const albumsRead = await safeRead('get_albums', () => device.getAlbums())

  const failedReads = [viewRead, equRead, albumsRead].filter(
  ...
  const stacked =
    findNumberByKeys(viewState, STACKED_COUNTER_KEYS) ??
    previousCounters.stacked
  const dropped =
    findNumberByKeys(viewState, DROPPED_COUNTER_KEYS) ??
    previousCounters.dropped
  const coordinates = normalizeEquCoord(equRead.value)
  const albums = summarizeAlbums(
    albumsRead.value,
    targetName,
    args.albumSampleSize,
  )
...
function normalizeEquCoord(value: EquCoord | null): {
  raHours?: number
  decDeg?: number
} {
  if (!value) {
...
function summarizeAlbums(
  ...
  const stackCandidates = targetName
    ? entries.filter(
        (entry) =>
          normalizeAlbumName(entry.name) === normalizeAlbumName(targetName),
      )
  ...
  const subCandidates = targetName
    ? entries.filter((entry) => {
        ...
        return (
          normalized === `${normalizedTarget}_sub` ||
  ...
  const recentEntries = [...entries]
    .sort((left, right) =>
      compareTimestampTokens(right.capturedAtToken, left.capturedAtToken),
    )
    .slice(0, sampleSize)
...
function pickLatestEntry(
  entries: AlbumSummaryEntry[],
): AlbumSummaryEntry | undefined {
  if (entries.length === 0) {
    return undefined
  }
...
function compareTimestampTokens(
...
function formatStatusLine(payload: StackWatchStatusPayload): string {
  ...
  const stacked =
    payload.frames.stacked !== undefined
      ? String(payload.frames.stacked)
      : 'n/a'
  const dropped =
    payload.frames.dropped !== undefined
      ? String(payload.frames.dropped)
      : 'n/a'
  const coord =
    payload.coordinates.raHours !== undefined &&
    payload.coordinates.decDeg !== undefined
  ...
  const warningLabel =
    payload.warnings.length > 0 ? ` warnings=${payload.warnings.length}` : ''
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (key) {
      ...
      case 'discovery-timeout-ms':
        out.discoveryTimeoutMs = parsePositiveInteger(
          '--discovery-timeout-ms',
          value,
        )
        break
      case 'reconnect-delay-sec':
        out.reconnectDelaySec = parsePositiveNumber(
          '--reconnect-delay-sec',
          value,
        )
        break
