  ...
  async sendSync(
    ...
    while (Date.now() - start < timeout) {
      ...
      if (!this.connected || !this.socket || this.socket.destroyed) {
        ...
        throw new Error(
          `Connection closed while waiting for response to ${method} (id=${id})`,
        )
      }
      await sleep(50)
    }
  ...
  async waitForPushEvent(
    ...
    return new Promise((resolve, reject) => {
      ...
      timeoutHandle = setTimeout(() => {
        ...
        finish(() =>
          reject(
            new Error(`Timeout waiting for pushed event after ${timeoutMs}ms`),
          ),
        )
      }, timeoutMs)
    })
    ...
    while ((idx = this.receiveBuffer.indexOf('\r\n')) >= 0) {
      ...
      try {
        ...
        if (typeof parsed.Event === 'string') {
          ...
          emitLog(this.logger, {
            ...
            data: {
              ...
              preview: this.traceProtocol
                ? previewValue(pushEvent, undefined)
                : undefined,
            },
          })
        ...
        emitLog(this.logger, {
          ...
          data: {
            code: response.code,
            resultPreview: this.traceProtocol
              ? previewValue(response.result, response.method)
              : undefined,
          },
        })
...
function previewValue(value: unknown, method?: string): unknown {
  if (
    ...
    return {
      dataLength:
        typeof record.data === 'string' ? record.data.length : undefined,
      sign: '[redacted]',
    }
  }
  if (
