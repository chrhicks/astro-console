...
async function main() {
  ...
  const solarFiles = albums.list.flatMap((e) => e.files).filter((f) =>
  ...
  for (const file of solarFiles) {
    const isVideo = file.name.toLowerCase().includes("video") || file.name.toLowerCase().includes("timelapse");
    ...
    try {
      ...
      console.log(`  Primary failed, trying fallback ${fallbackUrl} -> ${fallbackDest}`);
      try {
