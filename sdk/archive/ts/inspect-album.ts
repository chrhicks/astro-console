...
async function main() {
  ...
  const pemPath =
    process.env.SEESTAR_PEM || '../seestar_3.1.2_fw_7.32_interop.pem'
  const albumName =
    process.env.SEESTAR_ALBUM || process.argv[2] || 'Solar_video'
  ...
  for (const entry of entries) {
    ...
    console.log(
      `${kind.padEnd(4)} ${String(entry.sizeBytes).padStart(9)}  ${entry.modifiedRaw}  ${entry.name}`,
    )
  }
