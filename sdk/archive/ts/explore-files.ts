...
async function main() {
  ...
  for (const method of methods) {
    try {
      ...
      console.log(`   ${method}: code=${resp.code}, has_result=${resp.result !== undefined}`);
      if (resp.result) {
  ...
  try {
    const resp = await device.rawClient.sendSync("get_albums", { folder: "Solar_video" });
    console.log(`   code=${resp.code}, result=${JSON.stringify(resp.result).slice(0, 300)}`);
  } catch (e: any) {
