#!/usr/bin/env python3
"""Create a careful final from a PlanetarySystemStacker TIFF.

Goals:
- keep the result natural enough for evaluation
- improve local sunspot detail without obvious halos
- avoid the heavy-handed CLAHE look from the earlier prototype
"""
...
def flatten_limb(gray_norm: np.ndarray, mask: np.ndarray) -> np.ndarray:
    # Divide by a large smooth background estimate to reduce limb-darkening influence,
    # but only blend it in mildly so the disk still looks natural.
...
def finish(gray16: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    mask, radius = detect_disk(gray16)
    ...
    # Blend final result softly back into a black background.
    base8 = to_uint8(gray16, mask, low=0.2, high=99.8)
    final = (sharpened.astype(np.float32) * mask_soft + 0.0 * (1.0 - mask_soft)).clip(0, 255).astype(np.uint8)
    natural = (base8.astype(np.float32) * mask_soft).clip(0, 255).astype(np.uint8)

    return natural, final, mask
...
def make_sheet(images: list[tuple[str, Path]], out_path: Path, detail: bool = False) -> None:
    ...
    for label, path in images:
        ...
        if detail:
            ...
            cy = int(h * 0.39)
            r = int(min(w, h) * 0.11)
            img = img.crop((cx - r, cy - r, cx + r, cy + r)).resize((520, 520), Image.Resampling.LANCZOS)
            canvas = Image.new("RGB", (520, 560), "black")
        else:
            target_h = 900
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Finish a PSS TIFF into evaluation-ready solar finals")
    parser.add_argument("input", nargs="+", help="Input PSS TIFF(s)")
    parser.add_argument("--output-dir", "-o", default="./prototype_output/finals", help="Output directory")
    ...
    for input_name in args.input:
        ...
        label = stem.replace("2026-05-08-103245-Solar-timelapse-RAW_", "").replace("_", " ")
        rendered.append((f"{label} natural", natural_path))
        rendered.append((f"{label} final", final_path))

    if rendered:
        make_sheet(rendered, output_dir / "finals_overview.png", detail=False)
        make_sheet(rendered, output_dir / "finals_detail.png", detail=True)

    print(f"Final outputs written to: {output_dir}")
